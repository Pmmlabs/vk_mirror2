...
function postIt() {
 ...
 Ajax.postWithCaptcha('apps.php', {'act': 'getcomms', 'hash': ge('hash').value, 'n': 3, 'seed': ge('reply_field').value, 'mid': ge('id').value}, {onSuccess: postCommentSuccess, onFail: postCommentFail});
 window.status = 'Загрузка...';
...
function orderingInit(mid){
  var container = ge('appRows');
    ...
    setStyle(this.sortHelper, {backgroundColor: "#FFFFFF", marginLeft:10, padding:0, width:605});
    if (!browser.msie) setStyle(this.sortHelper, {marginBottom: 5});
...
function createApp(form) {
  ...
  Ajax.postWithCaptcha('apps.php', queryData, {onSuccess: function(obj, text) {
    try {
...
function disable_app(id) {
	...
	ajax.post('apps.php', {'act': 'disable', 'id': id});
}
...
function enable_app(id) {
	...
	ajax.post('apps.php', {'act': 'enable', 'id': id});
}


function join_app(app_id, hash) {
  Ajax.postWithCaptcha('apps.php?act=join', {id: app_id, hash: hash}, {
...
function getTransactions(offset, admin) {
  var query = {act: 'get_transactions', offset: offset, id:ge('app_id').value, admin: admin};
...
function changeComment(act, cid) {
  ...
  ajax.post('/apps.php', {'act': act, 'cid': cid});
}
...
function editComment(cid) {
  ...
  ajax.post('/apps.php', {'act': 'a_get_edit_comment', 'cid': cid});
}
...
function doEditComment(cid) {
  ...
  ajax.post('/apps.php', {'act': 'a_edit_comment', 'cid': cid, 'comment': ge('comment' + cid + 'edit').value});
}
...
function deleteMerchant(id, hash) {
  hide(ge('ma' + id).getElementsByTagName('small')[0]);
	...
	ajax.post('/merchants.php', {act: 'a_delete_merchant', id: id, hash: hash});
}
...
function restoreMerchant(id, hash) {
  ge('ma' + id).innerHTML += '<img src="images/upload.gif"/>';
	...
	ajax.post('/merchants.php', {act: 'a_restore_merchant', id: id, hash: hash});
}
