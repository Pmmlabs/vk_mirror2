...
function set_default_params() {
  ...
  if ((params['act'] == 's') || (params['act'] == 'default')) {
    ...
    if (!ge('filter_link')) {
      ge('results_header').innerHTML = "<a style='float: right; margin-top: 7px' id='filter_link' href='javascript: show_section_tree()'>Фильтр&#9660;</a>" + ge('results_header').innerHTML;
    }
...
function update_page_links()
{
  ...
  if (params['act'] == 't') {
    ...
    ge('current_posts').innerHTML = 'Тема';
  }
...
function show_section_tree() {
  ge('filter_link').innerHTML = 'Фильтр&#9650;';
...
function ban_user() {
  ...
  ajax.onDone = ajax_handler(function(text) {
    ...
    if (result == 'd') {
      ...
      show_message_box('message', 'Пользователь удачно забанен :)');
    } else if (result == 'f') {
      show_message_box('message', 'Пользователь забанен и все его сообщения удалены.');
    } else if (result == 'l') {
      show_message_box('error', 'Этот пользователь является руководителем.');
      return;
    }
...
function unban_user(index) {
  ...
  ajax.onDone = ajax_handler(function(text) {
    var result = text.substr(0, 1);
    if (result == 'n')
      show_message_box('error', 'Этого пользователя нету в списке.');
    if (result == 'g')
      show_message_box('message', 'Пользователь удален из списка.');
    ge('memberPages').innerHTML = text.substr(1);
  });
...
function showAutoAnswersBox(tid) {
  createBox();
  ge('boxTitle').innerHTML = 'Автоответы';
  str = "<div id='autoAnswers'><div id='answersTable'></div><div id='addAnswer'>";
  str += "<div><table style='margin-top:5px;margin-bottom:5px;' cellpadding=5><tbody><tr><td style='width:97px;'><input id='answerName' type='text' style='text-align:left; width:55px; padding: 2px 10px 0px;' value=''></td><td><a href='javascript:addAutoAnswer();'>Добавить</a></td></tr></tbody></table></div>";
...
function addAutoAnswer() {
  ...
  if (answer_name && answer_text) {
    ...
    alert("Введите название и текст ответа.");
  }
}
...
function can_be_notified(value) {
  ...
  if (value) {
    ...
    ge('can_be_notified').innerHTML = 'Запретить извещения';
  } else {
    ge('can_be_notified').href = 'javascript: can_be_notified(1)';
    ge('can_be_notified').innerHTML = 'Разрешить извещения';
  }
}
