PhotoViewer = function(options) {
  ...
  function setCanvas() {
    ...
    if (needIEFix) {
      setStyle(ge('pv_full_photo_img'), {position: 'static'});
    }
  ...
  function closeViewer() {
    ...
    if ($ah.enabled && pid != cur_photo) {
      ahOrigHandlers['before']({act:'photo_info', photo: pid, uid: window.watched_uid});
  ...
  function setFastNavigationArrows(range) {
    if (range[0] <= 0 && currentIdx == 0) {
      ...
      setStyle(ge('pv_bottom_layer_bg_left'), {opacity: 0.5});
      ...
      setStyle(ge('pv_bottom_layer_bg_right'), {opacity: 0.5});
  ...
  function getMainImageVersion(idx, maxViewSize, imageRatio) {
    var
      photo = photosList[idx], i,
      sizesTypes = {6: [2560, 2048], 5: [1280, 1024], 4: [807, 807], 3: [604, 604]},
      typeIdx,
      compFn,
      sizesDiff = {},
      sizeMinDiff = false,
  ...
  function setImageWrap(size, top) {
    var xy;
    setStyle(ge('pv_full_photo_outer_1'), {top: top - 1, width: size[0], height: size[1]});
    setStyle(ge('pv_full_photo_outer_2'), {top: top - 2, width: size[0] + 2, height: size[1]+2});
    setStyle(ge('pv_full_photo_outer_3'), {top: top - 3, width: size[0] + 4, height: size[1]+4});
    if (needIEFix) {
  ...
  function setImageWrapXY(e) {
    ...
    setStyle(ge('pv_full_photo_outer_1'), {left: xy[0]});
    setStyle(ge('pv_full_photo_outer_2'), {left: xy[0]});
    setStyle(ge('pv_full_photo_outer_3'), {left: xy[0]});
  }
...
'</div>'
  ...
  function setHash() {
    ...
    if ($ah.enabled && pid != start_photo && pid != currentHashPID) {
      currentHashPID = pid;
      $ah.go("photo", {act:'photo_info', photo: pid, uid: window.watched_uid});
    }
  }
