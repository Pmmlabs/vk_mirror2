var notes = {
  changeComment: function(act, oid, nid, cid, hash) {
    ...
    ajax.post('/notes', {act: act, oid: oid, cid: cid, hash: hash, comment: oid + '_' + cid}, {onDone: function(text) {
  ...
  editComment: function(oid, nid, cid) {
    ...
    var onDone = function(text) {
      ...
      var margin_top = '-3px', margin_left = '-5px'; // For Firefox, IE, Safari.
      var padding_left = '';
      if (browser.chrome) {
        margin_left = '-4px';
    ...
    ajax.post('/notes', {'act': 'a_get_edit_comment', 'oid': oid, 'cid': cid}, {onDone: onDone});
  },
  ...
  doEditComment: function(oid, cid) {
    ...
    ajax.post('/notes', {'act': 'a_edit_comment', 'oid': oid, 'cid': cid, 'comment': ge('comment' + cid + 'edit').value}, {onDone: onDone});
  },
  ...
  postComment: function() {
    ...
    if (comment) {
     ...
     var onCaptchaShow = function() {ge('notes_progr').style.display = 'none';};
     var onCaptchaHide = function() {};
     var options = {onDone: callback, onFail: onFail, onCaptchaShow: onCaptchaShow, onCaptchaHide: onCaptchaHide};
...
cur.lang.notes_livejournal_delete + '</div></div>' : '';
    ...
    var fastBox = showFastBox({title: cur.lang.notes_deletion}, content, cur.lang.notes_delete, function() {
      var lj_del = (lj && isChecked('lj_delete')) ? '1' : '0';
      fastBox.content('<div style="text-align: center">' + cur.lang.notes_deleting + '</div>');
      ajax.post('/notes', {act: 'do_delete', nid: nid, hash: hash, lj: lj_del}, {
        onDone: function() {
          fastBox.hide();
          nav.reload(true);
        },
  ...
  doPreloadNote: function() {
    ...
    var url = '/notes';
    var params = {act: 'notecomments', oid: oid, nid: nid};
  ...
  showNote: function(oid, nid, ncom, toComments) {
    ...
    //Comments preload
    ajax.post('/notes', {act: 'notecomments', oid: oid, nid: nid}, {
      cache: true,
      forceGlobalCache: true,
      onDone: function(comments, actions, options) {
        var commentsDiv = ce('div', {id: 'blogcommentsandform'}, {paddingLeft: '20px', paddingRight: '20px'});
  ...
  showMoreComments: function(oid, nid) {
    ...
    ajax.post('/al_notes.php', {act: 'more', oid: oid, nid: nid, before_id: firstComment, max_id: lastComment, all_comments: cur.num_comments}, {
...
//        geByClass1('notes_comment_body', commentsBody).style.borderTop = '';
  ...
  showAllComments: function(oid, nid) {
    ...
    var showMoreSize = getSize(geByClass1('notes_comments_showmore'))[1];
    ge('more_progress').previousSibling.data = '';
    ajax.post('/al_notes.php', {act: 'all', oid: oid, nid: nid, before_id: firstComment, max_id: lastComment}, {
...
//        geByClass1('notes_comment_body', commentsBody).style.borderTop = '';
  ...
  viewPhoto: function(event, photoRawId, href, text, controls_text) {
    ...
    var onReady = function(ajaxObj, responseText) {
      if (responseText == 'NO_ACCESS') {
        ...
        if (window.showWikiBox) {
          showWikiBox('photo', { src: responseText, full: href, text: text }, controls_text);
          return false;
        } else {
          return true;
        }
    ...
    ajax.post({ url: 'photos.php', query: { act: 'get_photo', photo: photoID.replace('photo', ''), rand: Math.random() }, onDone: onReady.bind(this) });
  },
  ...
  showMore: function(updated) {
    ...
    var next_rows = ge('notes_rows_next'), notes_rows = ge('notes');
    ...
    ajax.post('/notes', {act: cur.act, section: cur.section, id: cur.oid, offset: offset, part: 1}, {
      onDone: function (options, rows, comms_num) {
        ...
        if (rows) {
          var au = ce('div'), row, cont = updated ? next_rows : notes_rows;
          au.innerHTML = rows;
  ...
  saveNote: function(mid) {
    ...
    ajax.post('/notes', {act: act, hash: form.hash.value, title: form.title.value, wysiwyg: formWysiwyg, nid: mid,
                                ...
                                Post: form.Post.value}, {onDone: function() {}, onFail: function() {}});
  },
  ...
  showPhotosBox: function(url, photo_id, photo_index, target) {
    ...
    if (!cur.pic.width) {
      ...
      //show('imageProgress');
      cur.showTimer = setTimeout(function() {notes.showPhotosBox(url, photo_id, photo_index, target);}, 1500);
      return false;
    }

    var params = {};
    cur.photoBox = new MessageBox(params);
    cur.photoBox.addButton('Закрыть', function() {
  ...
  renderPhoto: function(url, photo_index, noteIndex) {
    ...
    cur.photoBox.setOptions({title: title, width: parseInt(cur.pic.width) + 30});
  ...
  init: function(options) {
    extend(cur, {uid: options.viewer_id});
    if (options.user_id) {
      extend(cur, {oid: options.user_id});
    }
    if (!cur.options) {
      cur.options = {};
    }
...
try{stManager.done('notes.js');}catch(e){}
