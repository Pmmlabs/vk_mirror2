var SiteStats = {
  toggleSmallGraphs: function(event, graphType) {
    ...
    each(geByTag('img', ge('site_stats_small_graphs_'+graphType)), function(i, el) { el.setAttribute('src', el.getAttribute('src1')); });
  ...
  showReferrersStat: function(type, id, word) {
    showBox('site_stats.php', {act: 'a_referrers_box', type: type, oid: id, word: word}, {params: {width: 650, hideButtons: true, bodyStyle: 'padding: 15px 25px;'}, onFail: function() {
      return true;
    }});
    return false;
  }
...
try{stManager.done('site_stats.js');}catch(e){}
