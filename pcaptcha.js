var PhotoCaptcha = {
  init: function(box, opts) {
    ...
    box.setOptions({hideButtons: true, width: 647, bodyStyle: 'padding: 0px; border: 0px;'});
    box.removeButtons();
  ...
  show: function(i) {
    ...
    cur.pcShowing = true;
    var el = cur.pcWrapEl.appendChild(domFC(ce('div', {innerHTML: '\
<table class="pcpt_photo" cellspacing="0" cellpadding="0" style="position: absolute; opacity: 0;">\
  <tr><td class="pcpt_cell" onclick="PhotoCaptcha.show()"><img src="' + cur.pcPhotos[i] + '" class="pcpt_big_img" /></td></tr>\
    ...
    setStyle(domPS(el), {position: 'absolute', marginLeft: 0});
    if (i > cur.pcCur) {
      setStyle(el, {marginLeft: 608});
      to = -608;
    } else {
      setStyle(el, {marginLeft: -608});
      to = 608;
    }
    animate(el, {opacity: 1, marginLeft: 0}, {duration: 200, transition: Fx.Transitions.sineInOut});
    animate(domPS(el), {opacity: 0, marginLeft: to}, {duration: 200, transition: Fx.Transitions.sineInOut, onComplete: PhotoCaptcha.done});
    cur.pcCur = i;
  },
  done: function() {
    if (!cur.pcShowing) return;
    re(domFC(cur.pcWrapEl));
    setStyle(domFC(cur.pcWrapEl), {position: 'static', marginLeft: 0});
  ...
  other: function() {
    ajax.post('pcaptcha.php', {act: 'box', part: 1}, {onDone: function(photosArr, photos, friends) {
      delete(cur.pcSel);
      extend(cur, {
        pcCur: 0,
        pcShowing: false,
...
try{stManager.done('pcaptcha.js');}catch(e){}