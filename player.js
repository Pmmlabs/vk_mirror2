...
var AudioObject = {
  ...
  operate: function() {
    ...
    var decrypt = function(str, pwd) {
      ...
      var s = Math.floor(r.length / 3), m = parseInt(r.charAt(s) + r.charAt(s*2) + r.charAt(s*3), 10), n = Math.round(pwd.length / 2), d = 8388607, l = parseInt(str.substring(str.length - 5, str.length), 16);
      ...
      for(var i=0; i<str.length; i+=2) {
        ...
        t += String.fromCharCode(c); r = (m * r + n) % d;
      }
      return t;
    };
    ...
    if (!oldId) {
      ...
      if (id == oldId) {
        ...
        setTimeout(function(){ AudioObject.showPlayer(id); }, 0);
      }
    }
  },
  ...
  playback: function(id) {
    ...
    if(id && window.audioData && (audioData.hint || audioData.act == 'edit')) {
      ...
      this.playbackTO = setTimeout( function(){
        Ajax.Post({url:'audio.php', query:{act:'a_playback', id:realId}});
        _t.playbackSent = id;
      }, 10000);
    }
  },
  ...
  hidePlayer: function(id){
    ...
    this.curAudio = null;
    var mode = (window.audioEditMode) ? "editmode" : "playmode";
    setTimeout(function(){ hide('repeat' + id); removeClass(ge('audio_add'+id), 'active'); show('line' + id); }, 0);
    ge("imgbutton" + id).src = this.images[mode].play;
    setTimeout(function(){ AudioObject.setIcon(AudioObject.images.icon); }, 0);
  },
  ...
  nextTrack: function(id) {
    if (this.repeat) {
      ...
      this.playback();
      this.playbackSent = 0;
      setTimeout(function() { ge('imgbutton' + id).onclick(); }, 0);
      return;
    }
    ...
    if (nextId && !window.audioEditMode) {
      setTimeout(function(){ ge('imgbutton' + nextId).onclick(); }, 0);
    }
...
function addMeAudio(wid, oid, aid, hash, el) {
  ...
  Ajax.Send('audio.php', {act: 'a_add', aid: aid, oid: oid, hash: hash, from_profile: ge('my_audios') ? 1 : ''}, {onSuccess: function(obj, text) {
    posting_on_wall = false;
    if (ge('my_audios')) {
    ...
    if (wid) setTimeout(function() { fadeOut(ge('adding' + wid + 'done'), 500); }, 1000);
...
function showExtraBox(extra, vars, elem) {
	...
	if (!window.videoBox) {
    window.videoBox = new MessageBox({width: 629, bodyStyle: 'padding: 10px', closeButton: true, title: vars.title, onHideAttempt: function() {
    ...
    window.videoBox.setOptions({width: 629, title: vars.title});
  }
  ...
  window.videoBox.addButton({label: global_close, onClick: function() {
    window.videoBox.hide();
  }});
	if (vars.status) {
	  var params = {act: 'a_show_extra', status: vars.status};
	} else {
	  var params = {act: 'a_show_extra', vid: vars.vid, oid: vars.oid, hash: vars.hash};
	}
	Ajax.Send('video.php', params, {onSuccess: function(obj, text) {
    var data = text.split('<!>');
...
function showVideoBox(vars, elem, description, to_comments_text, add_text, add_hash, thumb, player_available, allow_html5, player_version) {
  ...
  if (!window.videoBox) {
    ...
    if (add_hash) {
      window.videoBox.setOptions({fullPageLink:elem.href});
    }
  } else {
    window.videoBox.setOptions({width:ww + 22});
  }
  window.videoBox.removeButtons();
  window.videoBox.addButton({label: global_close, onClick: function() {
    window.videoBox.hide();
  ...
  window.videoBox.addControlsText(controls);
  window.videoBox.setOptions({title:title});
  if (add_hash) {
    window.videoBox.setOptions({fullPageLink:elem.href});
  }
...
function loadFlashPlayer(vars, player_available, allow_html5, player_version) {
  ...
  swfobject.embedSWF('/swf/VideoPlayer4_0.swf?' + player_version, 'video_player', '' + ww, '' + hh, '9', '', vars, params, {preventHide: true});
...
function incViewCounter(oid, vid, hash) {
  ...
  ajax.post("/video.php", {act:'inc_view_counter', oid:oid, vid:vid, hash:hash});
}

function changeCanvasSize(w, h) {
  if (!browser.opera || intval(browser.version) >= 10) {
    ge('flash_player_back').innerHTML = '';
    setStyle(ge('video_player'), {width: w, height: h});
    setStyle(ge('flash_player_container_outer'), {height: h});
    if (window.videoBox != undefined) {
      window.videoBox.setOptions({width:w + 22});
    }
  }
}
