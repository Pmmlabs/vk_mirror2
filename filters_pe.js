var FiltersPE = {
  ...
  onActionCrop: function(event, ref) {
    ...
    setStyle(cropAreaEl, {width: cnvSize[0], height: cnvSize[1]});
    ...
    each('Top Left Right Bottom'.split(' '), function(i, side) {
      ...
      setStyle(cropAreaEl, { bname: cur._peCrop[side.toLowerCase()[0]] });
    });

    function updateCropArea() {
      setStyle(cropAreaEl, { borderLeftWidth: cur._peCrop.l });
      setStyle(cropAreaEl, { borderRightWidth: cur._peCrop.r });
      setStyle(cropAreaEl, { borderTopWidth: cur._peCrop.t });
      setStyle(cropAreaEl, { borderBottomWidth: cur._peCrop.b });
      updateHandlers();
    }
    ...
    function updateHandlers() {
      each('top left bottom right top_left top_right bottom_left bottom_right'.split(' '), function(i, side) {
        ...
        setStyle(handle, { marginBottom: (marginBottom + Math.random() * 0.1) });
      });
      setStyle(cropApplyWrap, { marginBottom: Math.random() * 0.1 });
    }
      ...
      each(currSide.split('_'), function(i, side) {
        switch (side) {
          case 'top':
            ...
            setStyle(cropAreaEl, { borderTopWidth: top - cnvPos[1] });
          ...
          case 'left':
            ...
            setStyle(cropAreaEl, { borderLeftWidth: left - cnvPos[0] });
          ...
          case 'right':
            ...
            setStyle(cropAreaEl, { borderRightWidth: left - cnvPos[0] - cnvSize[0] });
          ...
          case 'bottom':
            ...
            setStyle(cropAreaEl, { borderBottomWidth: top - cnvPos[1] - cnvSize[1] });
  ...
  _hideActionBar: function (event, id) {
    ...
    each(FiltersPE.actionBars, function(elid, el) {
      ...
      cssAnim(el, { marginLeft: -5, opacity: 0 }, { duration: 100 }, function() {
        re(el);
  ...
  onActionBlur: function(event, ref) {
    ...
    setStyle(actionBarEl, { left: pos[0] - boxPos[0] + size[0] + 10, top: pos[1] - boxPos[1] });

    addClass(geByClass1('pe_actions'), 'active');
    cssAnim(actionBarEl, { marginLeft: 0, opacity: 0.9 }, { duration: 200 });
  ...
  onActionText: function(event, ref) {
    ...
    var pos = getXY(ref);
    var size = getSize(ref);
    setStyle(actionBarEl, { left: pos[0] - boxPos[0] + size[0] + 11, top: pos[1] - boxPos[1] - 2 });
    ...
    function updateTextareaHeight() {
      ...
      var testEl = ce('div', { className: 'pe_text_input_tester', innerHTML: clean(text).split(/\r\n|\r|\n/).join('<br>') + '&nbsp;' });
      document.body.appendChild(testEl);
      var nlCount = text.split(/\r\n|\r|\n/).length;
      setStyle(textarea, { height: testEl.offsetHeight });
      re(testEl);
    }
  ...
  onEditorKeypress: function(event) {
    ...
    switch (event.which) {
      case 82:
        FiltersPE.onActionRotate(); break;
      case 65:
        FiltersPE.onActionAuto(); break;
    }
  },
  ...
  init: function(opts, vars, filterSaveOptions, filterParams, customOpts, photo, hash, desc) {
    ...
    var whiteFade = ce('div', { className: 'pe_white_fade' });
    ...
    FiltersPE._initPE(vars, customOpts, function() {
      ...
      cssAnim(actionsEl, { opacity: 1.0 }, { duration: 200 });

      var tabs = ge('pe_tabs_panel');
      show(tabs);
      cssAnim(tabs, { opacity: 1.0 }, { duration: 200 });

      var buttons = geByClass1('pe_filter_buttons');
      show(buttons);
      cssAnim(buttons, { opacity: 1.0 }, { duration: 200 });

      cssAnim(whiteFade, { opacity: 0 }, { duration: 200 }, function() {
        re(whiteFade);
        whiteFade = null;
      });
    });
...
hideName: function(thumb) {
  var nameEl = geByClass1('pe_thumb_filter_name', thumb);
  cssAnim(nameEl, {opacity: 0}, {duration: 200}, function() {
    re(nameEl);
  });
},
...
showName: function(thumb, name) {
  ...
  setStyle(nameEl, {width: thumbSize[0] - 12});
  cssAnim(nameEl, {opacity: 0.7}, {duration: 100});
},
...
onHideAttempt: function() {
  ...
  if (settings !== FiltersPE.initialSettings) {
    var fastBox = showFastBox({title:  getLang('photos_pe_onhide_title'), dark: 1, forceNoBtn: true, bodyStyle: 'padding: 20px; line-height: 160%;'}, getLang('photos_pe_onhide_text'), getLang('photos_pe_onhide_yes'), function(button) {
      FiltersPE.showQuitConfirm = false;
      boxQueue.hideLast();
      setTimeout(function() {
...
eof: 1};try{stManager.done('filters_pe.js');}catch(e){}
