...
function initWorkRow(work, elem) {
  ...
  work.uiStart = new Dropdown(g('start' + work.id), [[0, getLang('select_year_not_selected')]].concat(selData.years), {
    width: 200,
    autocomplete: true,
  ...
  work.uiFinish = new Dropdown(g('finish' + work.id), [[0, getLang('select_year_not_selected')]].concat(selData.years), {
    width: 200,
    autocomplete: true,
    placeholder: getLang('select_year_not_selected'),
    placeholderColor: '#000',
...
function saveWorks() {
  var params = {act: 'a_career', hash: ge('hash').value};
  for (var i = 0; i < works.length; ++i) {
    var id = works[i].id;
    params['id' + i] = id;
