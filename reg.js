...
var bad_firstname = false, bad_lastname = false, bad_email = false, bad_password = false, bad_sex = false;
...
function showErrorBox(body, for_focus, close_caption, close_style, reg_button) {
  ...
  if (!error_box) {
    error_box = new MessageBox({title: lang_reg_error_incorrectinfo});
  }
  error_box.removeButtons();
  if (reg_button) {
    error_box.addButton({label: lang_reg_error_continuereg, style: 'button_gray', onClick: function(){
      error_box.hide();
      register(true);
    }});
  }
...
var no_email_reg = false, reg_login = '', reg_pass = '', own_password = false, own_email = false;
...
function checkLogin() {
  ...
  Ajax.postWithCaptcha('register.php', {act: 'check_login', login: theMail}, {onSuccess: callback});
}
...
function show_terms() {
  if (!terms_box) {
    ...
    terms_box.addButton({label: box_close, onClick: function() {
      terms_box.hide();
    }});
