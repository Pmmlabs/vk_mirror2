...
TopMenu = {
  ...
  toggle: function(s) {
    ...
    if (browser.msie8) { // redraw hack
      setStyle(tpMenu, {display: 'table'});
      setTimeout(setStyle.pbind(tpMenu, {display: ''}), 0);
    }
...
function testRedesign(obj, hash, needBox, type) {
  var isButton = obj.tagName == 'BUTTON';
  if (isButton ? buttonLocked(obj) : linkLocked(obj)) return false;
  isButton ? lockButton(obj) : lockLink(obj);

  if (needBox) {
    return showBox('al_index.php', {
      act: 'test_redesign_box',
      hash: hash,
      type: type
    }, {
      onDone: isButton ? unlockButton.pbind(obj) : unlockLink.pbind(obj)
    });
  } else {
    ajax.post('al_index.php', {
      ...
      onDone: function(data) {
        if (data.reload) {
          location = location.protocol + '//vk.com/' + nav.strLoc;
        } else {
          data.leftNav && val(geByTag1('ol', ge('side_bar')), data.leftNav);
          data.html && val('test_redesign_wrap', data.html);
        }
      }.bind(this),
      onFail: isButton ? unlockButton.pbind(obj) : unlockLink.pbind(obj),
      showProgress: isButton ? lockButton.pbind(obj) : lockLink.pbind(obj)
    }));
    return false;
  }
}
