...
createChildClass('Selector', UiControl, {
  ...
  updatePlaceholder: function() {
    ...
    var placeholderInputColorNew = ((zeroPlaceholder || this.disabled) && this.options.placeholderColor || '#3b4045');
...
function Slider(el, options) {
  ...
  var html = '<div class="slider_slide"> ' + backLineHtml + ' <div class="slider_amount"></div> <div class="slider_handler"></div> </div>';
  this._el = ce('div', { innerHTML: html, className: 'slider', id: el.getAttribute('id') || '' });

  var classes = el.classList, _this = this;
  ...
  if (this.options.color) {
    setStyle(this._amountEl, { backgroundColor: this.options.color });
    setStyle(this._handlerEl, { backgroundColor: this.options.color });
  }
  if (this.options.backColor) {
    setStyle(this._slideEl, { backgroundColor: this.options.backColor });
  }

  this.options.width = this.options.width || getSize(this._el)[0];

  addClass(this._el, 'slider_size_' + this.options.size);
  setStyle(this._el, { width: this.options.width });
  ...
  this.setValue(this.options.value || 0, false, true);
...
Slider.prototype.updateSize = function(newWidth) {
...
Slider.prototype._updateHint = function(event, fromDragging) {
  ...
  var slidePos = this._getPos(),
      left = Math.round(event.pageX - slidePos[0]);

  left = fromDragging ? Math.min(Math.max(0, left), this.options.width) : left;

  if (left >= 0 && left <= this.options.width) {
    var value = left / this.options.width;
...
Slider.prototype._onMouseMove = function(event) {
  ...
  left = Math.min(left, slidePos[0] + this.options.width);
  left = left - slidePos[0];

  this.setValue(left / this.options.width, true, true);
...
Slider.prototype.setValue = function(value /*0-1*/, skipFireEvent, fromDragging) {
  ...
  setStyle(this._amountEl, { width: left });
  setStyle(this._handlerEl, { left: left });
...
Slider.prototype.setBackValue = function(value/*0-1*/) {
  ...
  var left = this.options.width * value;
  setStyle(this._backEl, { width: left });
}
...
/*
  content
  showImmediate - show tooltip immideate in any case
  autoShow
  delay
  appendTo
  to: 'up' 'down' 'left'
...
ElementTooltip.prototype._initEvents = function(el) {
  ...
  if (!this._opts.hideOnClick && !this._opts.noHideOnMouseLeave) {
...
ElementTooltip.prototype._onMouseEnter = function(event) {
  ...
  if (!isVisible(this._ttel) && this._opts.autoShow) {
...
ElementTooltip.prototype.destroy = function() {
  ...
  re(this._ttel);
  removeEvent(window, 'mousedown', this._ev_wclick);
}
...
ElementTooltip.prototype.show = function() {
  ...
  if (!this._ttel) {
    ...
    (this._opts.appendTo || this._el).appendChild(this._ttel);
  ...
  setTimeout(function() {
    addClass(_this._ttel, 'eltt_vis');
  }, 10);

  this._opts.elClassWhenTooltip && addClass(this._el, this._opts.elClassWhenTooltip);

  if (this._opts.hideOnClick) {
    addEvent(window, 'mousedown', this._ev_wclick = this._onMouseWindowClick.bind(this));
  }
}
...
ElementTooltip.prototype.updatePosition = function() {
  if (this._opts.setPos) {
    ...
    if (this._opts.appendTo) {
      ...
      var parentPos = getXY(this._opts.appendTo);
      parentOffset[0] = elPos[0] - parentPos[0];
      parentOffset[1] = elPos[1] - parentPos[1];
    }
    ...
    } else if (this._opts.to == 'down') {
      style = {
        left: -ttelSize[0]/2 + elSize[0]/2 + this._opts.offset[0],
        top: -ttelSize[1] - arrowSize + this._opts.offset[1]
      };
