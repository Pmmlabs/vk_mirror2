...
function showMessage(params) {
  var defaults = {title: 'Ошибка', text: 'Произошла ошибка', close: 'Закрыть'};
...
function failFunction(progress, handler) {
  return function () {
    showMessage({text: 'Не удалось соединиться с сервером.'});
    hide(progress);
...
function doneFunction(progress, handler) {
  return function(obj, text) {
    ...
    try {
      ...
      showMessage({text: 'Ошибочный ответ сервера: ' + text});
    }
    handler(response);
  }
}
...
function addMainTopic(page) {
  ...
  ajax.onDone = doneFunction('edit_main_topic_progress', function(response) {
    if (response.error == -1) {
      showAddMainTopicError('Ошибочная ссылка.');
      focusAtEnd('new_main_topic');
    } else if (response.error == 1) {
      showAddMainTopicError('Указанный вопрос не найден. Возможно, он был удален.');
      ge('new_main_topic').value = '';
      focusAtEnd('new_main_topic');
    } else if (response.topics) {
      startEditMainTopics(response.topics);
    } else {
      showAddMainTopicError('Неизвестная ошибка: ' + response.error);
    }
  });
...
function removeMainTopic(id) {
  ...
  ajax.onDone = doneFunction('edit_main_topic_progress', function(response) {
    if (response.success) {
      ...
      showMessage({text: 'Неизвестная ошибка: ' + response.error});
    }
  });
...
function initMoveTopicSections(index, parent_id) {
  ...
  ajax.onDone = doneFunction('section_progress' + index, function(response) {
    if (!response.error) {
      next_section.innerHTML = '<a id="move_topic_link' + index + '" href="javascript: moveTopic(ge(\'section' + index + '\').value)">Перенести</a><input id="section' + index + '" name="section' + index + '" />';
      var sections_json = [[0, ' - Подраздел не выбран - ']];
      for (var i in response) {
        if (parseInt(response[i][1])) {
          sections_json.push([i, response[i][0], 'Раздел закрыт']);
        } else {
...
function showMoveTopicBox(id) {
  if (!moveTopicBox) {
    moveTopicBox = new MessageBox({title: 'Перемещение вопроса'});
    moveTopicBox.addButton({label: "Закрыть", style: "button_no", onClick: function() { moveTopicBox.hide(); }});
  }  
...
function startNotifications() {
  new Checkbox(ge('notify'), {label: 'Разрешить извещения', checked: ge('notify').value, width: 150, onChange: function() {
