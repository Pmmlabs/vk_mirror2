...
function initMilitaryRow(military, elem) {
  ...
  military.uiStart = new Dropdown(g('start' + military.id), [[0, getLang('select_year_not_selected')]].concat(selData.from_years), {
    width: 200,
    autocomplete: true,
  ...
  military.uiFinish = new Dropdown(g('finish' + military.id), [[0, getLang('select_year_not_selected')]].concat(selData.until_years), {
    width: 200,
    autocomplete: true,
    placeholder: getLang('select_year_not_selected'),
    placeholderColor: '#000',
...
function saveMilitaries() {
  var params = {act: 'a_military', hash: ge('hash').value};
  for (var i = 0; i < militaries.length; ++i) {
  ...
  show('save_progress');
  Ajax.postWithCaptcha('editProfile.php', params, {onSuccess: doneHandler, onFail: failHandler, onCaptchaShow: hideHandler});
  return false;
}
