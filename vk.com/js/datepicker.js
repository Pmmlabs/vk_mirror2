...
window.Calendar = function(params){
  ...
  var colorTypes = {"hoverDay":"#EDF0F3", "deHoverDay":"#FFFFFF",  "hoverToday":"#DFE6EB",  "deHoverToday":"#DAE2E8"};
  var dayColors = [];
  ...
  var getMonth = function(m, y, noheight) {
    ...
    case 'm':
      ...
      for(var i=1;i<=12;i++) {
        ...
        dayColors[i] = (i==selDay) ? {'over':'hoverToday','out':'deHoverToday'} : {'over':'hoverDay','out':'deHoverDay'};
      ...
      addEvent(rArrow, 'click', function(){getMonth(1, nextYear);return false;});
      for(var i=1;i<=12;i++) {
        var td = getChild(place, [1, 0, Math.floor((i-1)/2), (i-1)%2]);
        addEvent(td, 'click', (function(ii){ return function(){getDay(1, ii, y, 'm');}})(i));
        addEvent(td, 'mouseover', (function(ii){ return function(){setDayColor(ii, 'over');}})(i));
        addEvent(td, 'mouseout', (function(ii){ return function(){setDayColor(ii, 'out');}})(i));
      }
      break;
    ...
    default:
      ...
      for(var i=1;i<=42;i++) {
        ...
        dayColors[x] = (x>=selDay && x<selDay + lim) ? {'over':'hoverToday','out':'deHoverToday'} : {'over':'hoverDay','out':'deHoverDay'};
      ...
      if(mode != '-1'){
        ...
        addEvent(lArrow, 'click', function(){getMonth(lastMonth, lastYear);return false;});
        addEvent(rArrow, 'click', function(){getMonth(nextMonth, nextYear);return false;});
        for(var i in dayPos) {
          var x1 = dayPos[i];
          if(!parseInt(x1))continue;
          var td = getChild(place, [0, 0, Math.floor((i-1)/7) + 2, (i-1)%7]);
          addEvent(td, 'click', (function(xx1){ return function(){getDay(xx1, m, y, mode);}})(x1));
          addEvent(td, 'mouseover', (function(xx1){ return function(){setDayColor(xx1, 'over');}})(x1));
          addEvent(td, 'mouseout', (function(xx1){ return function(){setDayColor(xx1, 'out');}})(x1));
        }
      }
      break;
    }
...
window.DatePicker = function(params){
  if(!params)return;
  var calendarDiv, calendarBox, calendarFrame, dates = {}, lockHide = false, dates = {};
  var months = [];
  for(var i=1;i<13;i++){
