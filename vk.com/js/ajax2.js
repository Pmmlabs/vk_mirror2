...
var ajaxHistory = $ah = {
                ...
                for (var i in $ah.onLoad) {
                    ...
                    if (p != $ah.curHashes[i] || i == $ah.forceLoad) {
                        ...
                        if (!$ah.cache[i][p] || !$ah.useCache) {
                            var a = new Ajax((function(ll, ii, pp) {
    ...
    splitHash: function(hash) {
        ...
        if (hash.length == 1) {
            ...
            return {
                'default': this.sortParams(hash[0])
            };
        }
        var parsed = {};
        for (var i = 0; i < hash.length; i += 2) {
            var h = hash[i];
            var p = hash[i + 1];
            if (this.onLoad[h].show) {
                p = this.sortParams(this.onLoad[h].show.from(p));
            } else {
                p = this.sortParams(p);
    ...
    validateHash: function(hash) {
        return this.joinHash(this.splitHash(hash));
    },
