var ProfileEditor = {
    ...
    go: function(el, ev) {
        ...
        return uiRightMenu.go(el, ev);
    },

    showMsg: function(msg) {
        showMsg('pedit_result', msg, 'ok_msg', true);
        show('pedit_result');
        scrollToTop(200);
    },
    ...
    initBeforeUnload: function() {
        ...
        if (nav.objLoc.hl) {
            ProfileEditor.scrollHighlightRow(geByClass1('_' + nav.objLoc.hl));
        }
    },
    ...
    highlightRow: function(el) {
        ...
        addClass(el, 'pedit_row_selected');
        addClassDelayed(el, 'pedit_row_animated', 1500);
        removeClassDelayed(el, 'pedit_row_selected', 2500);
        removeClassDelayed(el, 'pedit_row_animated', 2500);
    },
    scrollHighlightRow: function(el) {
        el = ge(el);
        ...
        setTimeout(function() {
            var top = getClientRectOffsetY(el);
            ...
            if (top < head + st || top > st + client) {
                scrollToY(top);
                setTimeout(ProfileEditor.highlightRow.pbind(el), 300);
            } else {
                ProfileEditor.highlightRow(el);
            }
        }, 0);
    },
    ...
    initGeneral: function() {
        ...
        if (peditData.status < 0 || peditData.status > 8) {
            peditData.status = 0;
        } else if (peditData.status > 1 && peditData.status != 6) {
        ...
        var partnerInput = cur.uiPartner.input,
            partnerTtEl = ge('pedit_partner_tt_place'),
            partnerTtEl1 = ge('pedit_partner_tt_place1');
        addEvent(partnerInput, 'focus blur', function(e) {
            if (e.type == 'focus') {
                ...
                } else if (indexOf([2, 3, 4, 5, 7, 8], cur.uiStatus.val()) != -1) {
                    showTooltip(partnerTtEl1, {
                        text: peditData.status_helps[3 - cur.uiSex.val()],
                        slideX: 15,
                        className: 'pedit_tt',
                        shift: [-12, -15, 0],
                        dir: 'left',
        ...
        peditData.langs = peditData.langs.map(function(res) {
            res[1] = winToUtf(res[1]);
            return res;
        });
    ...
    initContacts: function() {
        ...
        each(['home', 'mobile', 'email'], function() {
            var el = ge('privacy_edit_' + this);
            el.setAttribute('data-title', el.innerHTML);
            el.onmouseover = showTitle.pbind(el, false, [15, 6]);
        });
        cur.onPrivacyShow = function(key) {
            var el = ge('privacy_edit_' + key),
                row = gpeByClass('pedit_privacy_control', el);
            if (row && row.tt && isFunction(row.tt.hide)) {
                row.tt.hide();
            }
        }
        cur.onPrivacyChanged = function(key) {
            ...
            if (el) {
                ...
                el = gpeByClass('pedit_privacy_control', el);
                if (v > 0) {
                    removeClass(el, 'pedit_privacy');
                    addClass(el, 'pedit_privacy_locked');
                } else {
                    removeClass(el, 'pedit_privacy_locked');
                    addClass(el, 'pedit_privacy');
                }
                geByTag1('a', el).setAttribute('data-title', label);
            }
        }
    ...
    generateDays: function(year, month) {
        var result = [
            ...
            last = ProfileEditor.getLastDay(year, month);
    ...
    doUpdateFriendsList: function() {
        var status = cur.uiStatus.val(),
            sex = cur.uiSex.val();
        if (status < 2 || status == 6 || !sex) return;
        var data = cur.friendsFull[sex].Nom,
            gs = (status == 2 || status == 7 || status == 5) ? 2 : 0,
            p = cur.uiPartner.val(),
            found = false;
        ...
        cur.uiPartner.setOptions({
            defaultItems: data[gs]
        });
        cur.uiPartner.setOptions({
            placeholder: data[gs][0][1]
        });
        cur.uiPartner.val(p, true);
    },

    deleteLinkHtml: function(onclick) {
        return '<a class="pedit_del_icon _del_icon" onclick="this.tt.hide(); ' + onclick + '; return false;" onmouseover="showTitle(this, \'' + getLang('global_delete') + '\');"> </a>';
    },
    ...
    onFriendsLoaded: function(type, selected) {
        var peditData = cur.options;
        var wrap = ce('div', {
            id: 'pedit_wrap_' + type + cur.relGuid,
            className: 'pedit_wrap_relation clear_fix'
        });
        var withDate = (type == 'child' || type == 'grandchild') && (!selected || selected[0] < 0);
        var name = type + cur.relGuid,
            id = 'pedit_' + name;
        var html = '<div class="pedit_relation_input"><div class="fl_l"><input class="pedit_dropdown" type="hidden" id="' + id + '" name="' + name + '"/></div><div class="pedit_right_control">' + ProfileEditor.deleteLinkHtml('ProfileEditor.removeRelation(\'' + type + '\', ' + cur.relGuid + ')') + '</div>';
        var t = 0;
        if (withDate) {
            t = selected ? parseInt(selected[3]) : 0;
            html += '<div class="pedit_next_input clear_fix clear"><input class="pedit_date_field" type="hidden" id="' + id + '_date" value="' + t + '"><div class="pedit_bday fl_l"><input type="hidden" id="' + id + '_day"/></div><div class="pedit_bmonth fl_l"><input type="hidden" id="' + id + '_month"/></div><div class="pedit_byear fl_l"><input type="hidden" id="' + id + '_year"/></div></div>';
        }
        wrap.innerHTML = html;
        ...
        if (withDate) {
            ...
            var dayDD = new Dropdown(ge(id + '_day'), ProfileEditor.generateDays(d.getFullYear(), d.getMonth() + 1), {
                selectedItems: t ? d.getDate() : 0,
                onChange: onDateChange,
                dark: 1
            });
            var monthDD = new Dropdown(ge(id + '_month'), peditData.bmonths, {
                selectedItems: t ? (d.getMonth() + 1) : 0,
                onChange: onDateChange,
                dark: 1
            });
            var yearDD = new Dropdown(ge(id + '_year'), years, {
                selectedItems: t ? d.getFullYear() : 0,
                onChange: onDateChange,
                dark: 1
            });
        }
    ...
    saveGeneral: function(btn) {
        ...
        if (peditData.family !== false) {
            ...
            each(geByClass('pedit_relation_input', ge('pedit_children')), function() {
                ...
                params['children_date[' + i + ']'] = (geByClass('pedit_date_field', this)[0] || {
                    value: 0
                }).value;
            ...
            each(geByClass('pedit_relation_input', ge('pedit_grandchildren')), function() {
                ...
                params['grandchildren_date[' + i + ']'] = (geByClass('pedit_date_field', this)[0] || {
                    value: 0
                }).value;
        ...
        ajax.post(ProfileEditor.controller, params, {
            onDone: function(response, introScript) {
                ...
                if (peditData.family !== false) {
                    ...
                    ProfileEditor.updateFamily(peditData.family);
                    if (peditData.family === false) {
                        each(geByClass('pedit_general_family_row'), function() {
                            hide(this);
                        });
                    }
                }
    ...
    clearSocialExport: function(service) {
        ...
        switch (service) {
            case 1:
                title = 'settings_status_export';
                msg = 'settings_status_confirm';
                break;
            case 2:
                title = 'settings_notes_export';
                msg = 'settings_livejournal_confirm';
                break;
            case 3:
                title = 'settings_status_export';
                msg = 'settings_facebook_confirm';
                break;
            case 4:
                title = 'settings_status_import_photo';
                msg = 'settings_instagram_confirm';
                break;
            case 7:
                title = 'profileEdit_import_rss';
                msg = 'profileEdit_rss_confirm';
                break;
        }
        var msg = getLang(msg);
        var clrBox = showFastBox(getLang(title), msg, getLang('global_continue'), function() {
            ajax.post('/al_profileEdit.php', {
                ...
                onDone: function(result) {
                     ...
                     */
                    if (service === 3) {
                        re(gpeByClass('pedit_social_row', ge('export_service_' + service)));
                    } else {
    ...
    initEducation: function() {
        ...
        cur.nav.push(function(changed, old, n) {
            ...
            if ((old['act'] == 'education' || old['act'] == 'higher_education') && (n['act'] == 'education' || n['act'] == 'higher_education')) {
                uiTabs.hideProgress(geByClass1('ui_tabs', 'profile_editor'));
    ...
    selectEduTab: function(tab, isInit) {
        ...
        if (tab == 'education') {
            ...
            if (!isVisible('schools')) {
                ...
                if (cur.schoolsCount) {
                    for (var i = 0; i < cur.schools.length; ++i) {
                        ge('schools').appendChild(ProfileEditorEdu.genSchoolRow(cur.schools[i].id, i == 0));
            ...
            if (!isVisible('primary_uni')) {
                ge('primary_uni').appendChild(ProfileEditorEdu.genUniRow(cur.primary_uni.id, true, true));
                cur.primary_uni = ProfileEditorEdu.initUniRow(cur.primary_uni);
                show('primary_uni');
                cur.unisCount = cur.unis.length;
                if (cur.unisCount) {
                    for (var i = 0; i < cur.unis.length; ++i) {
                        ge('unis').appendChild(ProfileEditorEdu.genUniRow(cur.unis[i].id, false, i == 0));
    ...
    initPersonal: function() {
        ...
        placeholderInit('pedit_inspired_by');
    ...
    editHome: function(placeId) {
        showBox('edit', {
            act: 'edit_home',
            place_id: cur.editAddrId || placeId
        });
    },
    ...
    addItemBox: function(type, objId) {
        ...
        cur.addItemBox = showBox(ProfileEditor.controller, params, {
            params: {
                width: 560,
                hideButtons: true,
                cache: 1
