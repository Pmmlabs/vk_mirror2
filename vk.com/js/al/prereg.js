function initReg(prefix, opts) {
    ...
    extend(cur, {
        ...
        regValidate: function(pref, ignoreBadPwd) {
            var fname = trim(ge(pref + 'fn').value),
                badfn = false;
            if (fname.length < 2) {
                cur.regShowError(pref, 'fn_details', opts.reg_error_fname);
                badfn = true;
            }
            var lname = trim(ge(pref + 'ln').value),
                badln = false;
            if (lname.length < 2) {
            ...
            var sex = intval(ge(pref + 'sex').value),
                badsx = false;
            if (!sex) {
        ...
        regErrorBox: function(pref, body, forFocus, closeCaption, regButton) {
            ...
            var box = showFastBox({
                title: opts.reg_error_incorrectinfo
            }, body, closeCaption, function() {
                box.hide();
        ...
        regShowError: function(pref, container, text) {
            var cont = ge(pref + container),
                was = isVisible(cont) && trim(cont.innerHTML);
        ...
        register: function(pref, ignoreBadPwd) {
            ...
            var options = {
                onDone: function(res, remail, rpass) {
                    res = intval(res);
                    if (res) {
                        cur.regBadEM = true;
