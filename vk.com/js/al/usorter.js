var usorter = {
    ...
    animate: function(e, sx, sy, x, y, handler, s) {
        ...
        usorter.animcache[e.parentNode.id] = extend(anim, {
            el: e
        });
    ...
    mousedown: function(ev) {
        ...
        var el = this,
            inner = usorter.first(el),
            s = el.parentNode.usorter,
            evc = usorter.evCoords(ev);
    ...
    mousemove: function(ev) {
        ...
        var cancel = true,
            evc = (ev.type == 'scroll') ? [usorter.lastX, usorter.lastY + scrollNode.scrollTop - usorter.lastS] : usorter.evCoords(ev);
        if (ev.type == 'scroll') {
            cancel = false;
        } else {
            usorter.lastX = evc[0];
        ...
        if (s.newbef !== el && s.newbef !== s.before) {
            animate(domFC(s.newbef ? domPS(s.newbef) : els[l - 1]), {
                left: 0
            }, 200);
            animate(domFC(s.newbef), {
                left: 0
            }, 200);
        }
        s.newbef = best;
    ...
    mouseup: function(ev) {
        ...
        if (s.newbef !== el && s.newbef !== s.before) {
            for (var i = 0, l = els.length; i < l; ++i) {
                var elem = els[i],
                    first = usorter.first(elem);
                if (elem != el && elem.nodeType !== 3) {
        ...
        var result = {
            ...
            stop: function() {
                ...
                for (var els = result.parent.childNodes, i = 0, l = els.length; i < l; ++i) {
                    var ch = els[i];
                    if (ch.nodeType === 3 || ch.getAttribute('nodrag')) continue;
