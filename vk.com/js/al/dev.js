var Dev = {
    ...
    initPage: function(opts) {
        if (opts.edit) {
            ...
            placeholderSetup(desc, {
                back: true
            });
            var simular = ge('dev_edit_simular');
            placeholderSetup(simular, {
                back: true
            });
    ...
    diselectLeftNav: function() {
        ...
        var id = selNode.id.replace('dev_mlist_', ''),
            submenu = ge('dev_mlist_submenu_' + id);
        if (submenu) {
            hide(submenu);
        } else {
    ...
    animLeftNav: function(oldSel, newSel, page) {
        ...
        var mark = ge('dev_left_nav_mark'),
            fromY = oldSel && oldSel.offsetTop || 0,
            toY, duration,
        ...
        if (oldSel) {
            ...
            setStyle(mark, {
                height: getSize(oldSel)[1],
                top: fromY
            });
            addClass(leftNav, 'anim');
            animate(mark, {
                height: getSize(newSel)[1],
                top: toY
            }, {
                duration: duration,
    ...
    scrollCheck: function() {
        ...
        var wh = window.lastWindowHeight || 0,
            ...
            pos = 0,
            head = ge('dev_top_nav_wrap'),
            headH = getSize(head)[1],
            position, ml,
            bottom = ge('dev_footer_wrap'),
            bottomH = isVisible(bottom) ? getSize(bottom)[1] : 0,
            navPos = getXY(nav)[1],
            navH = getSize(nav)[1],
    ...
    switchPage: function(page, edit, opts) {
        ...
        var mlist = ge('dev_mlist_list'),
            ...
            isMethod = !!pageRaw[1] || page == 'execute',
            newSel;
    ...
    onMouseOverDate: function(dateel) {
        ...
        var timestamp = dateel.getAttribute('data-date'),
            pretty;
    ...
    onSearchChange: function(el, ev) {
        ...
        setTimeout(function() {
            var v = val(el).toLowerCase().replace(/[^a-z�-�]+/g, '');
            if (v) {
                if (v == cur.prevSearch) {
    ...
    mainPageSliderChange: function(pos, nav, fast) {
        ...
        var item_opts = isObject(newItem) ? newItem : {
            className: newItem
        };
    ...
    clickSpoiler: function(el, block_id, hash) {
        var parent = el.parentNode,
            cont = geByClass1('dev_method_block_content', parent),
            act = 'hide'
    ...
    initWidgetAuthConstructor: function(options) {
        ...
        function updateLink() {
            ...
            linkTimeout = setTimeout(function() {
                ...
                if (url && state.url != url && !/^(javascript:|data:)/i.test(url.replace(/[\0-\x1F\x7F]/g, ''))) {
                    state.url = url;
                    updateCode();
                }
            }, 300);
