...
<div class="box_controls_wrap">\
  <div class="box_controls">\
    <div id="pv_tag_buttons" class="clear_all">\
        <button class="flat_button fl_r" id="pv_add_tag">' + getLang('global_add') + '</button>\
        <button class="flat_button secondary fl_r" id="pv_cancel_tag">' + getLang('global_cancel') + '</button>\
    </div>\
    ...
    addTag: function(mid, ev) {
        ...
        var x = (rect[0] / xy[0] * 100),
            y = (rect[1] / xy[1] * 100);
        var x2 = ((rect[0] + rect[2]) / xy[0] * 100),
            y2 = ((rect[1] + rect[3]) / xy[1] * 100);
    ...
    updateFriends: function() {
        ...
        var q = trim(cur.pvFriendName.value).toLowerCase().replace(/[�]/, '�'),
            frs = [];
        ...
        var friends = q ? cur.pvFriendsCache[q] : cur.pvFriendsList,
            tagged = cur.pvData[cur.pvListId][cur.pvIndex].tagged;

        for (var i in friends) {
            var fr = cur.pvFriendsList[i],
                mid = positive(i),
                name = fr[1];
            ...
            for (var i in queries) {
                ...
                if (searchIn) {
                    ...
                    for (var i in searchIn) {
                        var name = cur.pvFriendsList[i][1].replace(/[��]/, '�');
    ...
    showFriends: function() {
        ...
        var xy = getXY(cur.pvPhoto.firstChild),
            x = xy[0] + r[0] + r[2] + 20;
        ...
        var h = getSize(cur.pvFriends)[1],
            y = xy[1] + layerWrap.scrollTop + r[1] - ((layerWrap.offsetParent || {}).scrollTop || bodyNode.scrollTop || htmlNode.scrollTop);
