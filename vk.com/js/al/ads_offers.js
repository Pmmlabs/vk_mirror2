var Offers = {
  ...
  onStatusHover: function(elem, offerId, type) {
    var options = {
      status: {type: type, title: cur.options.statusText[offerId]},
  ...
  openDeleteOfferBox: function(offerId, hash, lock, unlock, updateStatus) {
    cur.deleteConfirmBox = showFastBox({title: getLang('ads_archive_box_offer_title'), dark: true, width: 430, bodyStyle: 'line-height: 160%; padding: 16px 20px;'}, getLang('ads_archive_box_offer_warning'), getLang('ads_archive_box_action'), function() {
      Offers.deleteOffer(offerId, hash, lock, unlock, updateStatus);
    }, getLang('box_cancel'));
  },
  deleteOffer: function(offerId, hash, lock, unlock, updateStatus) {
    if (!lock) {
      lock = function () { addClass('offers_info_archive', 'loading'); return true; }
      unlock = function () { removeClass('offers_info_archive', 'loading'); return true; }
    ...
    ajax.post('/offers?act=a_offer_delete', {offer_id: offerId, hash: hash, from: !updateStatus ? 'button' : 'table'}, {
  ...
  changeStatus: function(offerId, status, hash, addParams, force, lock, unlock, updateStatus, linkType) {
    if (status == 1 && !force) { // 1 = OFFERS_STATUS_REVIEWING
      var box = showBox('/offers', {act: 'a_review_box', offer_id: offerId, hash: hash}, {params: {width: 450, bodyStyle: 'padding: 20px;'}, dark: 1, onFail: Offers.onBoxFail});
      ...
      var box = showBox('/offers', {act: 'a_ban_users_box', offer_id: offerId, hash: hash}, {params: {width: 450, bodyStyle: 'padding: 20px;'}, dark: 1, onFail: Offers.onBoxFail});
      box.postData = function(addParams) {
        Offers.changeStatus(offerId, status, hash, addParams, true, lock, unlock, updateStatus, linkType);
      };
      if (updateStatus) {
      ...
      var box = curBox();
      var params = {offer_id: offerId, status: status, hash: hash, from: !updateStatus ? 'button' : 'table'};
      if (addParams) {
  ...
  showMsg: function(text, parentEl) {
    ...
    if (!msg) {
      ...
      msg = parent.insertBefore(ce('div', {id: 'offers_msg', className: 'msg'}), parent.firstChild);
    }
    msg.innerHTML = text;
  ...
  showError: function(text, parentEl) {
    ...
    if (!err) {
      var parent = parentEl || ge('ads_page');
      err = parent.insertBefore(ce('div', {id: 'offers_error', className: 'error'}), parent.firstChild);
    }
    err.innerHTML = text;
  ...
  updateOffersListTable: function(params) {
    ...
    delete obj[0];
    delete obj['offset'];
    ajax.post(url, extend(obj, params, {load: 1}), {
  ...
  addClient: function(union_id) {
    return !showBox('/offers', {act: 'a_create_client_box', union_id: union_id}, { params: {width: '315px', dark: true, bodyStyle: 'padding: 20px;'}});
  },
  createClient: function(union_id, hash) {
    ...
    ajax.post('/offers?act=a_create_client', {union_id: union_id, hash: hash, name: name}, {
      onFail: function(msg) {
        if (curBox()) {
  ...
  switchSectionsTab: function(el, tab, event, force) {
    ...
    var obj = clone(nav.objLoc), url = obj[0];
    delete obj[0];
    ajax.post('/offers', extend(obj, {section: tab, load: 1}), {
      ...
      showProgress: function() { hide('offers_offer_export_link'); show('offers_offer_tabs_progress'); },
      hideProgress: function() { hide('offers_offer_tabs_progress'); show('offers_offer_export_link'); },
    });
    return false;
  },
  showPreview: function(testMode) {
    var box = showBox('/offers', {act: 'a_preview_box', offer_id: cur.offerId, test_mode: testMode}, {params: {width: 500, bodyStyle: 'padding: 25px; line-height: 180%;', hideButtons: 1}, dark: 1});
  },
  ...
  showSecret: function() {
    showBox('/offers', {act: 'a_secret_box', offer_id: cur.offerId}, {params: {width: 400, bodyStyle: 'padding: 20px;'}, dark: 1});
  },
  ...
  showExportStats: function() {
    showBox('/offers', {act: 'a_export_stats_box', offer_id: cur.offerId}, {dark: 1});
    return false;
  },
  ...
  onLinkTypeClick: function(elem) {
    ...
    if (uiData) {
      ...
      cur.uiLinkId.setOptions({introText: placeholderText, placeholder: placeholderText, defaultItems: defaultData});
  ...
  generateMetricRow: function (id, metricType, metricTitle) {
    ...
    var elem = ce('div', {id: 'offer_metric_' + id});
    ...
    elem.innerHTML =
      ...
      '<div class="offers_edit_value offers_edit_value_text fl_l">' +
        '<div class="offer_metric_title fl_l"><a id="offer_metric_title_link_'+id+'" onclick="Offers.editMetricTitle('+id+')">'+metricTitleText+'</a><input onblur="Offers.saveMetricTitle('+id+')" id="offer_metric_title_input_'+id+'" type="text" placeholder="Название метрики" value="'+metricTitleValue+'" maxlength="100"></div>' +
  ...
  completeLinkType: function() {
    ...
    function initTT(name, tt) {
      ...
      var targetElem = ge('offers_' + name + '_wrap'),
          ...
          helpText = tt.text;
      var handler, context = {focus: false, over: 0, out: 2};
      var showTooltip = function() { AdsEdit.showHelpCriterionTooltip(name, targetElem, handler, cur.tt_data[name], helpText, -360, topOffset, cur); };
      var hideTooltip = function() { AdsEdit.hideHelpTooltip(cur.tt_data[name].tt); };
      handler = function(event){ Offers.onHelpTooltipEvent(event, name, context, showTooltip, hideTooltip); };
  ...
  showUploadPhotoBox: function() {
    showBox('/offers', {act: 'a_upload_photo_box', offer_id: cur.offerId, union_id: cur.unionId}, {params: {width: 460}, dark: 1});
  },
  openHelpBox: function(type, unionId) {
    showBox('/offers?act=a_help_text_box', {type: type, union_id: unionId}, {params: {width: 450, dark: 1, bodyStyle: 'padding: 20px;'}, cache: 1});
    return false;
  },
  ...
  saveOffer: function(confirm_cost_lowering) {
    ...
    each(['offer', 'view', 'targeting', 'settings'], function(i, v) { hide('offers_edit_error_' + v); });
  ...
  updateEditData: function(force, delayed) {
    ...
    var params = {offer_id: cur.offerId};
    params = extend(params, Offers.getOfferTargetData());
    ...
    var ret = {
      ...
      setItems: function(newItems) {
        ...
        for (var i in items) {
          if (compareTypes(items[i]['i'], options.status.type)) {
            var el = se(items[i]['l']);
            dd.setOptions({title: '<span class="' + el.className + '"></span>' + options.status.title});
            delete items[i];
          }
        }
        dd.setData(items);
  ...
  onExportStatsOfficesChanged: function (event) {
    switch (event.target.index) {
      default:
        show(ge('offers_export_grouping_offers'));
        break;
      case 1:
...
try{stManager.done('ads_offers.js');}catch(e){}
