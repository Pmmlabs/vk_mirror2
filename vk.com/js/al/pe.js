function PhotoEdit(parentEl, photoSrc, bigPhotoSrc, options, initCallback, sizeUpdateCallback) {
  ...
  var imageEl = new Image(), bigImageEl = new Image();
  ...
  this.applyChanges = function () {
    switch (this._mode) {
      case 'params':
        this.applyParameters(); break;
      case 'filter':
        this.applyFilter(); break;
    }
  }
  ...
  function loadImages(imagesArray, cb) {
    ...
    var count = imagesArray.length, images = new Array(count);
  ...
  */
  this.crop = function(left, right, top, bottom, isReset, force) {
    ...
    var currCrop = this.getLastCrop(), needCrop = !!force;
  ...
  */
  ...
  this.setFadeImage = function() {
    ...
    setStyle(fadeCanvas, { position: 'absolute', left: 0, top: 0 });
  }
  ...
  this.removeFadeImage = function() {
    ...
    cssAnim(fadeCanvasEl, { opacity: 0.0 }, { duration: 200 });
  ...
  this.setText = function(text, fontType) {
    ...
    // calc font sizes for
    var impactUpperSize = 9999, impactBottomSize = 9999;
    if (fontType == 0) {
      var fontSizes = [];
  ...
  var Shader = (function() {
    ...
    Shader.prototype.uniforms = function(uniforms) {
      ...
      for (var name in uniforms) {
        ...
        if (isArray(value)) {
          switch (value.length) {
            case 1: context.gl.uniform1fv(location, new Float32Array(value)); break;
            case 2: context.gl.uniform2fv(location, new Float32Array(value)); break;
            case 3: context.gl.uniform3fv(location, new Float32Array(value)); break;
            case 4: context.gl.uniform4fv(location, new Float32Array(value)); break;
            case 9: context.gl.uniformMatrix3fv(location, false, new Float32Array(value)); break;
            case 16: context.gl.uniformMatrix4fv(location, false, new Float32Array(value)); break;
            default: throw 'erro while loading uniform "' + name + '" of length ' + value.length;
          }
...
try{stManager.done('pe.js');}catch(e){}
