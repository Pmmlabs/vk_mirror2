    ...
    pushEvent: function(e, t) {
        ...
        switch (s) {
            ...
            case "edit_post":
                ...
                var H = geByClass1("wall_post_more", L);
                H && (H = isVisible(domNS(H))), (E = feed.needScrollPost(t, L)) && (c -= L.offsetHeight);
                var N = psr(rs(e[3], {
                        poll_hash: cur.wallTpl.poll_hash
                    })),
                    m = ge("post" + r);
                m && !isVisible(m.parentNode) && (N = wall.updatePostImages(N)), window.fifaReplaceText && (N = fifaReplaceText(N)), val(L, N), H && (H = geByClass1("wall_post_more", L), H && H.onclick()), wall.votingUpdateByPostRaw(r), E && (c += L.offsetHeight), nodeUpdated(L), window.Wall && Wall.updatePostAuthorData(r);
                break;
            case "edit_reply":
                var M = e[3],
                    L = ge("wpt" + M);
                if (!isVisible("post" + M) || !L) break;
            ...
            case "del_post":
                if (i) {
                    var j = domClosest("feed_row", i) || domClosest("feed_row_unshown", i) || i;
                    cur.wallMyDeleted[r] || (feed.needScrollPost(t, i) && (c -= i.offsetHeight + d(i)), revertLastInlineVideo(i), re(j)), cur.options.offset--, hasClass(cur.feedEls.wrap, "feed_has_new") && !isVisible(j) && (cur.newPostsCount--, cur.newPostsCount ? cur.feedEls.newPosts.innerHTML = getLang("news_new_posts", cur.newPostsCount) : removeClass(cur.feedEls.wrap, "feed_has_new"))
                }
                break;
            case "res_post":
            ...
            case "new_reply":
                ...
                var F = ge("replies" + r),
                    ...
                    f = !1,
                    q = !1;
                if (isVisible(F) && isVisible(D) && !isVisible("reply_link" + r)) {
                    var I = F.nextSibling,
                ...
                } else re("reply_link" + r), show(D, F), q = !0;
                r.split("_")[0] == vk.id && cur.feedUnreadCount++, f || (f = se(V)), F.appendChild(f), feed.needScrollPost(t, q ? f : I) && (c += i.offsetHeight - A), q && nodeUpdated(f), Wall.repliesSideSetup(r), Wall.updateMentionsIndex(), Likes.update("wall" + r, {
                    comment_num: e[13]
                });
                break;
            case "del_reply":
            ...
            case "like_reply":
                ...
                wall.likeFullUpdate(z, G, {
                    ...
                    like_num: e[3],
                    like_title: !1,
                    share_my: K && hasClass(K, "my_share"),
                    share_num: e[4],
                    share_title: !1
                });
                break;
