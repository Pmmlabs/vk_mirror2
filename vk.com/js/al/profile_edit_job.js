var ProfileEditorJob = {
  ...
  genOneRow: function(field, id, label, params, attr) {
    var key = field + id;
    if (!params) {
      params = '';
    }
    return '<div class="pedit_edu_row" id="row_' + key + '"'+(attr || '')+'>' +
  ...
  genWorkRow: function(work_id) {
    ...
    var html = '<div id="content' + work_id + '"><div class="pedit_job_progress"><img class="pedit_progress" src="/images/upload.gif" id="progress' + work_id + '" />' +
      '<a class="fl_r" onclick="ProfileEditorJob.deleteWork(' + work_id + ')">' + getLang('global_delete') + '</a></div>' +
      this.genOneRow('group', work_id, getLang('select_company'), 'type="text" class="dark pedit_edu_txt"') +
  ...
  initGroupCompany: function(work) {
    ...
    if (work.company_name) {
      ...
      if (work.company_gid) {
        ...
        opts.selectedItems = [[-1, companyName]];
      }
    }
    //var groupsUrl = '/adsedit?act=search_user_objects&section=groups&events_future=1';
    ...
    var doneHandler = function(response) {
      ...
      for (var i = 0; i < cur.works.length; ++i) {
        ...
        if (intval(new_work_id)) {
          worksList[i] = this.updateWork(cur.works[i], new_work_id, worksCont);
        }
      }
      cur.works = worksList;
      cur.worksCount = cur.works.length;
