var Pagination = {
    ...
    initScroll: function(e, r) {
        var i = !0 === e ? getXY(cur.pgCont.lastChild || cur.pgCont)[1] : intval(e);
        !0 === r && (vk.loaded ? setTimeout(Pagination.setScroll.pbind(i), 0) : addEvent(window, "load", function() {
    ...
    genFixed: function() {
        if (!(cur.pgStart + cur.pgCount <= cur.pgPerPage)) {
            var e, r, i, a, t = cur.pgHref ? ' href="' + cur.pgHref + '%d"' : "",
                ...
                p = [];
            c < n && (r = cur.pgByOne ? (e = (n - 1) * g, "&lsaquo;") : (e = 0, "&laquo;"), p.push('<a class="pg_flnk_nb"' + t.replace("%d", e) + o.replace("%d", e) + ">" + r + "</a>"));
            for (var d = Math.max(0, n - c); d < n; ++d) e = d * g, i = d >= cur.pgStartPage ? "pg_flnk_rd" : "pg_flnk", p.push('<a class="' + i + '"' + t.replace("%d", e) + o.replace("%d", e) + ">" + (d + 1) + "</a>");
            p.push('<a class="pg_flnk_sel"' + t.replace("%d", n * g) + o.replace("%d", n * g) + ">" + (n + 1) + "</a>");
            for (d = n + 1; d < n + c + 1 && d < u; ++d) i = (e = d * g) >= cur.pgStart + cur.pgNodesCount ? "pg_flnk" : "pg_flnk_rd", p.push('<a class="' + i + '"' + t.replace("%d", e) + o.replace("%d", e) + ">" + (d + 1) + "</a>");
            n < u - c - 1 && (r = cur.pgByOne ? (e = (n + 1) * g, "&rsaquo;") : (e = (u - 1) * g, "&raquo;"), p.push('<a class="pg_flnk_nb"' + t.replace("%d", e) + o.replace("%d", e) + ">" + r + "</a>")), cur.pgFixedPages.innerHTML = p.join(""), isVisible(cur.pgFixed) ? a = getSize(cur.pgFixedPages) : (show(cur.pgFixed), a = getSize(cur.pgFixedPages), hide(cur.pgFixed)), setStyle(cur.pgFixedBack, {
                width: a[0],
                height: a[1]
    ...
    keyNav: function(e) {
        if (!(layers.visible || cur.pgNoArrowNav && cur.pgNoArrowNav() || !e.ctrlKey && !e.metaKey)) {
            var r = cur.pgPage,
                i = cur.pgPerPage;
            if (e.keyCode == KEY.RIGHT ? ++r : e.keyCode == KEY.LEFT && --r, !(r == cur.pgPage || r < 0 || r >= Math.ceil(cur.pgCount / i))) {
                var a = nav.objLoc;
    ...
    updateFixed: function(e, r) {
        void 0 === r && (r = scrollGetY()), Pagination.updateFixedLeft(e), !cur.pgOver && r < cur.pgFixedStart ? isVisible(cur.pgFixed) && !cur.pgHiding && (cur.pgHiding = !0, cur.pgShowing = !1, fadeOut(cur.pgFixed, 300, function() {
            cur.pgHiding = !1
        })) : cur.pgShowing || !cur.pgHiding && isVisible(cur.pgFixed) || (cur.pgHiding && data(cur.pgFixed, "tween").stop(!0), cur.pgShowing = !0, fadeIn(cur.pgFixed, 300, function() {
            cur.pgShowing = !1
        }))
    },
    scrollResize: function(e) {
        if (!(browser.mobile && !browser.safari_mobile || cur.pgPaused)) {
            ...
            if ("scroll" == e.type) {
                ...
                if (50 < i && (t <= o || i < a[o * cur.pgPerPage].offsetTop + cur.pgFixedDelta)) {
                    for (o = Math.min(o, t - 1); 0 <= o && !(i >= a[o * cur.pgPerPage].offsetTop + cur.pgFixedDelta); --o);
                    ++o
                } else
                    for (i <= 50 && (o = 0); o < t && !(i < a[o * cur.pgPerPage].offsetTop + cur.pgFixedDelta); ++o);
                o == t - 1 && a[cur.pgNodesCount - 1] && i + lastWindowHeight > a[cur.pgNodesCount - 1].offsetTop + cur.pgFixedDelta && ++o, 0 < o && --o, cur.pgPage != cur.pgStartPage + o ? (cur.pgPage = cur.pgStartPage + o, Pagination.genFixed()) : Pagination.updateFixed(e)
            ...
            if (cur.pgOnScroll && cur.pgOnScroll(e, i), !(!cur.pgMore || cur.pgIgnore && cur.pgIgnore())) {
                ...
                if (isVisible(cur.pgMore)) i + (cur.pgCustomOffset || 0) + n > cur.pgMore.offsetTop && Pagination.showMore()
            }
        }
    },
    ...
    showMore: function() {
        ...
        !isVisible(cur.pgMore) || e && buttonLocked(cur.pgMore) || isVisible(cur.pgMorePrg) || (cur.pgLoading ? cur.pgLoading = 2 : ajax.post(cur.pgUrl, extend({
            offset: cur.pgOffset,
            part: 1
        }, cur.pgParams || {}), {
