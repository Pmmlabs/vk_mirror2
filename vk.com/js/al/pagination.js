var Pagination = {
    init: function(toBottom) {
        if (!cur.pgCont) return;
        cur.initScrollFn = Pagination.initScroll.pbind(toBottom, true);
...
<div class="pg_fixed_pages">\
    '
        ...
        if (cur.pgUrl) {
            ...
            if (cur.pgPreload) {
                ajax.preload(cur.pgUrl, extend({
                    offset: cur.pgOffset,
                    part: 1
                }, cur.pgParams || {}), cur.pgPreload);
    ...
    initScroll: function(toBottom, start) {
        var st = (toBottom === true) ? (getXY(cur.pgCont.lastChild || cur.pgCont)[1]) : intval(toBottom);
        if (start === true) {
            if (!vk.loaded) {
    ...
    pageTopUpdated: function() {
        cur.pgFixedStart = getXY(cur.pgCont)[1];
        cur.pgFixedDelta = -cur.pgFixed.offsetTop;
        if (cur.pgNodesCount) {
    ...
    genFixed: function() {
        ...
        var page = cur.pgPage,
            per = cur.pgPerPage,
            delta = cur.pgDelta || 2;
        var count = Math.ceil(cur.pgCount / per),
            offset, symbol, cls, sz, html = [];
        if (page > delta) {
            if (cur.pgByOne) {
                offset = (page - 1) * per;
                symbol = '&lsaquo;'
    ...
    nav: function(changed, oldLoc, newLoc, opts) {
        ...
        var offset = intval(changed.offset === undefined ? oldLoc.offset : changed.offset),
            fix = opts.pgFromFixed;
        ...
        ajax.post(cur.pgUrl, extend({
            ...
            onDone: function() {
                ...
                nav.setLoc(newLoc);
                setTimeout(function() {
                    if (fix || !cur.pgNoNavScroll) {
                        Pagination.setScroll(fix ? cur.pgFixedStart - cur.pgFixed.offsetTop : 0);
                    }
                }, 0);
            },
            frame: 1,
    ...
    updateFixedLeft: function(e) {
        ...
        var wrap = ge('wide_column') || ge('page_body');
        if (vk.rtl) {
            cur.pgFixed.style.left = getXY(wrap)[0] + 'px';
        } else {
            cur.pgFixed.style.left = getXY(wrap)[0] + getSize(wrap)[0] - getSize(cur.pgFixed)[0] + 'px';
        }
    },
    ...
    scrollResize: function(e) {
        ...
        if (e.type == 'scroll') {
            ...
            var children = cur.pgCont.childNodes,
                l = Math.ceil(cur.pgNodesCount / cur.pgPerPage);
            ...
            if (i == l - 1 && children[cur.pgNodesCount - 1] && st + lastWindowHeight > children[cur.pgNodesCount - 1].offsetTop + cur.pgFixedDelta) {
                ++i;
            }
            if (i > 0) --i;
    ...
    loaded: function(count, from, rows, offset, pages, preload) {
        ...
        if (gotNewPage) { // got new page
            ...
            var cont = ge(cur.pgCont),
                d = (cur.pgPrepare || Pagination.prepare).apply(window, arguments);
            while (d.firstChild) {
    ...
    showMore: function() {
        ...
        var btn = (cur.pgMore.tagName.toLowerCase() === 'button'),
            sp = btn ? lockButton.pbind(cur.pgMore) : function() {
                show(cur.pgMorePrg);
                hide(domFC(cur.pgMore));
            },
            hp = btn ? unlockButton.pbind(cur.pgMore) : function() {
                show(domFC(cur.pgMore));
                hide(cur.pgMorePrg);
            };
        if (!isVisible(cur.pgMore) || btn && buttonLocked(cur.pgMore) || isVisible(cur.pgMorePrg)) return;
        if (cur.pgLoading) {
            cur.pgLoading = 2;
            return;
        }
