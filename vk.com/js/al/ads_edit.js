...
AdsEdit.showHelpCriterionTooltip = function(helpTooltipName, targetElem, ttHandler, ttContainer, helpText, shiftLeft, shiftTop, curLocal, forceTooltip) {
  ...
  if (cur.lastHelpTooltipName && ((cur.lastHelpTooltipName != helpTooltipName))) {
    ...
    if (lastTooltip) {
      lastTooltip.hide();
      if (inArray(helpTooltipName,  ['description', 'title'])) {
        forceTooltip = true;
      }
    }
  }
...
AdsViewEditor.prototype.init = function(options, editor, targetingEditor, params, paramsData, paramsParams) {
  ...
  this.params = {
    ...
    title:                  {value: '', value_escaped: '', value_default: '', max_length: 0, max_new_lines: 0, value_p: '', value_a: '', value_k: '', value_d: '', value_max: '', update_value_max: true},
...
AdsViewEditor.prototype.updateUiParam = function(paramName) {
  ...
  switch (paramName) {
    ...
    case 'views_places':
      ...
      this.params[paramName].disabled      = (this.params[paramName].disabled_web);
...
AdsViewEditor.prototype.getUiParamData = function(paramName) {
  switch (paramName) {
    ...
    case 'views_places': // Needed to allow different items for different coditions, not only for web, Example: For only_apps it may be: 1) in_catalog, 2) in_news, 3) in_left_menu, 4) everywhere
      var data = [];
      for (var i in this.params[paramName].data_all) {
        var item = this.params[paramName].data_all[i];
        if (item[0] == 0 && this.params[paramName].disabled_web) {
          // Skip
        } else {
          data.push(item);
        }
...
AdsViewEditor.prototype.updateUiParamVisibility = function(paramName) {
  switch (paramName) {
    case 'format_type':
      ...
      toggleClass(this.options.targetIdPrefix + paramName + '_exclusive_wrap',           'unshown', elemsHidden[AdsEdit.ADS_AD_FORMAT_TYPE_EXCLUSIVE]           = !(inArray(linkTypeValue, [AdsEdit.ADS_AD_LINK_TYPE_GROUP, AdsEdit.ADS_AD_LINK_TYPE_EVENT, AdsEdit.ADS_AD_LINK_TYPE_PUBLIC, AdsEdit.ADS_AD_LINK_TYPE_URL, AdsEdit.ADS_AD_LINK_TYPE_VIDEO])));
    ...
    case 'title':
      ...
      break;
    case '_title':
      toggleClass(this.options.targetIdPrefix + 'title_reduce', 'unshown', !(inArray(this.params.format_type.value, [AdsEdit.ADS_AD_FORMAT_TYPE_PROMOTION_COMMUNITY, AdsEdit.ADS_AD_FORMAT_TYPE_APP_IN_NEWS, AdsEdit.ADS_AD_FORMAT_TYPE_APPS_ONLY, AdsEdit.ADS_AD_FORMAT_TYPE_GROUPS_ONLY, AdsEdit.ADS_AD_FORMAT_TYPE_MOBILE]) && this.params.title.value_max.match(/[^\s:,][\s:,]+[^\s:,]/)));
      break;
    case 'description':
      this.initUiParam(paramName);
      toggleClass('ads_edit_ad_row_' + paramName, 'unshown', !!this.params[paramName].hidden);
  ...
  do {
    ...
    switch (paramName) {
      ...
      case 'link_type':
        ...
        this.params.views_places.hidden = !!(!this.params.views_places.allow_web || !inArray(this.params.link_type.value, [AdsEdit.ADS_AD_LINK_TYPE_GROUP, AdsEdit.ADS_AD_LINK_TYPE_EVENT, AdsEdit.ADS_AD_LINK_TYPE_PUBLIC, AdsEdit.ADS_AD_LINK_TYPE_APP, AdsEdit.ADS_AD_LINK_TYPE_URL]));
      ...
      case 'cost_per_click':
        ...
        break;
      case 'views_places':
        this.params.views_places.value_normal = this.params.views_places.value;
...
AdsViewEditor.prototype.completeLink = function() {
  ...
  if (isChangedImportant) {
    ...
    this.updateUiParamVisibility('format_type');
    this.updateUiParamVisibility('description');
    this.updateUiParamVisibility('views_places');
  }
