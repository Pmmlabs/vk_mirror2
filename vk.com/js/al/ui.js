...
Ui.tableInitFilters = function(table) {
    var tableFilterId = table.getAttribute('ui_table_filter_id');

    Ui.tableInitFilterDropdowns(table);
    if (tableFilterId) {
        uiSearch.init(ge(tableFilterId));
        setTimeout(elfocus.pbind(ge(tableFilterId)), 0);
    }

    Ui.tableUpdateFilterTerm(table);
}

Ui.tableInitFilterDropdowns = function(table) {
    var tableFilterId = table.getAttribute('ui_table_filter_id');

    var column;
    table.uiFilterDropdowns = {};
    each(geByClass('ui_table_filter_dropdown', ge(tableFilterId).parentNode), function(k, v) {
        column = v.getAttribute('data-column');
        table.uiFilterDropdowns[column] = new InlineDropdown(v, {
            withArrow: true,
            onSelect: Ui.tableUpdateFilterTerm.pbind(table)
        });
    });
}

Ui.tableUpdateFilterTerm = function(table) {
    var tableFilterId = table.getAttribute('ui_table_filter_id');
    var tableFilterTextColumn = table.getAttribute('ui_filter_text_column');

    var filterTerm = {};
    var value;
    each((table.uiFilterDropdowns || {}), function(k, v) {
        value = v.val().trim();
        if (value) {
            filterTerm[k] = value;
        }
    ...
    value = val(tableFilterId).trim();
    if (value) {
        filterTerm[tableFilterTextColumn] = value;
    }

    Ui.tableFilterRows(table, filterTerm);
}

Ui.tableSetFilterTerm = function(table, filterTerm, filterColumns) {
    filterColumns = filterColumns.split(',');

    var filterTermObject = filterTerm;

    if (typeof filterTermObject === 'string') {
        filterTermObject = {};
        filterTermObject[filterColumns[0]] = trim(filterTerm);
    }

    // convert object term to string term
    ...
    for (var i in filterColumns) {
        ...
        if (column in filterTermObject) {
            filterTermItem = '[^~]*' + filterTermObject[column].replace(/[~]/g, '') + '[^~]*';
        } else {
            filterTermItem = '[^~]*';
        }
        filterTermArray.push(column + '=' + filterTermItem);
    }

    filterTerm = '~' + filterTermArray.join('~') + '~';

    var currentTerm = table.getAttribute('ui_filter_term');
    if (currentTerm === filterTerm) {
        return false;
    }
    table.setAttribute('ui_filter_term', filterTerm);
    if (filterTerm.length == 0) {
        table.removeAttribute('ui_filter_rows_indices');
        return true;
    } else {
        var filterDataRaw = table.getAttribute('ui_filter_data');
        var filterIndex = parseJSON(table.getAttribute('ui_filter_index'));
        var filterRegexp;
        var filterMatch;
        var rowsMatches;
...
Ui.tableOnFilterKeypress = function(event, tableId) {
    ...
    Ui.tableFilterTimeout = setTimeout(function() {
        Ui.tableUpdateFilterTerm(ge(tableId));
    }, 200);
}
...
Ui.tableGetRowsSort = function(table, tableRows, tableMore, tableExcluded, filterRowsIndices, groupIndices) {
    ...
    if (!rowsSort) {
        ...
        if (Object.defineProperty) {
            Object.defineProperty(table, 't_rows_sort', {
                configurable: true,
                writable: true
            });
...
Ui.tableGetAllIndices = function(table, headerCell) {
    ...
    if (!groupIndices) {
        ...
        if (Object.defineProperty) {
            Object.defineProperty(table, 'p_group_indices', {
                configurable: true,
                writable: true
            });
    ...
    if (!wideIndices) {
        ...
        if (Object.defineProperty) {
            Object.defineProperty(table, 'p_wide_indices', {
                configurable: true,
                writable: true
            });
    ...
    if (!sortIndicesAsc) {
        ...
        if (Object.defineProperty) {
            Object.defineProperty(headerCell, 'p_sort_indices_asc', {
                configurable: true,
                writable: true
            });
            Object.defineProperty(headerCell, 'p_sort_indices_desc', {
                configurable: true,
                writable: true
            });
...
Ui.tableFilterRows = function(table, term) {
    ...
    var filterColumns = table.getAttribute('ui_filter_columns');
    var tableResultId = table.getAttribute('ui_table_result_id');
    var tableResult = ge(tableResultId);
    var tableRows = ge(tableRowsId);
    var tableMore = ge(tableMoreId);
    ...
    var setFilterResult = Ui.tableSetFilterTerm(table, term, filterColumns);
...
Ui.tableUpdateSort = function(table, tableRows, tableMore, tableExcluded, headerCell, changeSortOrder) {
    ...
    // Modify detached DOM
    {
        ...
        if (rowsMore && rowsMore.length) {
            Object.defineProperty(tableMore, 'p_rows_more', {
                configurable: true,
                writable: true
            });
    ...
    if (headerCell && changeSortOrder) {
        removeClass(geByClass1('sort', headerCell.parentNode.parentNode), 'sort');
        if (!sortCurrent || sortOriginal != 3) {
