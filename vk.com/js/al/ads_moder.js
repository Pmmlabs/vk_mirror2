...
AdsModer.openFeaturesEditBox = function(unionId, hash, featuresInfo, featuresEditHtml) {
    ...
    editBox.addButton('��������', saveFeaturesHandler, 'yes');
...
AdsModer.saveFeatures = function(unionId, hash, featuresInfo, editBox) {
    ...
    ajax.post('/adsmoder?act=a_edit_features', ajaxParams, {
        onDone: onComplete,
        onFail: onComplete
    });
...
AdsModer.saveNote = function(noteEditBox, ajaxParams, isDelete, doneRedirect) {
    ...
    ajax.post('/adsmoder?act=a_edit_notes', ajaxParams, {
        onDone: onComplete,
...
AdsModer.openCategoriesEditBox = function(requestKey, linkElem) {
    var editBox = showFastBox({
        title: '��������� �������� ����������',
        width: 440
    }, cur.categoriesEditBoxHtml);
    editBox.removeButtons();
    editBox.addButton(getLang('box_cancel'), false, 'no');
    editBox.addButton('��������', applyChanges, 'yes');
...
AdsModer.premoderationProcessRequest = function(action, requestKey, requestKeyModer, onCompleteExternal) {
    ...
    ajax.post('/adsmoder?act=a_premoderation_process', ajaxParams, {
        onDone: onComplete,
        onFail: onComplete
    });

    function onComplete(response) {
        var responseText = ((response && response.text) ? response.text : '������!');
...
AdsModer.premoderationProcessRequestsMassCheck = function(action, requestKey) {
    ...
    var processOneText = ((action === 'approve') ? '�������� ����' : '��������� ����');
...
AdsModer.premoderationTakeUnion = function(unionId, hash) {
    ...
    ajax.post('/adsmoder?act=a_premoderation_manage_work', ajaxParams, {
        onDone: onComplete,
        onFail: onComplete
    });
...
AdsModer.premoderationFixRequest = function(wrapElemId, requestId, action, hash) {
    ...
    ajax.post('/adsmoder?act=premoderation_fix_request', ajaxParams, {
        onDone: onComplete,
        onFail: onComplete
    });
...
AdsModer.cancelClicks = function(box, ajaxParams) {
    ...
    ajax.post('/adsweb?act=log_cancel', ajaxParams, {
        onDone: onComplete,
        onFail: onComplete
    });
    ...
    function onComplete(response) {
        ...
        if (isError) {
            showFastBox('������', (response && typeof response === 'string') ? response : '������');
        }
        return true;
    }
...
AdsModer.initClickfraudersPrepareBanBox = function(box, hash, usersIds, day) {
    ...
    box.removeButtons();
    box.addButton(getLang('box_cancel'), false, 'no');
    box.addButton('��������', prepareBanHandler, 'yes');
}
...
AdsModer.clickfraudersPrepareBan = function(box, hash, usersIds, day) {
    ...
    ajax.post('/adsweb?act=clickfrauders_prepare_ban', ajaxParams, {
        onDone: onComplete,
        onFail: onComplete
    });
...
AdsModer.increaseBudget = function() {
    ...
    ajax.post('/adsmoder?act=increase_budget', ajaxParams, {
        onDone: onComplete,
        onFail: onComplete
    });
    ...
    function onComplete(response) {
        ...
        if (response && response.redirect) {
            ...
            var msg = ((response && response.error) ? response.error : '������');
            showFastBox('������', msg);
        }
        return true;
    }
...
AdsModer.openDisableOfficeBox = function(ajaxParams, isCompleteCompany, companyName, companyClearFields) {
    ...
    var box = showFastBox(getLang('ads_disable_office_box_title'), boxHtml, getLang('ads_disable_office_box_button'), function() {
        AdsModer.disableOffice(box, ajaxParams);
    }, getLang('box_cancel'));
}
...
AdsModer.disableOffice = function(box, ajaxParams) {
    ...
    ajax.post('/adsmoder?act=a_disable_office', ajaxParams, {
        onDone: onComplete,
        onFail: onComplete.rpbind(true)
    });
...
AdsModer.doCreateOffice = function(buttonElem) {
    ...
    ajax.post('/adsmoder?act=do_create_office', ajaxParams, {
        onDone: onComplete,
        onFail: onComplete
    });
...
AdsModer.initTemplateActions = function() {
    ...
    each(geByClass('ads_moder_template_actions'), function(i, v) {
        ...
        if (hasClass(v, 'ads_moder_template_inactive')) {
            this_items.push(['enable', '��������']);
        }
        if (hasClass(v, 'ads_moder_template_active')) {
            this_items.push(['disable', '���������']);
        }
...
AdsModer.initReachChart = function(data, lines, is_cpc, ratio, ratio_index) {
    var options = {
        tooltip: {
            ...
            shared: true,
            formatter: function() {
                var s = '������� ������: ' + getLang('ads_target_audience_edit', this.x, true) + "<br>";
                var count = this.x / ratio;
                s += (is_cpc ? '������� ���������' : '������� �������') + ': ';
                s += langNumeric(Number(count * (is_cpc ? 1 : 1000)).toFixed(0), "<b>%s</b>", true) + "<br>";
                if (ratio_index < this.points.length) {
                    s += "������� �������: " + getLang('ads_moder_bold_money_amount_rub_short', Number(count * this.points[ratio_index].y).toFixed(2).replace('.00', ''), true) + "<br>";
                }
    ...
    AdsModer.initChart('highcharts_container', data, options, '�����',
        (is_cpc ? '���� �� ����, ���.' : '���� �� 1000 �������, ���.'), lines);
};
...
AdsModer.premoderationValidationUpdateRequestsSelected = function() {
    ...
    if (cur.premoderationValidation.selectedRequestsNum > 0) {
        text += ', ����� ' + langNumeric(cur.premoderationValidation.selectedRequestsNum, ['', '%s ����������', '%s ����������', '%s ����������']);
    }
...
AdsModer.initPremoderationValidationRequests = function() {
    ...
    if (selectableRequestWraps.length > 0) {
        ...
        ge('process_premoderation_validation_btn').innerHTML = '<span>���������� ������ ������</span>';
...
AdsModer.premoderationValidationProcessSelectedRequests = function() {
    var processPremoderationValidationBtn = ge('process_premoderation_validation_btn');
    var ajaxParams = {
        hash: cur.premoderationValidation.hash,
        valid_requests: '',
        invalid_requests: ''
    };
    ...
    ajax.post('/adsmoder?act=a_premoderation_validation_process', ajaxParams, {
        onDone: onComplete,
        onFail: onComplete
    });
};
...
AdsModer.premoderationSetResultReasonsText = function(requestId) {
    ...
    resultHolderEl.innerHTML = selectedReasonIds.map(function(reasonId) {
        return cur.moderReasonsDict[reasonId];
    }).join(', ');
}
