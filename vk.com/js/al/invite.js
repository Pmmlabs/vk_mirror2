var inviter = {
    ...
    genDays: function(year, month) {
        var result = [
            ...
            last = inviter.getLastDay(year, month);
    ...
    init: function(opts) {
        ...
        var slideShow = inviter.slideShow,
            slideHide = inviter.slideHide;

        var uiLang = new Dropdown(ge('inv_lang'), [
            [0, '�������'],
            [3, 'English'],
            [73, 'Portugues']
        ], {
    ...
    send: function(force) {
        ...
        var fnameRaw = trim(ge('inv_fname').value),
            lnameRaw = trim(ge('inv_lname').value);
        var name = cleanName(fnameRaw, lnameRaw);
        fname = name[0];
        lname = name[1];

        var sex = isVisible('inv_sex_row') ? intval(ge('inv_sex').value) : -1,
            phone = ge('inv_mobile').value;
        ...
        var params = {
            ...
            sex: sex,
            phone: phone,
            lang: isVisible('inv_lang_row') ? ge('inv_lang').value : 0
        };
        if (curBox()) {
            params.from = 'box';
        }
