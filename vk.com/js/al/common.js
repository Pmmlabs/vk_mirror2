...
function __adsGetAjaxParams(ajaxParams, ajaxOptions) {
  ...
  //aes_light.js moved to common.js
  //var result = stManager.add(['aes_light.js'], __adsGetAjaxParams.pbind(ajaxParams, ajaxOptions));
  var result = __adsGetAjaxParams.pbind(ajaxParams, ajaxOptions);
...
function __adsSet(adsHtml, adsSection, adsCanShow, adsShowed, adsParams) {
  ...
  if (adsHtml && (adsHtml.slice(0, adsExperimentMarker.length) === adsExperimentMarker)) {
    if (Math.random() < 0.01) { ajax.post('/wkview.php?act=mlet&mt=9003', {}, {onFail: function () { return true; }}); }
  }
  if (Math.random() < 0.01) { ajax.post('/wkview.php?act=mlet&mt=9000', {}, {onFail: function () { return true; }}); }
  ...
  __adsSet = function() {
    if (Math.random() < 0.01) { ajax.post('/wkview.php?act=mlet&mt=9002', {}, {onFail: function () { return true; }}); }
  ...
  __adsSet(adsHtml, adsSection, adsCanShow, adsShowed, adsParams);
}
  ...
  AdsLight.restoreVisibility = function (elem, tempStyles) {
    ...
    var cssStyles = {
      display: function (element) {
        ...
        if (/^(:?textarea|acronym|strong|select|object|button|small|label|input|time|span|samp|code|cite|abbr|var|sup|sub|map|kbd|img|dfn|big|bdo|tt|em|br|q|i|b|a)$/i.test(element.tagName)) {
          displayVal = 'inline';
        } else {
    ...
    var matches = Function.call.bind(Element.prototype.matchesSelector ||
        Element.prototype.mozMatchesSelector || Element.prototype.webkitMatchesSelector ||
        Element.prototype.msMatchesSelector || Element.prototype.oMatchesSelector);

    var childrenClassesUniq= arrUnique(getChildrenClasses(elem));
    var childrenClassesRexExp = '';

    for(var i in childrenClassesUniq) {
      childrenClassesRexExp += '\\.' + childrenClassesUniq[i] + (i == childrenClassesUniq.length - 1 ? '|ads_ad' : '|');
    }

    var cssRulesAll = getCssRules();
    var cssRules    = cssRulesAll.filter(function(cssRule) {
      return !!cssRule.selectorText.match(childrenClassesRexExp);
    });
    var checkParams = {
      target: elem,
      rules: cssRules
    ...
    parents.reduce(function (promise, el) {
    ...
    function getChildrenClasses(elem) {
      ...
      for (var i = 0; i < elem.childNodes.length; i++) {
        if (elem instanceof Element) {
          ...
          if (child.className) {
            ownClasses = child.className.split(' ').filter(function(className) {
              return className && (className.indexOf('ads_ad') < 0)
            });
          }
          classes = classes.concat(ownClasses);
          classes = classes.concat(getChildrenClasses(child));
        }
      }
      return classes;
    }

    function getStyleSheetsRules(styleSheet) {
      var rules = [];
      if (!(styleSheet.media && styleSheet.media.length && styleSheet.media[0] === 'print') && styleSheet.cssRules) {
        Array.prototype.slice.call(styleSheet.cssRules).reduce(function (rules, rule) {
          if (rule.styleSheet) {
            Array.prototype.push.apply(rules, getStyleSheetsRules(rule.styleSheet));
    ...
    function getCssRules() {
      return Array.prototype.slice.call(document.styleSheets).reduce(function (rules, styleSheet) {
        Array.prototype.push.apply(rules, getStyleSheetsRules(styleSheet));
        return rules;
      }, []);
    }

    function getElementStyles(elem, cssRules) {
      var styles = {};
      var inlineStyles = elem.style.cssText.split(';');

      var elemRules = cssRules.filter(function (cssRule) {
        AdsLight.cntr +=1;
        return matches(elem, cssRule.selectorText);
      });
      if (elemRules.length) {
        each(elemRules, function (idx, cssRule) {
          styleNames.reduce(function (styles, prop) {
            if (prop in cssRule.style && cssRule.style[prop] !== '') {
              styles[prop] = cssRule.style[prop];
            }
            return styles;
          }, styles);
        });
      ...
      for (var i in inlineStyles) {
        var inlineStyle = inlineStyles[i].split(":");
        styles[inlineStyle[0]] = inlineStyle[1];
      }
      return styles;
    }

    function checkElemStyles(params, elem) {
      var computedStyles;
      var shown = true;
      ...
      if (window.getComputedStyle(elem).display === 'none') {
        ...
        var elemStyles = getElementStyles(elem, params.rules);
        each(styleNames, function (idx, styleName) {
          var styleValue = elemStyles[styleName];
          if (!styleValue) {
            styleValue = typeof cssStyles[styleName] === 'function' ?
                cssStyles[styleName](elem) :
                cssStyles[styleName];
          }
          if (!computedStyles || styleValue !== computedStyles[styleName]) {
            elem.style.setProperty(styleName, styleValue, 'important');
          }
        });
        if (window.getComputedStyle(elem).display === 'none') {
          shown = false;
    ...
    function checkElemParams(params, elem) {
      checkElemStyles(params, elem);
