...
function updateOtherCounters(url, referrer) {
    ...
    var tnsPixelSocdem = vk.tnsPixelSocdem || '83';
    var tnsPixelRef = referrer.replace(/\*/g, '%2a');
    vkImage().src = locProtocol + '//www.tns-counter.ru/V13a**clid:' + tnsPixelSocdem + '*' + tnsPixelRef + '*vk_com/ru/UTF-8/tmsec=vksite_total/' + Math.round(Math.random() * 1E9);
    statlogsValueEvent('counters_check', 1);
...
function showWiki(page, edit, e, opts) {
    ...
    ajax.post('wkview.php', extend({
        act: 'show',
        loc: nav.objLoc[0]
    }, page, opts.ads_params, (cur.getWkviewOpts && cur.getWkviewOpts())), params);
    return cancelEvent(e);
}
