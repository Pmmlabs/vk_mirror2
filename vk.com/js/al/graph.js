if (!window.Graph)
    (function(window) {
        ...
        ]; // using 'year' ('00, '01, '02...) seems a bad idea
            ...
            for (var l = 0; l < lines.length; l++) {
                ...
                if (smooth && (enIdx - stIdx + 1 > 2)) {
                    ...
                    for (var i = stIdx; i <= enIdx; i++) {
                        knots[n] = {
                            X: x + (ps[i].x - stTime) * xfactor,
                            Y: y + h - (ps[i].y + yDelta) * yfactor
                        };
                        n++;
                    }
                    n--;
                    ...
                    var ys1 = getFirstControlPoints(rhs),
                        xs2 = [],
                        ys2 = [];
                    for (var i = 0; i < n; ++i) {
                        if (i < n - 1) {
                ...
                ctx.beginPath();
                var px = x + (ps[stIdx].x - stTime) * xfactor,
                    py = y + h - (ps[stIdx].y + yDelta) * yfactor;
                moveTo(ctx, px, py);
                if (lines[l].b) {
                    for (var i = stIdx; i <= enIdx; i++) {
      ...
      <div style="float: left; width: 270px; padding-top: 8px; -moz-user-select: none; -khtml-user-select: none;"></div><div style="float: left; width: 270px; padding-top: 8px; -moz-user-select: none; -khtml-user-select: none;"></div>\
            ...
            if (typeof params.multiple == 'object' && params.multiple.items) {
                ...
                if (params.multiple.graph_item) {
                    ...
                    var clickContext = {
                        clickNode: params.multiple.items.length,
                        graph: this,
                        alt_graph: true
                    };
                    addEvent(item, 'click', this.onClickMenu, null, clickContext);
                ...
                for (i in params.multiple.altitems) {
                    ...
                    var clickContext = {
                        clickNode: i,
                        graph: this,
                        force_func: params.multiple.altitems[i][1]
                    };
                    addEvent(item, 'click', this.onClickMenu, null, clickContext);
                    cur.destroy.push(function() {
                        removeEvent(item, 'click', this.onClickMenu, null, clickContext);
            ...
            try {
                ...
                this.message.innerHTML = (cur.graphVars['lang.bad_browser'] || '��� ����������� ������� ���������� ��� ���������� {link}�������� �������{/link}.').replace('{link}', '<a onclick="Graph.prototype.showBadBrowserBox();">').replace('{/link}', '</a>');
                setStyle(this.graphDiv, {
            ...
            if (vk.al) {
                stManager.add(['tooltips.js', 'tooltips.css'], function() {
                    tooltips.create(graph.mainLayout, {
                        text: '...',
                        black: 1,
                        forcetoup: 1,
                        nohide: 1
        ...
        Graph.prototype = {
            ...
            wheelView: function(ev) {
                ...
                var g = ev.currentTarget.graph,
                    ...
                    dist = g.rightTime - g.leftTime,
                    diffL = (delta * dist) / 3500,
                    diffR;
                if (g.disable_zoom) return;
            ...
            downloadCSV: function(ev) {
                var data = ev.currentTarget.line.d,
                    fileName = ev.currentTarget.line.name.replace(/[^a-z�-�0-9\-_\(\)\.\,\;]/gi, "_").slice(0, 100),
                    CSV = '',
                    minDate, maxDate;
                ...
                var blob = new Blob([csv_header, CSV], {
                    type: 'text/csv;'
                });
            ...
            showDots: function(ev) {
                ...
                var ccx = ev.pageX - getXY(g.zinView)[0],
                    ccy = ev.pageY - getXY(g.zinView)[1];
                var max_coltext = [0, 0];
                var activeLine = -1;
            ...
            hideDots: function(ev) {
                var g = (ev.currentTarget ? ev.currentTarget.graph : this),
                    vwPos = getXY(g.zinView);
            ...
            expandLabel: function(ev) {
                var g = ev.currentTarget.graph,
                    line = ev.currentTarget.line,
                    dot = line.dotLabel.dot,
                    pos = line.dotLabel.pos;
                if (!g.smoothLines) {
                    var text = '';
                    if (line.l) {
            ...
            redrawWindow: function() {
                ...
                // get visible data
                var localMax = (this.adjust ? -1e9 : 0),
                    localMin = (this.adjust ? 1e9 : 0),
                    outMax = 0,
                    outMin = 0,
                    dotsVisible = 0;
                ...
                for (var i = i_start;
                    ...
                    if (i < xscales.length) {
                        ...
                        var sc = {
                            base: yearCnt * YEAR,
                            format: ['yearFull'],
                            sub: Math.max(1, yearCnt / 10) * YEAR
                        };
                    }
    ...
    <style>\
      ...
      #stats_good_browsers a:hover {\
        ...
        background-color: #edf1f5!important;\
                ...
                cur.badBrowserBox = MessageBox({
                    width: 540,
                    hideButtons: true,
                    bodyStyle: 'padding: 25px; text-align: center;',
                    hideOnBGClick: true
