window.events = {
    ...
    createEvent: function(hash) {
        ...
        /*.setButtons(getLang('global_save'), function() {
    ...
    subscribeSMS: function(eid, hash) {
        ...
        ajax.post('al_mobile.php', {
            act: 'a_subscribe_page',
            gid: eid,
            hash: hash
        }, {
            onDone: function(subscribed) {
                onChange(subscribed);
            }
        });
            ...
            try {
                ...
                each(cur.tabs, function(i, tab) {
                    if (cur.lists[tab] && cur.lists[tab].data) {
                        each(cur.lists[tab].data, function(j, v) {
                            if (v && v[0] == eid) {
                                debugLog('set status ', status);
                                v[2][4] = status;
                            }
                        });
                    }
                ...
                switch (status) {
                    case 1:
                        status_text = getLang('events_you_unsure');
                        break;
                    case 2:
                        status_text = getLang('events_you_cant_participate');
                        break;
                    default:
                        status_text = getLang('events_you_will_participate');
                        break;
                }
    ...
    <div class="button_cancel fl_l"><div class="leave_button">' + getLang('events_user_cant_participate') + '</div></div>';
        ...
        if (norequest) return;
        ajax.post('al_events.php', {
            act: 'a_enter',
            eid: eid,
            hash: hash,
            action: 'undecided'
        }, {
            onDone: function(res, count) {
    ...
    reportSpam: function(el, eid, hash) {
        ...
        ajax.post('al_events.php', {
            ...
            onDone: function() {
                var row = ge('event_row' + eid);
                if (!row) return;
                var status = geByClass('status', row)[0],
                    actions = geByClass('actions', row)[0];
    ...
    friendsBox: function() {
        showBox('al_events.php', {
            act: 'a_get_friends',
            pid: cur.options.event_id
        }, {
            params: {
                width: 467,
                dark: 1
    ...
    switchTab: function(tab, onEnd, onHeader) {
        ...
        var w = ge('info_module_wrap'),
            c = ge('info_module_cont');
        if (tab == 'info_module_tab') setStyle(w, {
        ...
        var urls = {
            'info_module_tab': '',
            'edit_module_tab': '?act=edit',
            'list_module_tab': '?act=edit&list=' + cur.options.list
        };
        var t1 = ge(cur.tab),
            t2 = ge(tab),
            back = (pos[tab] > pos[cur.tab]);
        var h1 = getSize(t1)[1];
    ...
    extend(cur, {
        ...
        calTrailOn: function(el, fid, day) {
            ...
            var showTT = function() {
                ...
                showTooltip(el, {
                    content: '<div class="wrap"><div class="content"><a href="' + item[2] + '" onclick="return nav.go(this, event);"><img src="' + item[5] + '"/></a><div class="name">' + name + '</div>' + text + '</div></div>',
                    className: 'rich wall_tt cal_tt',
                    slide: 15,
                    shift: [40, 3, 3],
                    toup: true
                });
...
var AutoLoadList = function(container, data, options) {
    ...
    var getRow = options.getRow.bind(this),
        id = container.id;
    var showMoreBox = ge(id + '_show_more'),
        showMoreLink = ge(id + '_show_more_link');
    ...
    this.preload = function() {
        ...
        self.loading = [false];
        ajax.post(options.url, options.query, {
            onDone: self.loaded
        });
    }
    ...
    this.showMore = function(search, force, listLen) {
        ...
        var len = Math.min(fLen, offset + listLen),
            j = 0,
            html = [],
            start = refresh ? 0 : offset;
