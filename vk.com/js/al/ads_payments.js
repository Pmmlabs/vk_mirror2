...
AdsPayments.checkBudgetSumInc = function(isRequest) {
  ...
  if (isNormalCheck && strBudgetInc.indexOf('.') != -1) {
    if (window.curIsMoney && !window.curLegal && (window.curOfficeLegal || window.userMoney > cur.unionsLimits.budget_increase_rubles_min)) {
  ...
  if (window.curIsMoney) {
    if (window.curLegal) {
      if (floatBudgetInc < cur.unionsLimits[window.curLimitMinKey]) {
        var moneyMin = getLang(window.langMoneyAmount, cur.unionsLimits[window.curLimitMinKey]);
      ...
      if (isNormalCheck && floatBudgetInc < cur.unionsLimits.budget_increase_rubles_min) {
    ...
    if (floatBudgetInc < cur.unionsLimits.budget_increase_votes_min) {
      budgetError.innerHTML = window.lowMoneyPayment;
      hide(budgetNoticeBox);
      show(budgetErrorBox);
      return false;
...
AdsPayments.onBudgetRublesChanged = function(delayed) {
  if (!window.curPaymentPage) {
    ...
    window.changeBudgetDelayedCounter++;
    setTimeout(function() { AdsPayments.onBudgetRublesChanged(true); }, 300);
  }
...
AdsPayments.changeBudget = function() {
  ...
  ajax.post('/ads?act=a_union_change_budget', ajaxParams, {onDone: onAjaxSuccess, onFail: onAjaxFail});
...
AdsPayments.reqsInit = function(params) {
  ...
  if (isVisible('row_req_non_resident') && isVisible('row_req_resident_edit')) {
    cur.uiNonResident0 = new Radiobutton(ge('req_non_resident0'), {width: 250, label: getLang('ads_reqs_box_resident'), onSelect: function(value) { AdsPayments.reqsOnChangeNonResident(value); }});
    cur.uiNonResident1 = new Radiobutton(ge('req_non_resident1'), {width: 250, label: getLang('ads_reqs_box_non_resident'), onSelect: function(value) { AdsPayments.reqsOnChangeNonResident(value); }});
...
AdsPayments.reqsReinitContractDay = function(year, month, day) {
  if (cur.uiContractDate) {
    ...
    var datePickerOptions = {
      onUpdate: function(date, mode) { cur.contractDate = date; },
      //mode: 'h',
      width: 250 + 8,
      pastActive: true,
      year: year,
...
AdsPayments.reqsCheckInn = function(delayed) {
  if (delayed) {
    ...
    cur.checkReqInnDelayed++;
    setTimeout(function() { AdsPayments.reqsCheckInn(true); }, 300 );
  }
  return false;
}
...
AdsPayments.reqsSave = function(checkInnOnly) {
  ...
  ajax.post('/ads?act=a_save_reqs', ajaxParams, {onDone: onCompleteWrap, onFail: onCompleteWrap});
...
try{stManager.done('ads_payments.js');}catch(e){}
