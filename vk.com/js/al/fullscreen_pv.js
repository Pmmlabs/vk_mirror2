var FullscreenPV = {
    init: function() {
        ...
        var ss = positive(getCookie('remixfsss')),
            ssd = Math.min(positive(getCookie('remixfsdt')) || 5, 15);
...
<div id="pv_fs_right_wrap" class="pv_fs_controls"><div id="pv_fs_right"></div></div>',
    ...
    over: function(ev) {
        var e = ev || window.event,
            t = e.target || e.srcElement || document;
    ...
    showControls: function(ev) {
        var x = cur.pvFSrX = ev.pageX,
            y = cur.pvFSrY = ev.pageY,
            l, r;
    ...
    startSlide: function() {
        if (hasClass(cur.pvFSSlideCont, 'pv_fs_slide_on')) {
            ...
            cur.hideAboutTO = setTimeout(function() {
    ...
    updateInfo: function() {
        var cnt = ((cur.pvData || {})[cur.pvListId] || []).length,
            ind = cur.pvIndex + 1,
            topright, bottomleft;
