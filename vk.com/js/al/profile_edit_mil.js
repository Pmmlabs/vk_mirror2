var ProfileEditorMil = {
        ...
        if (!isVisible('militaries')) {
            ...
            if (cur.militariesCount) {
                for (var i = 0; i < cur.militaries.length; ++i) {
                    ge('militaries').appendChild(this.genMilitaryRow(cur.militaries[i].id, i == 0));
    ...
    genOneRow: function(field, id, label, params, additional) {
        ...
        return '<div class="pedit_row clear_fix" id="row_' + key + '">' +
            ...
            '<div class="pedit_labeled"><input class="pedit_dropdown" type="hidden" id="' + key + '" name="' + key + '" ' + params + '/></div>' +
            additional + '</div>';
    },

    genMilitaryRow: function(military_id, first_row) {
        return ce('div', {
            className: 'pedit_edu_big_row',
            id: 'military' + military_id,
            innerHTML: (!first_row ? '<div class="pedit_separator"></div>' : '') +
                '<div id="content' + military_id + '">' +
                this.genOneRow('country', military_id, getLang('select_country'), '',
                    '<div class="pedit_right_control">' + ProfileEditor.deleteLinkHtml('ProfileEditorMil.deleteMilitary(' + military_id + ')') + '</div><img src="/images/upload.gif" class="pedit_progress" id="progress' + military_id + '" />') +
                ...
                '<a class="pedit_right_control" onclick="ProfileEditorMil.restoreMilitary(' + military_id + ')">' + getLang('global_dont_delete') + '</a>' +
                '</div>'
        }, {
            display: 'none'
        });
    ...
    initMilitaryRow: function(military, elem) {
        var g = elem ? function(id) {
            return ProfileEditorMil.get_by_id(elem, id);
        } : ge;
    ...
    addMilitary: function() {
        ...
        var new_el = ge('militaries').appendChild(this.genMilitaryRow(new_military.id, cur.militariesCount == 0));
        if (!cur.militariesCount) {
        ...
        var doneHandler = function(response) {
            ...
            for (var i = 0; i < cur.militaries.length; ++i) {
                ...
                if (intval(new_military_id)) {
                    cur.militaries[i] = this.updateMilitary(cur.militaries[i], new_military_id, ge('militaries'), i);
                } else {
                    to_remove.push(i);
                }
            }
    ...
    updateMilitary: function(old_military, new_military_id, parent, i) {
        ...
        var new_elem = this.genMilitaryRow(new_military_id, i == 0);
