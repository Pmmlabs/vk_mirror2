  ...
  initWrite: function(options) {
    ...
    cur.toPeople = new Autocomplete(ge('mail_to'), 'hints.php?act=a_json_friends&from=mail', {
      ...
      customSearch: function(term) {
        return (term.indexOf('@') != -1) ? [[clean(term), clean(term), cur.lang['mail_enter_email_address'], '/images/pics/contact_info.png', 0, '']] : false
      },
      ...
      if (itemData !== undefined && typeof(itemData[3]) == 'string' && itemData[3].length) {
        if (itemData[3] == 'none') {
          ...
          if (cur.thread.id != itemValue) {
            ...
            var onl = '';
            if (itemData[4]) {
              onl = langSex(1, window.global_online);
              if (itemData[4] > 0 && itemData[4] < 6) {
...
/* Mail list actions */
  ...
  restoreSpamMsg: function (msg_id, show, out) {
    var suspected = cur.section == 'inbox' && (show || (intval(out) & 32784));
    var mb = ge('mb' + msg_id), ma = ge('ma' + msg_id), tr = ge('mess' + msg_id), bu = cur.listRowsHTML[msg_id];
    if (!tr || !mb || !ma) return false;
    if (tr.className != 'mail_spam_row' && !bu) {
      cur.listRowsHTML[msg_id] = [mb.innerHTML, ma.innerHTML, tr.className];
    }
    if (cur.section == 'spam' || suspected) {
      ma.innerHTML = '<img src="/images/upload.gif" />';
      cur.mail_cached[msg_id].deleted = true;
    } else {
      mb.innerHTML = '<img src="/images/upload.gif" />';
    ...
    ajax.post('al_mail.php', {act: 'a_restore_spam', id: msg_id, from: suspected ? 'spam' : cur.section, hash: cur.mark_hash}, {onDone: function (res, body, actions) {
      if (cur.section == 'spam' || suspected) {
        mb.innerHTML = body;
        ma.innerHTML = actions;
        cur.offset--;
    ...
    if (cur.section != 'spam' && cur.mail_cached[msg_id].unread) {
      cur.new_msg++;
      mail.updateUnread();
    }
    mail.updateSummary();
