...
function ListEditBox() {
    ...
    var listBox, self = this,
        saving_album = false,
        audioFilter, selectedAudios;

    function initBox() {
        listBox = new MessageBox({
            width: '480px',
            bodyStyle: 'padding:0px;_height:361px;_overflow:hidden;'
        });
        listBox.addButton({
            onClick: function() {
                listBox.hide(200)
        ...
        addEvent('alist_name_edit', 'keydown', function(e) {
            if (e.keyCode == KEY.RETURN) saveListName.call(this, e);
        });
        addEvent('list_edit_box_list_tab', 'click', function() {
            switchTab(0);
            audioFilter.getPage(0, getFilter());
        });
        addEvent('list_edit_box_list_selected_tab', 'click', function() {
            switchTab(1);
            audioFilter.getPage(0, getFilter());
        });
            ...
            onDone: function(obj, text) {
                try {
                    ...
                    if (window.albumDDM) {
                        ...
                        albumList.push({
                            i: 0,
                            l: getLang('audio_common_list')
                        });
                        albumDDM = new DropdownMenu(albumList, {
                            showHover: false,
                            onSelect: function(e) {
                                onAlbumChange(e);
                            }
    ...
    function showList(windowTitle, listId, listName, selectedIds) {
        ...
        var initFilter = function(data) {
            return new PagedList('list_edit_box_alist_data', data, {
                ...
                onEnd: function() {
                    animate(ge('list_edit_box_alist_data').parentNode, {
                        scrollTop: 0
                    }, 200);
                },
...
function getQueryString() {
    var query = window.location.search.substr(1),
        resultObj = {},
        keyValueArray = query.split('&');
    each(keyValueArray, function(i, obj) {
...
function showTooltip(obj, text, direction) {
    if (!tooltip) {
        var t = ge('tooltip');
        if (!t) {
            document.body.appendChild(t = ce('div', {
                id: 'tooltip',
                innerHTML: '<div class="tooltip_top"></div><div class="tooltip_bottom"></div><div class="tooltip_line1"></div><div class="tooltip_line2"></div>'
            }));
        }
        tooltip = {
            obj: t,
