! function(e) {
    ...
    function J(e, t) {
        return void 0 === t && (t = !X(e)), t ? q(e) : Q(e), t
    }
    ...
    window.ge = l, window.geByTag = d, window.geByTag1 = u, window.geByClass = p, window.geByClass1 = h, window.gpeByClass = f, window.domQuery = _, window.domQuery1 = g, window.domClosest = v, window.ce = b, window.re = w, window.se = P, window.sech = C, window.rs = E, window.psr = O, window.domReplaceEl = j, window.domEL = T, window.domNS = x, window.domPS = S, window.domFC = I, window.domLC = k, window.domPN = L, window.domChildren = A, window.domInsertBefore = N, window.domInsertAfter = M, window.domByClass = D, window.domData = R, window.domChildIndex = B, window.domCA = H, window.domClosestSibling = z, window.matchesSelector = U, window.isHover = F, window.isAncestor = W, window.getScroll = Y, window.domClosestPositioned = $, window.domClosestOverflowHidden = V, window.show = q, window.hide = Q, window.isVisible = X, window.clientHeight = G, window.getClientRectOffsetY = K, window.toggle = J, window.boundingRectEnabled = Z, window.getXYRect = ee, window.getXY = te, window.isWindow = re, window.getSize = ie, window.hasClass = oe, window.addClass = se, window.addClassDelayed = ce, window.removeClass = le, window.removeClassDelayed = de, window.toggleClass = ue, window.toggleClassDelayed = pe, window.replaceClass = he, window.getStyle = fe, window.setStyle = _e, window.setStyleDelayed = ge, window.setPseudoStyle = ve, window.data = ye, window.attr = me, window.removeAttr = be, window.removeData = we, window.cleanElems = Pe, window.setTitle = Ce, window.getZoom = Ee, window.val = Oe, window.elfocus = je, window.traverseParent = Te, window.getH = ae, window.getW = ne, window.domClosestByTag = y, window.setDocumentTitle = Se, window.lockDocumentTitle = Ie
}, , function(e, t, r) {
    "use strict";
    r.r(t);
    var i = r(34),
        ...
        d = function(e) {
            ...
            return function(e, t) {
                ...
                return a(), r.addEventListener("click", function(r) {
                    var i = e.getEditor();
                    return i.closeAllCarouselEditors(), i._resizeTooltip.hide(), addClass(e._objectEl, "article_ed__carousel_edit_open"), e._carouselEditor = new o.default(t, e, function(r) {
        ...
        each(geByClass("_article_unmute_button"), function(e, t) {
            ...
            r == n.ParagraphType.ObjectPhoto && Object(a.initPhotoCarousel)(t, d), d.mobile || r != n.ParagraphType.ObjectPhoto || function(e) {
                ...
                v.addEventListener("click", function() {
                    ...
                    r && r.innerHTML && n.appendChild(se('<div class="article_full_view__caption"><div class="article_full_view__caption_inner">' + r.innerHTML + "</div></div>")), i.length > 1 && (toggleClass(E, "article_full_view__carousel", i.length > 1), P = se('<div class="article_full_view__right"></div>'), n.appendChild(P), C = se('<div class="article_full_view__left"></div>'), n.appendChild(C), P.addEventListener("click", function(e) {
                        return t = Math.min(i.length - 1, Math.max(0, t + 1)), j(a, i, t), y && y(1), cancelEvent(e)
                    }), C.addEventListener("click", function(e) {
                        return t = Math.min(i.length - 1, Math.max(0, t - 1)), j(a, i, t), y && y(-1), cancelEvent(e)
                    })), E = se('<div class="article_full_view__counter"><div class="article_full_view__counter_text"></div><div class="article_full_view__close"></div></div>'), n.appendChild(E), i.length > 1 && toggleClass(E, "article_full_view__carousel", i.length > 1), c.appendChild(n), n.addEventListener("click", function(e) {
                        domClosest("article_full_view__caption_inner", e.target) || T()
                    }), n.addEventListener("mousewheel", T), p = n;
    ...
    function j(e, t, r) {
        (toggleClass(C, "article_full_view__nav_hidden", 0 == r), toggleClass(P, "article_full_view__nav_hidden", r == t.length - 1), t.length > 1) && (geByClass1("article_full_view__counter_text", E).innerHTML = getLang("global_article_carousel_counter").replace("{counter}", r + 1).replace("{total}", t.length));
        O = r;
    ...
    window.initArticle = v, window.deinitArticle = m, window.updateArticle = y, window.articleCloseImageFullSize = T
}, , function(e, t, r) {
    "use strict";
    r.r(t);
    var i = r(20),
            ...
            if (!this.isLocked()) {
                ...
                this._els.canvas.addEventListener("keydown", function(t) {
                    ...
                    if (P) {
                        ...
                        if (a != re && a != J || !e._resizeTooltip.isShown() || e._resizeTooltip.hide(), a == ie && C && 0 == T) return Object(l.focusEl)(e._getParagraphElByIndex(1)), cancelEvent(t);
                        ...
                        var M = a == Q && t.altKey,
                            ...
                            H = !!(R || Object(_.gpeByTag)("pre", P.endContainer) || P.startContainer.nodeType == Node.ELEMENT_NODE && "PRE" == P.startContainer.tagName);
                        if (M) {
                            ...
                            if (!H && inArray(D, [d.ParagraphType.Text, d.ParagraphType.NumericList, d.ParagraphType.BulletList])) {
                                ...
                                var z = Object(_.gpeByTag)("code", P.startContainer) || Object(_.gpeByTag)("code", P.endContainer);
                                if (z) {
                                    e._saveCursorMarker();
                                    var F = L("<span></span>");
                        ...
                        if (a == re) {
                            ...
                            if (I)
                                if (!(ve._object.isCaptionFocused() && !!ve.lines[0].text)) {
                                    var Pe = e._getParagraphElByIndex(T),
                            ...
                            ye && Object(l.isParagraphEmpty)(ve) && inArray(ye.type, [d.ParagraphType.Header2, d.ParagraphType.Header3]) && (ve.type = ye.type, e._setParagraphDirty(T), e._redraw()), e._setAllParagraphsDirty(), (m.msie && 0 == P.startOffset && 0 == T || v) && setTimeout(function() {
            ...
            this._els.canvas.addEventListener("mousedown", function(o) {
                ...
                if (Object(l.isObjectParagraph)(v)) {
                    ...
                    window.addEventListener("mousemove", t = function(t) {
                        ...
                        !1 !== a && f && (e._ps.splice(f, 1), Object(l.hasSeparator)(v) && (e._ps[f].sep = 1, delete v.sep), e._ps.splice(a + 1, 0, v), e._redraw(!0, !0), e.saveUndoStateAndDraft(), e._resizeTooltip.isShown() && e._resizeTooltip.hide()), d()
                    })
                }
            })
    ...
    var W, Y = i((W = null, [function(e) {
        ...
        $ = Y[0],
        V = Y[1];
    window.isRetina = z, window.extractUrls = H, window.serializeForm = D, window.addTemplates = N, window.getTemplate = M, window.rand = s, window.irand = c, window.isUndefined = l, window.isFunction = d, window.isArray = u, window.isString = p, window.isObject = h, window.isEmpty = f, window.vkNow = _, window.vkImage = g, window.trim = v, window.stripHTML = y, window.escapeRE = m, window.intval = b, window.floatval = w, window.positive = P, window.isNumeric = C, window.winToUtf = E, window.replaceEntities = O, window.clean = j, window.unclean = T, window.each = x, window.indexOf = S, window.inArray = I, window.clone = k, window.arrayKeyDiff = L, window.extend = A, window.vkLocal = a, window.lTimeout = o, window.getCaretCharacterOffsetWithin = U, window.formatCount = F, window.encodeHtml = $, window.decodeHtml = V
}, function(e, t, r) {
    "use strict";
