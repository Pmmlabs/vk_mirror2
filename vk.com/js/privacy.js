...
window.customBoxDefaults = {
  who_can_see_title: 'Кому показывать?',
  who_cannot_see_title: 'Кому не показывать?',
  show_to: 'Показывать {friends}.'
};
...
window.showCustomBox = function(options) {
  ...
  if(!customMB[key]) {
    ...
    '<div style="padding: 5px 0"><input type="hidden" id="minus_friends_list"/></div></div>');
      ...
      customMB[key].plus_flist = new Selector(ge('plus_friends_list'), friendsPrivacyList, {defaultItems:def, width:350, placeholder:'Введите имя друга или название списка друзей', onChange:function(v) {
        if(v==='')return;
        var changed = false;
        v = v.split(',');
        var items = [];
      ...
      }, selectedItems:plus, introText:'введите имена друзей, которым Вы хотите разрешить доступ'});
      customMB[key].minus_flist = new Selector(ge('minus_friends_list'), friendsPrivacyList, {defaultItems:def, width:350, placeholder:'Введите имя друга или название списка друзей', onChange:function(v){
      if(v==='')return;
        var changed = false;
        v = v.split(',');
        var items = [];
      ...
      }, selectedItems:minus, introText:'введите имена друзей, которым Вы хотите запретить доступ'});
    };
