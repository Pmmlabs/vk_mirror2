...
function FriendsFilter(dataObj, input, options) {
 ...
 var indexData = [],
     dataObj = dataObj.friends ? dataObj : {friends: dataObj},
     self = this,
     filteredData,
     options = extend({
       pages: true,
 ...
 function initIndex() {
  ...
  for (var i in dataObj.friends) {
    var obj = dataObj.friends[i], addedChars = [],
 ...
 this.setData = function(data) {
   ...
   dataObj = data.friends ? data : {friends: data};
   initIndex();
 }
 ...
 this.showFriends = function(query, offset) {
  ...
  if (options.pages) {
    var matchesLength = matches.length, pageSize = options.pageSize;
    ...
    if (selected_filter == 'requests') {
      ...
      if (need_load_common.length > 0) {
        Ajax.Post({
         ...
         query: {act: 'get_user_commons', uids: need_load_common.join(',')},
         onDone: function(ajaxObj, responseText) {
    ...
    if (matchesLength > pageSize){
      ...
      var pagesCount = Math.ceil(matchesLength / pageSize), minPage = 1, maxPage = pagesCount, page = Math.floor(offset / pageSize) + 1;
      if (pagesCount > 5) {
 ...
 addEvent(input, 'keydown keypress', function(e) {
   clearTimeout(t);
   t = setTimeout(function(){self.showFriends(input.active?input.value:'')}, 100);
 });
};

function PrivacyBox(options) {
  var privacyBox, saveListBox,
      self = this, tabListSelected = 0, saving_list = false, privacyItem = '', btnSelectAll,
      friendsFilter, friendsIndex, friendsData, filterQuery, filteredItems, selectedFriends, selectedCount = 0,
      options = extend({saveList: true, saveListMinUsers: 3}, options);
  function initBox() {
    privacyBox = new MessageBox({title: getLang('friends_privacy_title'), width: '480px', bodyStyle: 'padding:0px', onHide: options.onHide});
    privacyBox.addButton({
      onClick: function(){privacyBox.hide(200)},
      style:'button_no',
    ...
    addEvent('privacy_box_list_tab', 'click', function(){ t_filter_on(this, privacy_box_list_tabs); onListTabChange(false); });
    addEvent('privacy_box_list_selected_tab', 'click', function(){ t_filter_on(this, privacy_box_list_tabs); onListTabChange(true); });
    btnSelectAll = ge('btn_select_all');
    addEvent(btnSelectAll, 'click', selectAll);

    saveListBox = new MessageBox({title: getLang('friends_create_list_title'),bodyStyle: '',width:'300px'});
    ...
    addEvent('privacy_box_flist_name', 'keydown', function(e) {if (e.keyCode == KEY.RETURN) saveFriendsList.call(this, e);});
  }
  ...
  function showFriendsFilter(offset) {
    ...
    if (!offset) {
      ...
      setTimeout(function(){ showFriendsFilter(12); },0);
    } else {
      flist.innerHTML += html;
    }
  }
  ...
  function saveFriendsList() {
    ...
    Ajax.Post({
      url: 'friends_ajax.php',
      query: {act: 'save_list', name: listName, friends: friendsIds.join(','), hash: friendsData.hash},
      onDone: function(ajaxObj, responseText) {
        var result = eval('(' + responseText + ')');
        if (result.error) {
          alert(result.error);
  ...
  this.editPrivacy = function(item, selectedIds) {
    ...
    var flookup = ge('privacy_box_friend_list_lookup'), flname = ge('privacy_box_flist_name');
