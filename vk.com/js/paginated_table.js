...
(function() {
    ...
    var getDiff = function(setNew) {
        if (lastStamp === undefined) return 0;
        var curStamp = new Date().getTime();
        var ret = curStamp - lastStamp;
        if (setNew) lastStamp = curStamp;
        return ret;
    };
    var alertDiff = function(setNew) {
        alert(getDiff(setNew));
    };
    ...
    var initCb = function(elem, rown) {
        ...
        if (rown == '') {
            var onClick = function() {
                var v = this;
                while (v.tableObj === undefined) v = v.parentNode;
                return v.tableObj.toggleCheckbox(this);
            };
    ...
    var PaginatedTableControl = function(container, options) {
        ...
        for (var i = 0; i < sizes.length; i++) {
            switch (sizes[i]) {
                case 20:
                    showSizeValues.push([i, getLang('ads_paginated_table_show_20')]);
                    break;
                case 50:
                    showSizeValues.push([i, getLang('ads_paginated_table_show_50')]);
                    break;
            }
        }
        ...
        if (this.filterDdContainer) {
            ...
            this.filterDd = new DropdownMenu(_t.parent.options.filter.types || [
                [0, 'no filter']
            ], filterDdOptions);
        } else {
    ...
    PaginatedTableControl.prototype.updateFilter = function(filter, index) {
        ...
        if (!filter || !filter.enabled) {
            ...
            this.filterDd.setOptions({
                title: filter.types[index][1]
            });
            for (var node = this.filterDdContainer.parentNode; node && node.nodeName != 'NOBR'; node = node.parentNode);
            show(node);
    ...
    window.PaginatedTable = function(container, options, content) {
        ...
        var innerTableContainer = ce('div', {
            className: 'paginated_table'
        });
        _t.table = ce('table', {
            className: _t.options.tableClassNames['table']
        });
    ...
    window.PaginatedTable.prototype.applyData = function() {
        ...
        if (this.options.paginationEnabled && (this.options.representation.ALWAYS_SHOW_SHOWSIZE_LIST || _t.tableSize > 20)) {
            ...
            if (false && _t.tableSize <= 20) {
                ...
                if (!hidemask) {
                    hidemask = ce('div', {
                        id: 'paginated_table_' + _t.globalNum + '_show_size_hide_mask'
                    }, {
                        position: 'absolute',
                        backgroundColor: '#000',
                        opacity: '0.03',
    ...
    window.PaginatedTable.prototype.goToPage = function(pagen, noHashUpdate) {
        var from = (pagen - 1) * this.showSize,
            to = Math.min(from + this.showSize - 1, this.tableSize - 1);
        this.curPage = pagen;
    ...
    window.PaginatedTable.prototype.sortData = function(keyN, desc, length) {
        ...
        if (keyN == -1) {
            ...
            cmp = function(a, b) {
                var oa = _t.content.data[a][sortCol],
                    ob = _t.content.data[b][sortCol];
    ...
    window.PaginatedTable.prototype._constructTable = function(visible) {
        ...
        var errRow = ce('tr', {
            className: _t.options.tableClassNames['emptyRow']
        });
        if (browser.msie && parseFloat(browser.version) < 9.0) {
            cell = ce('<td colspan="' + (content.header ? content.header.length + (_t.options.checkboxPickingEnabled & !_t.options.representation.HIDE_CHECKBOX_ON_EMPTY) : 1) + '">', {
                innerHTML: (_t.content.emptyRowText !== undefined ? _t.content.emptyRowText : getLang('ads_paginated_table_no_data'))
            });
        } else {
            cell = ce('td', {
                colSpan: content.header ? content.header.length + (_t.options.checkboxPickingEnabled & !_t.options.representation.HIDE_CHECKBOX_ON_EMPTY) : 1,
                innerHTML: (_t.content.emptyRowText !== undefined ? _t.content.emptyRowText : getLang('ads_paginated_table_no_data'))
            });
        }
    ...
    window.PaginatedTable.prototype._constructHeader = function() {
        ...
        var headerRow = ce('tr', {
            className: _t.options.tableClassNames['header']
        });
        ...
        if (content.data && content.data.length && this.options.checkboxPickingEnabled) {
            ...
            n++;
            curCell = ce('th', {
                className: getCellClass(_t, 0, n, 'cb_cell'),
                innerHTML: '<a id="total_cb_' + _t.globalNum + '">&lrm;</a>'
            });
        ...
        for (i = 0; i < content.header.length; i++) {
            curCell = ce('th', {
                className: getCellClass(_t, i + iadd, n),
                innerHTML: '<div><span class="table_header_upper_span">' + content.header[i] + '</span></div>'
            });
    ...
    window.PaginatedTable.prototype._constructFooter = function() {
        ...
        var footerRow = ce('tr', {
            className: _t.options.tableClassNames['footer']
        });
        ...
        if (this.options.checkboxPickingEnabled) {
            ...
            if (this.options.representation.SHOW_CHECKBOX_IN_FOOTER) {
                curCell = ce('td', {
                    className: getCellClass(_t, 0, n, 'cb_cell'),
                    innerHTML: '<a id="total_ft_cb_' + _t.globalNum + '"></a>'
                });
                initCb(curCell.firstChild, undefined);
        ...
        for (var i = 0; i < content.footer.length; i++) {
            ...
            footerRow.appendChild(ce('td', {
                className: getCellClass(_t, i + iadd, n),
                innerHTML: text
            }, (_t.options.noBrEnabled ? {
                whiteSpace: 'nowrap'
            } : {})));
        }
    ...
    window.PaginatedTable.prototype._constructRow = function(i) {
        ...
        if (_t.options.checkboxPickingEnabled) {
            ...
            curCell = ce('td', {
                className: getCellClass(_t, 0, n, 'cb_cell'),
                innerHTML: '<a id="cb_row_' + i + '_' + _t.globalNum + '">&lrm;</a>'
            });
        ...
        if (browser.mozilla || browser.msie) {
            curRow = ceRowHTML({
                className: _t.options.tableClassNames['row'],
                innerHTML: curHTML.join('')
            });
        } else {
            curRow = ce('tr', {
                className: _t.options.tableClassNames['row'],
    ...
    window.PaginatedTable.prototype.toggleCheckbox = function(elem, rown, forceVal, isChained) {
        ...
        if (rown == undefined) {
            ...
            var t1 = ge('total_cb_' + this.globalNum),
                t2 = ge('total_ft_cb_' + this.globalNum);
...
try {
    stManager.done('paginated_table.js');
} catch (e) {}