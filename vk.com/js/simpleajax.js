...
function add_graffiti_post_type(to_id, label) {
    wall_post_types.push({
        ...
        onClick: function(type) {
            return drawGraffiti(to_id, type);
        }
    });
}
...
function add_video_post_type(to_id, label) {
    ...
    wall_post_types.push({
        ...
        label: label,
        onClick: function(type) {
            return chooseVideo(to_id, url, type);
        }
    });
}
...
function add_photo_post_type(to_id, label) {
    ...
    wall_post_types.push({
        ...
        label: label,
        onClick: function(type) {
            return choosePhoto(to_id, url, type);
        }
    });
}
...
function add_audio_post_type(to_id, label) {
    ...
    wall_post_types.push({
        ...
        label: label,
        onClick: function(type) {
            return chooseAudio(to_id, url, type);
        }
    });
}
...
function add_app_post_type(to_id, app_id, name, icon) {
    ...
    wall_post_types.push({
        ...
        label: name,
        onClick: function(type) {
            return chooseApp(app_id, to_id, url, type);
        }
    });
}
...
onDomReady(function() {
    if (wall_post_types.length) {
        ...
        if (wall_menu) {
            addEvent(ge('add_wall_menu'), 'mouseover', function() {
                show_add_wall_media();
            });
            addEvent(ge('add_wall_menu'), 'mouseout', function() {
                hide_add_wall_media();
            });
        }
        if (status_menu) {
            addEvent(ge('add_wall_menu_status'), 'mouseover', function() {
                show_add_wall_media('status');
            });
            addEvent(ge('add_wall_menu_status'), 'mouseout', function() {
                hide_add_wall_media(false, 'status');
            });
        }
        ...
        addEvent(ge('add_wall_header'), 'click', function() {
            hide_add_wall_media(true);
        });
        if (status_menu) addEvent(ge('add_wall_header_status'), 'click', function() {
            hide_add_wall_media(true, 'status');
        });
...
function showGraffiti(grid, owner, elem) {
    ...
    graffitiBox.setOptions({
        title: ge('graffiti_title' + owner).value,
        fullPageLink: elem.href
    });
...
function showWallPhoto(phid, owner, elem) {
    ...
    setTimeout(function() {
        ...
        wallPhotoBox.setOptions({
            width: (intval(width) > 410) ? (width + 20) : 430,
            title: ge('wallphoto_title' + owner).value,
            fullPageLink: elem.href
        });
...
function drawGraffiti(to_id) {
    ...
    if (!drawBox) {
        drawBox = new MessageBox({
            title: ge('draw_graffiti_title').value,
            width: 610,
            bodyStyle: 'padding: 0px',
            type: 'POPUP',
            hideOnClick: false,
            progress: 'graffiti_draw_progress',
            onLoad: function() {
                if (ge('graffitiDraw')) {
                    drawBox.setOptions({
    ...
    drawBox.loadContent('graffiti.php', {
        act: 'a_draw_box',
        to_id: to_id
    }, true).show();
    return false;
    ...
    var options = {
        ...
        onCaptchaHide: function(done) {
            ...
            box.setOptions({
                ...
                onHide: function() {
                    if (!drawWarningBox) {
                        ...
                        drawWarningBox.addButton({
                            label: global_continue,
                            onClick: function() {
                                drawNeedConfirm = false;
                                drawWarningBox.setOptions({
                                    onHide: null
                                });
                                drawWarningBox.hide();
                            }
            ...
            box.removeButtons().addButton({
                ...
                label: window.lang_graffiti_send,
                onClick: function() {
            ...
            Ajax.Post({
                ...
                onDone: function(obj, text) {
                    if (text && ge('uploadedGraffiti')) {
                        ...
                        box.content(global_unknown_error).removeButtons().addButton({
                            label: global_cancel,
                            style: 'button_no',
                            onClick: function() {
                                box.hide();
...
function showApp(app_id, to_id, from_id, loc, href, photo) {
    ...
    if (!showAppBox) {
        ...
        showAppBox = new MessageBox({
            title: ge('show_app_title').value,
            width: 609,
            closeButton: true,
            fullPageLink: href,
    ...
    showAppBox.setOptions({
        fullPageLink: href
    });
    showAppBox.removeButtons().addButton({
        label: global_close,
        onClick: showAppBox.hide
    }).addControlsText('<a href="' + href + '">' + ge('go_to_app_label').value + '</a>');
    showAppBox.loadContent('apps.php', {
        act: 'a_show_app_box',
        to_id: to_id,
        from_id: from_id,
        loc: loc,
...
function chooseApp(app_id, to_id, href) {
    ...
    if (!chooseAppBox) {
        attachScript('appsFlash', base_domain + 'js/apps_flash.js?35');
        chooseAppBox = new MessageBox({
            title: ge('choose_app_title').value,
            width: 609,
            progress: 'choose_app_progress',
            closeButton: true,
            fullPageLink: href,
            bodyStyle: 'height: 412px; padding: 0px;',
            onHideAttempt: function() {
    ...
    chooseAppBox.loadContent('apps.php', {
        act: 'a_choose_app_box',
        to_id: to_id,
        aid: app_id
    }, true, 'height: 412px').show();
    return false;
...
function chooseAudio(to_id, href) {
    ...
    href = href.href ? href.href : href;
    if (!chooseAudioBox) {
        chooseAudioBox = new MessageBox({
            title: ge('choose_audio_title').value,
            width: 610,
    ...
    chooseAudioBox.loadContent('audio.php', {
...
function chooseVideo(to_id, href) {
    ...
    href = href.href ? href.href : href;
    if (!chooseVideoBox) {
        chooseVideoBox = new MessageBox({
            title: ge('choose_video_title').value,
            width: 610,
    ...
    chooseVideoBox.loadContent('video.php', {
...
function choosePhoto(to_id, href, type) {
    ...
    if (!choosePhotoBox) {
        choosePhotoBox = new MessageBox({
            title: ge('choose_photo_title').value,
            width: 610,
    ...
    choosePhotoBox.loadContent('photos.php', {
        act: 'a_choose_photo_box',
        to_id: to_id,
        scrollbar_width: window.sbWidth() /*, status_add:type=='status'?1:0*/
    }, true, 'height: 412px').show();
    return false;
}
...
function showPlaceBox(el, id, tab) {
    ...
    return showBox('placeBox', 'places.php', {
        act: 'a_get_place_box',
        id: id
    }, true, true, options, files);
}