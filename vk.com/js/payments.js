...
function showPostponedBox(hash) {
    if (!postponedMB) {
        postponedMB = new AlertBox('��������� �������', '', function() {
            return onSendPostponedCode(hash);
        }, {
            width: 350,
            boxType: 'CONFIRM',
            yes: getLang('captcha_send'),
            no: getLang('global_close')
        });
    }
    postponedMB.loadContent('payments.php', {
        act: 'a_postponed_form'
    }, 1).show();
}

function postponedSendAgain(phid, hash) {
    Ajax.Send('payments.php', {
        act: 'a_postponed_resend',
        hash: hash,
        phid: phid
    }, {
        onSuccess: function(obj, text) {
