            ...
            case 'lite.js':
                extend(window, {
                    ...
                    showBox: Widgets.showBox({
                        ...
                        'al_wall.php': {
                            'canvas_draw_box': true
                        },
                        'al_im.php': {
                            'stickers_store': true,
                            'sticker_preview': true
                        },
                        'al_audio.php': {
                            'a_choose_audio_box': {
                                stat: ['audioplayer.css', 'audioplayer.js']
                            }
                        }
                    }),
            ...
            case 'ui_media_selector.js':
                window.MediaSelector = function(lnk, previewId, mediaTypes, opts) {
                    ...
                    each(mediaTypes || [], function(i, v) {
                        ...
                        switch (type) {
                            case 'graffiti':
                                handler = showBox.pbind('al_wall.php', {
                                    to_id: cur.postTo,
                                    act: 'canvas_draw_box',
                                    flash: browser.flash
                                }, {
                                ...
                                break;
                            case 'photo':
                                handler = showBox.pbind('al_photos.php', {
                                    to_id: cur.postTo,
                                    act: 'choose_photo',
                                    max_files: opts.limit || 10
                                }, {
                                    cache: 1,
                                ...
                                break;
                            case 'video':
                                handler = showBox.pbind('al_video.php', {
                                    to_id: cur.postTo,
                                ...
                                break;
                            case 'audio':
                                handler = showBox.pbind('al_audio.php', {
                                    to_id: cur.postTo,
                        ...
                        chooseMedia: function(type, media, data, url, noboxhide, isGraffiti) {
                            ...
                            switch (type) {
                                ...
                                case 'video':
                                    ...
                                    if (editable) {
                                        if (!data.editable) return false;
                                        if (!opts.nocl) data.editable.click = showVideo.pbind(media, false, {
                                            queue: 1
                                        });
                                    }
                                ...
                                case 'share':
                                    ...
                                    addMedia.shareData = extend(addMedia.shareData || {}, data, {
                                        imagesStyles: ['']
                                    });
                                    toEl = ldocsEl;
                                    break;
                                ...
                                case 'album':
                                    ...
                                    if (editable) {
                                        ...
                                        extend(data.editable, {
                                            title: data.title,
                                            size: data.count,
                                            click: opts.nocl ? false : nav.change.pbind({
                                                z: 'album' + media
                                            })
                                        });
                                    }
                    ...
                    ' + (thumb ? '<div class="page_album_thumb_wrap"><img class="page_album_thumb" src="' + clean(thumb) + '"/></div>' : '') + '\
                      <div class="page_album_title">\
                            ...
                            if (multi) {
                                ...
                                if (editable && toPics === 1) {
                                    ...
                                    if (!domLC(toEl) || !hasClass(domLC(toEl), 'editable_thumbs')) {
                                        toEl = toEl.appendChild(ce('div', {
                                            id: 'thumbs_edit' + lnkId,
                                    ...
                                    stManager.add(['thumbs_edit.css', 'thumbs_edit.js'], function() {
                                        ...
                                        if (domFC(toEl)) {
                                            ...
                                            ThumbsEdit.init(toEl, [teMed], {
                                                width: opts.teWidth,
                                                height: opts.teHeight,
                                                force: true,
                                                onMove: opts.onAddMediaChange,
                                                onUpdate: opts.onChangedSize
                                            });
                                        } else {
                                            ThumbsEdit.init(toEl, [teMed], {
                                                onMove: opts.onAddMediaChange,
                        ...
                        unchooseMedia: function(ind) {
                            ...
                            if (multi) {
                                ...
                                if ((ind == 1) && addMedia.postponePreview) {
                                    ...
                                    each([addMedia.sharePreview, addMedia.pollPreview], function() {
                                        re(this);
                                    });
                        ...
                        getMedias: function() {
                            if (multi) {
                                var edited = window.ThumbsEdit ? ThumbsEdit.getMedias('thumbs_edit' + lnkId) : [],
                                    already = {};
                                var chosen = addMedia.chosenMedias || [],
                                    result = [],
                                    check = function(id, ck, cv) {
                                        if (cv[0] + cv[1] == id) {
                                            result.push(cv);
                                            already[id] = true;
                                            return false;
                    ...
                    <div class="page_attach_progress_wrap" style="margin-top: 3px; margin-bottom: 4px;"> \
                      <div id="upload' + ind + '_progress" class="page_attach_progress ui_progress"> \
                        ...
                        createPoll: function(data) {
                            var h = data.question ? '' : '1px',
                                html = [],
                                ans;
                            var incCl = data[4 + (10 - 1) * 2] ? 'disabled' : '',
                                decCl = data[4 + 2 * 2] ? '' : 'disabled';
                            addMedia.pollPreview = pdocsEl.appendChild(ce('div', {
                  ...
                  <div class="medadd_c_polladd fakeinput dark" onclick="cur.addMedia[' + lnkId + '].incPoll()">' + data.lang.i + '</div>\
                        ...
                        keyPoll: function(el, ev) {
                            ...
                            if (ev && (ev.keyCode == 10 || ev.keyCode == 13 || ev.keyCode == 9)) {
                                var q = hasClass(el, 'medadd_c_pollq'),
                                    s = ev.shiftKey;
                                if (s && q) return;
                        ...
                        pollData: function(silentCheck) {
                            var answers = ge('create_poll_answers' + lnkId),
                                q = trim(val('create_poll_question' + lnkId)),
                                a;
                            var result = {
                                    media: q,
                                    anonymous: isChecked('create_poll_anonymous' + lnkId)
                                },
                                ind = 0,
                                found = false;
                        ...
                        addPreview: function(progress) {
                            return (addMedia.sharePreview = ldocsEl.appendChild(ce('div', {
                                className: 'medadd_c medadd_c_link',
                                innerHTML: '<div class="medadd_c_linkcon"><div></div>' + (progress ? '<div class="progress medadd_c_linkprg"></div>' : '') + '</div>'
                            })));
                        },
                        ...
                        showPreview: function(fast) {
                            ...
                            if (fast) {
                                ...
                                cur.preventShareAnim = animate(domFC(prev), {
                                    height: height
                                }, 200, function() {
                                    clearInterval(cur.animateUpdateInterval);
                                });
                        ...
                        uploadShare: function(callback) {
                            var data = addMedia.shareData,
                                prev = addMedia.sharePreview;
                            var uploadCont = prev.appendChild(ce('div', {
                                    innerHTML: '<iframe class="upload_frame" name="share_upload_iframe' + lnkId + '"></iframe>'
                                })),
                                uploadForm = uploadCont.appendChild(ce('form', {
                                    action: '/share.php',
                                    method: 'post',
                                    target: 'share_upload_iframe' + lnkId
                                })),
                            ...
                            each({
                                ...
                                uploadForm.appendChild(ce('input', {
                                    type: 'hidden',
                                    name: i,
                                    value: v
                                }));
                        ...
                        setupPostpone: function(data, export_row) {
                            ...
                            var ed = (cur.editingPost && domPN(toEl).id == 'wpe_media_preview'),
                                h = (ed || !multi) ? '' : '1px',
                                addedhtml = false;
                ...
                <div class="fl_l"><input type="hidden" id="postpone_time' + lnkId + '"/></div></div>';
                            ...
                            addMedia.postponePreview = toEl.appendChild(ce('div', {
                                className: 'medadd_c medadd_c_timer clear_fix' + (addedhtml ? ' medadd_c_nofixed' : ''),
                                innerHTML: html
                            }));
                            ...
                            stManager.add(['ui_controls.css', 'ui_controls.js', 'datepicker.css', 'datepicker.js'], function() {
                                new Datepicker('postpone_date' + lnkId, {
                                    time: 'postpone_time' + lnkId,
                                    width: 155,
                                    noPast: true,
                                    minStep: 1,
                                    onUpdate: opts.onMediaChange
                                });
                                if (!ed && multi) {
                                    animate(addMedia.postponePreview, {
                                        height: 33
                                    }, 200, function() {
                            ...
                            setItems: function(types) {
                                ...
                                each(types, function(i, v) {
                                    ...
                                    if (needHide && (hideItem ? type == hideItem : i == maxShown)) {
                                        var moreWrap = menuNode.appendChild(ce('div', {
...
try {
    stManager.done('api/widgets/al_comments.js');
} catch (e) {}