var MediaPoster = function(postType, target, preview, msgBox, toId, types, isAdmin) {
        ...
        addEvent(ge(id + '_post_type' + i), 'click', function(onClick) {
            ...
            switch (v[0]) {
                case 'graffiti':
                    ...
                    break;
                case 'photo':
                    showPhotoBox(toId, t, url, h, isAdmin);
                    break;
                case 'video':
                ...
                case 'audio':
                    ...
                    break;
                case 'app':
                    showAppBox(toId, t, appId, url, appHash, v[1], icon);
                    break;
                case 'poll':
    ...
    function postAttach(media, media_id, toId, hash, options, target, preview) {
        ...
        var type = ge('wall_type') ? ge('wall_type').value : 0,
            export_el = ge('export_to_twitter'),
            reply_to;
    ...
    var showGraffitiBox = function(toId, title, post_hash) {
        ...
        boxes['graffiti'].removeButtons().loadContent('graffiti.php', {
            act: 'a_draw_box',
            to_id: toId
        }, true).show();
                    ...
                    box.setOptions({
                        ...
                        onHide: function() {
                            if (!boxes['graffiti_warning']) {
                                var bw = new MessageBox({
                                    title: global_warning
                                });
                                boxes['graffiti_warning'] = bw;
                                bw.content(window.lang_graffiti_cancel_confirm).addButton({
                                    label: global_cancel,
                                    style: 'button_no',
                                    onClick: function() {
                                        bw.hide();
                            ...
                            boxes['graffiti_warning'].setOptions({
                                onHide: function() {
                                    box.show();
                                }
                            }).show();
        ...
        window.doCancelDrawing = function() {
            cancelBox('graffiti');
        }
        return false;
    }
    ...
    var showPhotoBox = function(toId, title, href, post_hash, isAdmin) {
        ...
        if (!b) {
            boxes['photo'] = b = (new TabbedBox('', [], {
                returnHidden: true,
                title: title,
                search: '<div style="padding:4px;">&nbsp;</div>',
                width: 610,
                progress: 'choose_photo_progress',
                closeButton: true,
                fullPageLink: href,
                showOneTab: true,
                bodyStyle: 'height: 374px; padding: 0px;',
                onTab: function(tab, loaded) {
                    if (!loaded) {
                        b.tabLoadContent(tab, 'photos.php', {
    ...
    var showVideoBox = function(toId, title, href) {
        ...
        if (!b) {
            boxes['video'] = b = (new TabbedBox('', [], {
                ...
                bodyStyle: 'height: 374px; padding: 0px;',
                showOneTab: true,
                search: '<div style="padding:7px 4px;">&nbsp;</div>',
                onTab: function(tab, loaded) {
                    if (!loaded) {
                        b.tabLoadContent(tab, 'video.php', {
    ...
    var showAudioBox = function(toId, title, href) {
        ...
        if (!b) {
            boxes['audio'] = b = new TabbedBox('', [], {
                ...
                onTab: function(tab, loaded) {
                    if (!loaded) {
                        b.tabLoadContent(tab, 'audio.php', {
...
function initMentions(el) {
    ...
    var _tmp = function() {
        ...
        if (!el.id.indexOf('status')) {
            ...
            onsubmit = function() {
                postWall(ge('mid').value, window.wall_phash);
            };
        }
        jsDispatcher.include('mentions', function() {
            var mention = new MentionAutocomplete(el, {
                ...
                onSubmit: onsubmit,
                checkLen: !el.id.indexOf('status') ? function(text) {
                    checkStatusLength(window.max_status_length, text, ge('status_warn'), ge('status_note'), ge('add_wall_media_link_status'));
                } : null,
                onValueChange: data(el, 'checkLink')
            });
            addEvent(mention.rtaEl, 'focus', function() {
                if (!el.id.indexOf('status')) {
                    if (ge('group')) showGroupStatusBox();
                    else showStatusBox();
                } else showReplyBox();
            });
...
function likePreview(likeButton, params) {
    ...
    showTT(likeButton, LikeWallTooltip, 'like.php', extend({
        'act': 'a_get_stats'
    }, params));
}
...
function showReplies(to_id, msg_id, count, hl, offset) {
    ...
    var r = ge('replies' + to_id + '_' + msg_id),
        h = geByClass('replies_header_wrap', r)[0];
