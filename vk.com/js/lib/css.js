var css = {
    ...
    addClass: function(obj, name) {
        if (!(new RegExp('\\b' + name + '\\b')).test(obj.className)) obj.className = (obj.className ? obj.className + ' ' : '') + name;
        return obj;
    },
    removeClass: function(obj, name) {
        var re = new RegExp(name + ' ?| ?' + name);
        while (re.test(obj.className)) obj.className = obj.className.replace(re, '');
        return obj;
    }
}