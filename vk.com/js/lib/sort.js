...
sortable = {
    ...
    updatePos: function(elem, event) {
        setStyle(elem, {
            left: event.pageX - this.xOff,
            top: event.pageY - this.yOff
        });
    },
    ...
    mousedown: function(event) {
        ...
        this.pushStyles(elem, {
            width: getStyle(elem, 'width'),
            zIndex: "10000",
        ...
        this.sortHelper = ce("div", {
            innerHTML: '&nbsp; ',
            className: 'sort_blank'
        }, {
            height: s[1] - 2,
            width: s[0] - 2,
            cssFloat: getStyle(elem, 'cssFloat'),
            styleFloat: getStyle(elem, 'styleFloat')
        });
    ...
    mousemove: function(event, wheel) {
        ...
        for (var i = 0;; i += dir) {
            var itm = nodes[i],
                xy = [itm.offsetLeft, itm.offsetTop];
...
var PagedList = function(container, data, options) {
    ...
    this.getPage = function(page, search, force) {
        ...
        setStyle(ge(container), {
            height: (page && !options.autoheight) ? h : 'auto'
        });
