...
function YCustomZoomControl(offices) {
    this.onAddToMap = function(map, position) {
        ...
        this.container.find('.ymap_zoom_btn').bind('click', function() {
            map.zoomBy(parseInt(this.getAttribute('data-z')), {
                smooth: true
            });
        });
            ...
            vkMaps.load(api, (function() {
                ...
                YMaps.Events.observe(yandexMap, yandexMap.Events.Click, (function(map, mouseEvent) {
                    ...
                    this._curClickTO = setTimeout(function() {
                        _this.click.fire({
                            'location': new vkMaps.LatLonPoint(lat, lon)
                        });
                    }, 250);
                }).bind(this));
                ...
                YMaps.Events.observe(yandexMap, yandexMap.Events.DblClick, (function(map, mouseEvent) {
                    ...
                    map.setZoom(map.getZoom() + 1, {
                        position: mouseEvent.getCoordPoint(),
                        smooth: true
                    });
                }).bind(this));
    ...
    VKMap: {
        init: function(element, api) {
            vkMaps.load(api, (function() {
                ...
                yandexMap.events.add('click', (function(ev) {
                    ...
                    this.click.fire({
                        'location': new vkMaps.LatLonPoint(coords[0], coords[1])
                    });
                }).bind(this));
        ...
        addControls: function(args) {
            ...
            if (args.overview) {
                ...
                this.controls.unshift(new ymaps.control.MiniMap({
        ...
        geocode: function(address) {
            ...
            if (address.lat && address.lon) {
                ...
                q = {
                    ...
                    lat: address.lat,
                    lon: address.lon,
                    request_id: address.lat + ',' + address.lon
                };
            ...
            vkMaps.load(api, (function() {
                ...
                if (element) {
                    ...
                    google.maps.event.addListener(map, 'click', (function(location) {
                        this.click.fire({
                            'location': new vkMaps.LatLonPoint(location.latLng.lat(), location.latLng.lng())
                        });
                    }).bind(this));
    ...
    VKMap: {
        init: function(element, api) {
            vkMaps.load(api, (function() {
                ...
                var mapboxMap = this.maps[api] = L.mapbox.map(element, false, {
                    zoomControl: false
                });
                mapboxMap.on('click', (function(e) {
                    this.click.fire({
                        'location': new vkMaps.LatLonPoint(e.latlng.lat, e.latlng.lng)
                    });
                }).bind(this));
                mapboxMap.on('zoomend', (function(e) {
                    this.changeZoom.fire();
                }).bind(this));
                this.tileLayer = L.mapbox.tileLayer(vkMaps.mapboxApiId, {
                    detectRetina: true,
                    retinaVersion: vkMaps.mapboxApiId_2x
                }).addTo(mapboxMap);
