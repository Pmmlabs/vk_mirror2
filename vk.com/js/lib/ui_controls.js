...
function addTootlip(t, e) {
    ...
    if (!data(t, "tt")) {
        var n = (new NTooltip(t, e), t.getAttribute("id") || ""),
            l = se('<div class="ntt" id="ntt_' + n + '"></div>');
        data(t, "tt", l), addEvent(t, "mouseenter", s), addEvent(t, "mouseleave", o)
    }
}
...
extend(UiControl.prototype, {
    ...
    initDOM: function(t, e) {
        ...
        this.container = ce("div", {
            ...
            onHide: function() {
                _ui.sel(!1), i.highlightInput(!1), isFunction(i.options.onHide) && i.options.onHide()
            }
        })
    },
    initEvents: function() {
        var t = this;
        this.options.dropdown && this.initDropdownEvents(), addEvent(this.resultList, "wheel", function(e) {
            return e.deltaY > 0 && t.resultList.scrollTop + t.resultList.offsetHeight >= t.resultList.childNodes[0].offsetHeight ? cancelEvent(e) : t.resultList.scrollTop <= 0 && e.deltaY < 0 ? cancelEvent(e) : void 0
        }), this.options.limitedListHeight && (addEvent(this.selectedItemsContainer, "scroll", this.updateSelectedItemsScroll.bind(this)), this.updateSelectedItemsScroll({
            target: this.selectedItemsContainer
        }), addEvent(this.selectedItemsContainer, "wheel", function(t) {
        ...
        var e = browser.opera || browser.mozilla ? "keypress" : "keydown",
            i = browser.opera ? "keypress" : "keydown";
        this.onEvent = function(s) {
            if ("mousedown" == s.type) {
                for (var o = !0, n = s.target; n && n != n.parentNode;) {
                    if (n == t.container) {
                        o = !1;
                        break
                    }
                ...
                o && (t.select.hide(), t.deselectTokens())
            }
            s.type == e && t.handleKeyboardEventOutside(s), s.type == i && t.select.handleKeyEvent(s), "keypress" == s.type && t.select._doQuickSearch(s)
        ...
        if (e.options.enableCustom && (e.input.value.length || t && t.length)) {
            ...
            if (0 == e._selectedItems.length || e.options.multiCustom) {
                ...
                if (e.options.multiCustom) {
                    ...
                    for (var n = 0, l = this._selectedItems.length; l > n; n++)
    ...
    _selectItem: function(t, e, i) {
        if (null !== t && void 0 !== t) {
            ...
            }.bind(this), 100) : this.options.noBlur || this.input.blur(), e && (this.options.multiselect && isFunction(this.options.onTagAdd) && this.options.onTagAdd(s, this.resultField.value), isFunction(this.options.onChange) && this.options.onChange(this.resultField.value, s))
        }
    },
    addTagData: function(t) {
        if (t && !(t.length < 2)) {
            ...
            var h = s.firstChild.nextSibling.firstChild;
            ...
            for (var r = s.firstChild.nextSibling.firstChild.nextSibling, a = r.innerHTML; s.offsetWidth > n && a.length > 3;) a = a.substr(0, a.length - 2), r.innerHTML = a + "...";
    ...
    needsReverse: function() {
        var t, e = window.scrollGetY ? scrollGetY() : getScroll()[1],
            ...
            h = this.resultList && this.resultList.firstChild;
        if (h && h.firstChild) {
            var r = getStyle(this.resultList, "display"),
                a = getStyle(this.resultList, "visibility");
            setStyle(this.resultList, {
                visibility: "hidden",
                display: "block"
    ...
    _showSelectList: function(t, e, i) {
        return this.currenDataItems = e, this.currenDataText = t, this.setSelectContent(t, e, i), this.select.hasItems() ? this.options.multiselect || !this._selectedItems.length ? this.select.show() : this.select.show(this._selectedItems[0][0]) : this.select.hide(), !0
    },
    ...
    initDOM: function() {
        this.list = ce("ul"), this.container.appendChild(this.list)
    },
    show: function(t) {
        ...
        }!e && isFunction(this.options.onShow) && this.options.onShow()
    },
    hide: function() {
    ...
    initDOM: function(t) {
        this.minHeight = intval(this.options.minHeight) || intval(getStyle(t, "height")), this.maxHeight = intval(this.options.height), this.fontSize = intval(getStyle(t, "fontSize"));
        var e = intval(getStyle(t, "width"));
        1 > e && (e = intval(getStyle(t, "width", !1))), this.defaultOptions.padding && (e -= 2 * this.defaultOptions.padding), this.common._autosize_helpers.appendChild(this.helper = ce(this.options.helperTA ? "textarea" : "div", !1, {
            wordWrap: "break-word",
            width: (0 > e ? 0 : e) + "px",
            fontFamily: getStyle(t, "fontFamily"),
            fontSize: this.fontSize + "px",
            lineHeight: getStyle(t, "lineHeight")
        })), this.input.helper = this.helper, setStyle(this.input, "overflow", "hidden")
    },
    initEvents: function() {
    ...
    updateSize: function(t) {
        ...
        if ("keyup" != t.type && (13 != t.keyCode || t.ctrlKey || t.altKey || (s += "\n")), s != i.oldValue) {
            if (i.oldValue = s, i.options.helperTA ? (i.helper.value = s, e = i.helper.scrollHeight + i.fontSize + 4) : (i.helper.innerHTML = trim(replaceChars(s)).replace(/<br>$/, "<br>&nbsp;"), e = getSize(i.helper, !0)[1] + i.fontSize + 4), e = Math.max(e, i.minHeight), i.maxHeight > 0 && e > i.maxHeight) {
    ...
    moveTo: function(t, e) {
        ...
        var l = getSize(this.header)[0],
            h = getSize(this.body)[0];
        l > h && setStyle(this.rows, "width", l - 2 + "px"), h = getSize(this.body)[0];
        var r = document.documentElement.clientWidth,
            a = t + h > r && t + l > h ? -l - 1 + "px" : "auto";
        setStyle(this.body, "right", a)
    },
    ...
    indexItem: function(t, e, i) {
        ...
        for (l += (parseLatin(l) || "") + (parseCyr(l) || ""),
            l = trim(winToUtf(l).toLowerCase()).split(/\s+/), s = 0; s < l.length; s++)
...
window.inlineOnEvent || (window.inlineOnEvent = function(t) {
    ...
    function t(t, i) {
        ...
        var s = h._opts.sublists && h._opts.sublists[t[0]] ? "idd_sublist" : "",
            n = '<div class="idd_item ' + i + " " + e + " " + s + '" id="' + o + "idd_item_" + t[0] + '" data-id="' + t[0] + '" tabindex="0">' + (h._opts.withIcon ? '<div class="idd_item_icon" id="' + t[0] + '"></div>' : "") + '<div class="idd_item_name">' + (h._opts.html ? h._opts.html(t) : t[1]) + "</div></div>";
        return n
    }
    ...
    function i(t) {
        ...
        var s, o, n = {
            ...
            l = t && t.ownerDocument;
        if (l) return s = l.documentElement, void 0 !== typeof t.getBoundingClientRect && (n = t.getBoundingClientRect()), o = i(l), [n.left + o.pageXOffset - s.clientLeft, n.top + o.pageYOffset - s.clientTop]
    }
    ...
    var l = getSize(o),
        h = 0;
    getSize(this._els.popupItems.childNodes[0])[1] > l[1] && (h = sbWidth()), this._opts.checkable && (h += 30);
    ...
    if (clearTimeout(this._hideSubmenuTimeout), t) {
        ...
        if (this._opts.sublists && this._opts.sublists[s]) {
            ...
            var o = this._opts.sublists[s],
                ...
                l = o.withIcon ? "idd_with_icon" : "";
            each(o.items, function(t, i) {
                n += e(i)
            }), n += "</div></div></div>", this._els.currCascade = se(n), this._iddEl.appendChild(this._els.currCascade);
            for (var h = getSize(this._els.popupEl), r = (getXY(this._els.popupEl), getSize(t)), a = getSize(this._els.currCascade), d = 0, c = t; null != (c = c.previousSibling);) d++;
            var u = h[0] + (this._opts.headerLeft || InlineDropdown.IDD_HEADER_CORRECTION_LEFT) - 1;
            setStyle(this._els.currCascade, {
                marginLeft: u - 3,
                marginTop: r[1] * d + 6 - (this.openToUp ? h[1] : 0),
                "z-index": 200,
                width: a[0] + 30
            }), setTimeout(function() {
                ...
                if (e) {
                    var s = !0,
                        ...
                        l = geByClass1("idd_item_name", e).innerHTML;
                    o.onSelect ? s = o.onSelect(n, l) : i._opts.onSelect && (s = i._opts.onSelect(n, l)), s && i._hide()
                }
                cancelEvent(t)
