...
var on_create_group = function() {
    ...
    var params = {
        'act': 'a_create',
        'name': ge('new_name').value,
        'hash': ge('create_hash').value
    };
    ...
    var options = {
        onSuccess: redirect,
        onFail: create_box_show,
        onCaptchaShow: box_hidden,
        onCaptchaHide: create_box_show
    };
    Ajax.postWithCaptcha('/groups.php', params, options);
}
onDomReady(function() {
    if (ge('create_hash')) {
        create_box = new MessageBox({
            title: groups_create_new,
            width: 330
        });
        create_box.addButton({
            label: groups_cancel,
            style: 'button_no',
            onClick: function() {
                create_box.hide();
        ...
        create_box.addButton({
            label: groups_group_create_button,
...
var refinedSearch = function(searchItems, itemsContainer, itemContainerId, itemNameId, summaryRefined, noItemsFoundCont, noItemsFoundContText) {
    ...
    var performLookup = function(input, event) {
        ...
        if (summaryRefined) {
            summaryRefined.innerHTML = '&nbsp;(' + (skipped ? '������ ' : '') + shown + ')';
            show(summaryRefined);
        }
