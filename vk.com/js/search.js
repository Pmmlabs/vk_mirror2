...
function updatePage(ajaxObj, responseText) {
  var show_error = -1, result = {script: ''};
  try {
    result = eval('(' + responseText + ')');

    var str = ''; for (var i in result) { str += ', ' + i; }; debugLog('result=' + str);
    ...
    */
    ...
    if (result.filters) {
      ...
      if (result.q) {
        ...
        if (window.iSearch && iSearch.select && iSearch.select.isVisible()) { triggerEvent(ge('search_input'), 'keyup'); }
      }
      show_error = 8;
...
function postVideoOnWall(hash, to_id, video_id) {
  ...
  var params = {act: 'a_post_wall', hash: hash, to_id: to_id, media:'video', media_id:video_id, redirect:1};
  var stop = function(obj, text) {
    posting = false;
  }
  var options = {onSuccess: callback, onFail: stop, onCaptchaShow: stop, onCaptchaHide: stop};
...
function showPhoto(el, full_id, one_photo, count) {
  ...
  if (!window.showPhotoProgress) {
    window.showPhotoProgress = document.body.appendChild(ce('img', {src: '/images/upload.gif'}, {position: 'absolute', display: 'none', width: 32, height: 8}));
  }
  var img = el.firstChild, xy = getXY(img);
  ...
  Ajax.Send('newsfeed.php', {act: 'a_show_photos', full_code: full_code, full_id: full_id, photo: one_photo, count: count}, {onSuccess: function(o, t) {
    if (!t) {
  ...
  }, onCaptchaShow: hide.pbind(showPhotoProgress), onFail: hide.pbind(showPhotoProgress)});
  return false;
}
...
var iSearch =  {
  init: function (options) {
    ...
    addEvent('search_input', 'keyup focus mouseup', function (e) {
      ...
      var el = ge('search_input'), term = trim(el.getValue ? el.getValue() : el.value);
      self.currentTerm = term;
      var section = ge('section') ? ge('section').value : '', cache_key = section + '#' + term;
