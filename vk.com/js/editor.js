var EditBox = {
    ...
    init: function() {
        ...
        var editContainer = dom.create("div", {
            id: "editContainer"
        });
        var editor = dom.append(editContainer, "div", {
            id: "edit",
            className: "editBox"
        });
        if (!browser.msie) {
            var frame = dom.append(editor, "iframe", {
                id: "editFrame"
            }, {
                width: area.w + "px",
                height: area.h + "px",
            ...
            dom.append(editContainer, "div", {
                id: "editButtonRow"
            });
            form.replaceChild(editContainer, areaEl);
            this.input = dom.append(form, "input", {
                type: "hidden",
                name: area.name,
                id: area.id
            });
            ...
            var onload = function() {
                ...
                doc.close();
                _t.body = doc.body;
                dom.append(_t.body.previousSibling, "link", {
                    type: "text/css",
                    href: "css/editor.css",
                    rel: "stylesheet"
                });
                _t.body.className = "editBody";
                dom.setStyle(_t.body, {
                    padding: "4px"
                });
                _t.doc = doc;
    ...
    initButtons: function() {
        ...
        for (btn in this.buttons) {
            var img = dom.append(ge("editButtonRow"), "img", {
                id: btn + "Btn",
                src: "images/" + btn + ".gif",
                className: "editButton editButtonOff"
            });
            if (this.buttons[btn].sep) {
                dom.setStyle(img, {
                    marginLeft: "4px"
                });
            }
            this.buttons[btn].img = img;
            events.addEvent(img, "click", function(cmd, _img) {
                return function() {
                    _t.btnClick(cmd, _img)
                }
            }(this.buttons[btn].cmd, img));
        }
        ...
        for (ev in evs) {
            events.addEvent(evs[ev], ev, function(event) {
                return function() {
                    _t.editorEvent(event, arguments)
                }
            }(ev));
        }
    },
...
var checkLoad = function() {
    ...
    if (tags && tags[0]) {
        ...
        ta.value = "";
        // image size - 150x8
        progImg = dom.append(document.body, "img", {
            src: "images/progress7.gif"
        }, {
            position: "absolute",
            left: findX(ta) + (ta.offsetWidth - 150) / 2 + "px",
            top: findY(tags[0]) + (ta.offsetHeight - 8) / 2 + "px"
        });
