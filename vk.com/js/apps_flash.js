...
function showAddStatusBox() {
  if (!settingsBox) {
    settingsBox = new MessageBox({title: apps_app_settings, progress: 'settingsProgress'});
  }
  settingsBox.removeButtons();
  settingsBox.addButton({label: box_close, onClick: function() {settingsBox.hide(200);}});
  settingsBox.loadContent('apps.php?act=a_load_settings', {id: appId, mask: settings, main: main ? 1: 0}, true).show();
}
    ...
    if (m == 'wall.post' || m == 'activity.set') {
      showWallPrePost(Api.api_id, params[m == 'wall.post' ? 'message' : 'text'], params['attachment'], params['owner_id'], function(hash, error) {
        if (error) {
          ...
          Api.call(method, extend(params, {method_access: hash}), callback);
        }
      });
      return false;
    }
    ...
    function pass() {
      ...
      var done = function(text) {
        ...
        if (response.error && response.error.error_code == 14) { // Captcha needed
          ...
          if (captcha) window.Ajax._captchaBox.setOptions({onHide: function(){}}).hide();
      ...
      if (vk.al) {
        ajax.plainpost('/api.php', params, {onDone: done, onFail: fail});
      } else {
        Ajax.Post({url: '/api.php', query: params, onDone: function(obj, text) {
          done(text);
        }, onFail: fail});
      }
    }
...
var appLoaded = false, lastLocation = '', hashHandler, appContainer;
...
function extShowSettingsBox(settings) {
  if (settingsTries > 0) {
    ...
    settingsTries--;
    setTimeout(function(){settingsTries++;}, 3000);
  }
}
...
function showWallPrePost(appId, text, attachment, ownerId, callback) {
  if (boxTries > 0) {
    if (!wallPostBox) {
      wallPostBox = new MessageBox({title: getLang('box_loading'), progress: 'settingsProgress'});
    }
    wallPostBox.apiCallback = callback;
    wallPostBox.removeButtons();
    wallPostBox.addButton({label: box_close, onClick: function() {settingsBox.hide(200);}});
    ...
    boxTries--;
    setTimeout(function(){boxTries++;}, 3000);
  } else {
    callback('', {error_code: 10005, error_msg: "Too frequently"})
  }
}
...
function extScrollWindow(y, speed) {
  ...
  speed = intval(speed);
  if (speed && speed > 0) {
    animate(document.getElementsByTagName('html')[0], {scrollTop: scrollTop}, speed);
...
function extShowProfilePhotoBox(photoHash) {
  if (inlineApp) return;
  if (!profilePhotoBox) {
    profilePhotoBox = new MessageBox({progress: 'ppProgress', title: box_loading});
  }
  profilePhotoBox.removeButtons().addButton({label: box_close, onClick: function() {
    if (profilePhotoBoxCallback) {
      profilePhotoBoxCallback(false);
    }
    profilePhotoBox.hide();
  }}).loadContent('apps.php', {act: 'a_load_profile_photo', photo_hash: photoHash, id: appId}, true).show();
}

var appName;
function extSetNavigation(labels, links) {
  if (inlineApp) return;
  var appNav = ge('appNav'), appNavStr = "";
  if (!appName)
...
function extShowInstallBox() {
  ...
  if (!window.isAppUser) {
    if (installTries > 0) {
      showInstallBox();
      installTries--;
      setTimeout(function(){ installTries++; }, 3000);
    }
    return;
  }
...
function showWallPostBox(postHash) {
  if (!wallPostBox) {
    wallPostBox = new MessageBox({progress: 'wpProgress', title: box_loading});
  }
  if (wallPostBox.isVisible()) return;
  wallPostBox.removeButtons().addButton({label: box_close, onClick:function() {
    runCallback('customEvent', 'onWallPostCancel');
    wallPostBox.hide();
  }}).loadContent('apps.php', {act: 'a_load_wall_post', post_hash: postHash, id: appId}, true).show();
}
...
function showMerchantPaymentBox(params) {
  ...
  showBox('merchantPaymentBox', targetUrl, params, {reload: true, params: boxOptions})
...
function showInviteBox() {
 if (!inviteBox) {
  inviteBox = new PrivacyBox({params: {from:'apps', aid: window.app_id}, saveList: false, onPrivacySave: function(item, friendsIds) {
    friendsInvited = friendsIds;
    Ajax.Send('apps.php?act=a_invite_friends', {friends: friendsIds.join(','), app_id: window.app_id, app_hash: window.appHash}, onFriendsInvited);
  }});
 }
...
function onFriendsInvited(ajaxObj, responseText) {
 ...
 if (!invitedBox) {
   invitedBox = new MessageBox({title: app_invited});
...
function getVotes(from_box, count) {
  ...
  if (!window.getVotesBox) {
    window.getVotesBox = new MessageBox({title: window.lang_increase_balance, width: '480px', progress: 'getVotesProgress', bodyStyle: 'padding: 0px', onHide: function() {
  ...
  window.getVotesBox.removeButtons().addButton({label: window.lang_close, onClick: window.getVotesBox.hide});
  window.getVotesBox.loadContent('payments.php?return=apps', {'act':'a_get_votes_box', votes: count}, true).show();
  return false;
}
...
(function() {
  ...
  window.showInstallBox = function() {
    if (!installBox) {
      installBox = new MessageBox({title: window.lang_add_app_title, progress: 'joinProgress'});
      installBox.addButton({label: box_no, style: 'button_no', onClick: installBox.hide});
      installBox.addButton({label: box_yes, onClick: installApp});
  ...
  function installApp(app_id) {
    show('joinProgress');
    Ajax.Send('apps.php', {act: 'join', id: window.app_id, hash: appHash}, function(ajaxObj, responseText) {
  ...
  window.showPaymentBox = function(votes) {
    addingVotes = false;
    if (!paymentBox) {
      paymentBox = new MessageBox({title: window.lang_add_votes_title, progress: 'addVotesProgress'});
    }
    ...
    paymentBox.addButton({label: box_cancel, style: 'button_no', onClick: paymentBox.hide});
    paymentBox.addButton({label: window.lang_add_votes, onClick: addVotes});
  ...
  function addVotes() {
    ...
    Ajax.Send('apps.php', {act:'a_add_votes', id: window.app_id, hash: window.appHash, votes: votes}, {onSuccess: function(ajaxObj, responseText) {
      ...
      if (result.error) {
        ...
        hide('a_money_to_votes');
        show('apps_error');
        animate(ge('apps_error').firstChild, {backgroundColor: '#F9F6E7', borderColor: '#D4BC4C'}, 5000);
      } else {
        paymentBox.removeButtons().addButton({label: box_close, onClick: paymentBox.hide});
        ...
        setTimeout(function(){paymentBox.hide()}, 900);
      }
    }, onCaptchaShow: function() { hide('addVotesProgress'); }, onCaptchaHide: function(success) { if (success) paymentBox.show(); else showPaymentBox(window.paymentBoxVotes); }});
  }
})();
...
window.extFuncs = {
  ...
  setLocation: function() { extSetLocation.apply(null, arguments); },
...
try{stManager.done('apps_flash.js');}catch(e){}
