...
function showAboutAds() {
    if (!adBox) {
        adBox = new MessageBox({
            title: getLang('market_about_paid_ads')
        });
    }
    adBox.removeButtons().addButton({
        label: getLang('market_ok'),
        onClick: function() {
            adBox.hide();
        }
...
function showNoBox() {
    if (!noBox) {
        noBox = new MessageBox({
            title: getLang('market_available_later')
        });
    }
    ...
    $.ajax({
        ...
        success: function(data, textStatus) {
            ...
            if (where == 0) {
                ...
                setOpacity(ge('item' + id), 50);
            }
            ge('res' + id).innerHTML = data;
            show('res' + id);
        }
    });
    return false;
    ...
    $.ajax({
        ...
        success: function(data, textStatus) {
            if (where == 0) {
                show('item' + id);
                hide('res' + id);
                ge('faved' + id).innerHTML = data;
            } else {
                setOpacity(ge('item' + id), 100);
                ge('res' + id).innerHTML = data;
                show('res' + id);
            }
        }
...
function reportItem(id) {
    if (!reportBox) reportBox = new MessageBox({
        title: getLang('market_report')
    });
    reportBox.removeButtons().addButton({
        label: getLang('market_send'),
        onClick: function() {
            $.ajax({
                url: 'market.php?act=report',
                type: 'post',
                data: {
                    id: id,
                    reason: reason
...
function addToFavesSearch(id) {
    ...
    $('#faved' + id).load('market.php?act=add_to_faves&mode=1', {
        id: id,
        hash: ge('faveHash').value
    });
    plusFave();
    return false;
}
...
function minusFave() {
    var fave_num = parseInt(ge('faveNum').value);
    if (fave_num == 1 && ge('market_tab')) {
        ge('market_tab').style.display = 'none';
    }
    ge('faveNum').value = fave_num - 1;
}
...
function deleteFromFavesSearch(id) {
    ...
    $('#faved' + id).load('market.php?act=delete_from_faves&mode=1', {
        id: id,
        hash: ge('faveHash').value
    });
    minusFave();
...
function addToFaves(id) {
    var hash = ge('faveHash').value;
    $.ajax({
        url: 'market.php?act=add_to_faves',
        type: 'post',
        data: {
            id: id,
            hash: hash
...
function deleteFromFaves(id) {
    var hash = ge('faveHash').value;
    $.ajax({
        url: 'market.php?act=delete_from_faves',
        type: 'post',
        data: {
            id: id,
            hash: hash
    ...
    if (status == -1 && !sure) {
        ...
        if (selected_items.length) {
            ...
            confirm.addButton({
                label: '���',
                onClick: function() {
                    user_moderation_status(id, status, true);
                    confirm.hide();
                }
            });
            confirm.addButton({
                label: '��',
                onClick: function() {
                    location.replace('/market.php?act=delete_warn&items=' + selected_items);
                }
            });
            confirm.show();
            return;
...
function show_message_box(cls, msg) {
    ...
    setTimeout(function() {
        (new effects.fader()).fade(ge(cls), [0xFFFFFF, 0xDDDDDD], 3000);
    }, 2500);
}
...
function blacklist_user() {
    ...
    ajax.post('/market.php', {
        'act': 'a_blacklist_user',
        'page': ge('adding_page').value,
        'hash': ge('hash').value
    });
}
...
function unblacklist_user(user_id) {
    ...
    ajax.post('/market.php', {
        'act': 'a_unblacklist_user',
        'user_id': user_id,
        'page': ge('removing_page').value,
        'hash': ge('hash').value
    });
}
...
function spamlist_words() {
    ...
    ge('adding_progress').style.display = 'inline';
    ajax.post('/market.php', {
        'act': 'a_spamlist_words',
        'words': ge('adding_words').value,
        'hash': ge('hash').value
    });
}
...
function unspamlist_words(id) {
    ...
    ajax.onDone = function(obj, text) {
        if (id) {
            ...
            setTimeout(function() {
                new effects.fader().fade(ge('memberRow_' + id), [0xFFFFFF, 0xD8DFEA], 3000);
            }, 2500);
    ...
    ajax.post('/market.php', {
        'act': 'a_unspamlist_words',
        'id': id,
        'words': ge('removing_words').value,
        'hash': ge('hash').value
    });
}
