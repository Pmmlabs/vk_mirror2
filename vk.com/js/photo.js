...
function postComment() {
    ...
    var data = {
        ...
        aid: ge('aid').value,
        comment: ge('commentArea').value,
        hash: ge('hash').value,
        id: ge('id').value
    };
    Ajax.postWithCaptcha('/photos.php', data, {
        onSuccess: onDone,
        onFail: onFail,
        onCaptchaHide: onHide
    });
}
...
function dupPages() {
    var toppages = ge('toppages'),
        bottompages = ge('bottompages');
...
function showPhoto(params) {
    ...
    ge('myphoto').onload = function() {
        ge('myphoto').onload = null;
        testPhotoSize(ge('myphoto'));
    };
...
function editPhotoInit(album_id, owner_id) {
    ...
    sortable.makeSortable(ge('sortalbum'), {
        target: ge('albums'),
...
function showText(obj, mess) {
    var imgPath;
    if (mess == 1) {
        ge(obj).innerHTML = "<h4>" + photo_rotating + "<img src='http://vkontakte.ru/images/upload.gif'></h4>";
    }
    if (mess == 2) {
        ge(obj).innerHTML = "<div id='uploadingInner'><h4>" + photo_uploading_photo + "<img src='http://vkontakte.ru/images/upload.gif'></h4><p style='margin-top:11px'>" + photo_dont_close + "</p></div>";
    }
}
...
function testPhotoSize(photo) {
    ...
    if ((w > 100) && (h > 100)) {
        ...
        if (ge('album_to_profile')) {
            timerMenuVisible = setInterval(function() {
                hide('album_to_profile');
            }, 10);
            setTimeout(function() {
                if (timerMenuVisible) clearInterval(timerMenuVisible);
            }, 800);
...
function likePhoto(oid, photo_id, hash) {
    ...
    Ajax.Send('like.php?act=a_do_like', {
        'object': 'photo' + oid + '_' + photo_id,
        hash: hash
    }, {
...
function unlikePhoto(oid, photo_id, hash) {
    ...
    Ajax.Send('like.php?act=a_do_unlike', {
        'object': 'photo' + oid + '_' + photo_id,
        hash: hash
    }, {
        onSuccess: function(o, t) {
...
function showLikesBox(oid, photo_id) {
    showBox('likesBox', 'like.php?act=a_get_members', {
        'object': 'photo' + oid + '_' + photo_id
    }, false, true, {
        height: 381,
        width: 478,
...
function getLikedByPage(item, offset) {
    winBoxes.likesBox.loadContent('like.php?act=a_get_members', {
        'object': item,
        offset: offset
    }, true);
    return false;
...
function photoNavState(el, state) {
    ...
    animate(el.firstChild.firstChild, {
        opacity: state ? 1 : 0.7
    }, 100);
    animate(el.firstChild, {
        backgroundColor: state ? '#e1e7ed' : '#FFF'
    }, 80);
}