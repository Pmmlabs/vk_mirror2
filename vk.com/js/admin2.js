...
function showRecentHistory(hash) {
    var params = {
        act: 'a_moder_names_recent_history',
        hash: hash
    };
    mb = new MessageBox({
        title: '������� ������������ ������',
...
function searchName(id, newName) {
    ...
    ajax.post('/admin.php', {
        ...
        'id': ajax.id,
        'name': ajax.name,
        'parent_id': ajax.parent_id,
        'type_id': ajax.type_id
    });
...
function added(id, act, cb) {
    if (act == 'a_postpone') {
        searchSuccess({
            id: id
        }, "{html: '<div id=\"msg\" style=\"margin:10px 0px\">������ ��������.</div>'}");
...
function added_items_toggle_rev(el) {
    var input_node = ge('added_items_rev'),
        rev_value = parseInt(input_node.value);
...
function added_items_page(page) {
    ...
    ajax.post('/admin.php', {
        'act': 'a_added_items_page',
        'index': page,
        'filter': filter_current,
        'c': ge('country_type').innerHTML,
        'user': ge('user').value,
        rev: ge('added_items_rev').value
    });
...
function toggle_informer_moders() {
    if (informer_moders_visible) {
        ...
        events.addEvent(document, 'keydown', function(e) {
            if (e.keyCode == 27) toggle_informer_moders();
        });
...
function add_informer_moder() {
    if (ge('page').value.replace(/\s/g, '').length) {
        ...
        ajax.onDone = function(obj, text) {
            ...
            setTimeout(function() {
                (new effects.fader()).fade(ge('addingModeratorMessage'), [0xFFFFFF, 0xDDDDDD], 3000);
            }, 2500);
        }
        ajax.post('admin.php', {
            'act': 'a_add_informer_moder',
            'page': ge('page').value.replace(/\s/g, '')
        });
    } else {
        ge('page').focus();
    }
...
function remove_informer_moder(id) {
    ...
    ajax.onDone = function(obj, text) {
        if (text.charAt(0) == 'e') {
            if (text.charAt(1) == '1') {} else if (text.charAt(1) == '2') {} else {}
...
var Datepicker = function(el, options) {
    ...
    var showCalendar = function() {
        ...
        setStyle(calendarFrame, {
            width: calendarDiv.offsetWidth,
            height: calendarDiv.offsetHeight
        });
        //dpShown = _t;
        ...
        setStyle(calendarBox, {
            top: xy[1] + dateInput.offsetHeight + 1,
            left: xy[0] - 1 + (window.is_rtl ? getSize(dateInput)[0] : 0)
        });
        dateInput.focus();
    };
    ...
    setStyle(wrap, {
        ...
        backgroundPosition: (w - 18) + 'px center',
