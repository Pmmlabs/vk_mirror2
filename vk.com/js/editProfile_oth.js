...
var uiSex, uiStatus, uiPartner, uiBday, uiBmonth, uiByear, uiBdayVisibility, uiPolitical, uiReligion, friendsFull = false,
    help_timeout = false;
...
function onFriendsLoaded(type, selected) {
    ...
    debugLog(2);
    var relDD = new Selector(ge(type + rel_guid), friends_list, {
        width: 220,
        placeholder: placeholders[type],
        introText: '',
        multiselect: false,
        enableCustom: true,
        defaultItems: friends_list,
    ...
    if (withDate) {
        ...
        var dayDD = new Dropdown(ge(id + '_day'), generate_days(d.getFullYear(), d.getMonth() + 1), {
            width: 55,
            selectedItems: t ? d.getDate() : 0,
            onChange: onDateChange
        });
        var monthDD = new Dropdown(ge(id + '_month'), general_data.bmonths, {
            width: 95,
...
function saveGeneral() {
    ...
    each(geByClass('relation_input', ge('children')), function() {
        ...
        params['children_date[' + i + ']'] = (geByClass('dateField', this)[0] || {
            value: 0
        }).value;
...
function generate_days(year, month) {
    var result = [
            [0, lang.profileEdit_main_sel_bday + ':']
        ],
        last = get_last_day(year, month);
...
function do_update_friends_list() {
    ...
    uiPartner.setOptions({
        defaultItems: data[0]
    });
    uiPartner.setOptions({
        placeholder: data[0][0][1]
    });
...
onDomReady(function() {
    if (act == 'contacts') {
        ...
        each(['activities', 'interests', 'music', 'movies', 'tv', 'books', 'games', 'quotes', 'about'], function() {
            ...
            autosizes['' + this] = new Autosize(ge('' + this), {
                minHeight: 80
            });
        });
    }
