    ...
    createChildClass('MentionAutocomplete', UiControl, {
        ...
        initDOM: function(input) {
            this.textareaEl.parentNode.insertBefore(
                this.cont = ce('div', {
                    className: 'mini_editor_cont selector_container'
                }),
                this.textareaEl.nextSibling
            );
            this.cont.appendChild(
                this.rtaEl = ce('div', {
                    className: 'mention_rich_ta',
        ...
        initEvents: function() {
            ...
            this.textareaEl.getValue = function() {
                return self.updateWikiValue(true, true);
            };
            var oldSV = this.textareaEl.setValue;
            this.textareaEl.setValue = function(v) {
                self.rtaEl.innerHTML = v ? v : (browser.mozilla ? '<br/>' : ' ');
                self.updateWikiValue(true);
                if (oldSV) oldSV(v);
            }
        },
        ...
        onFriendSelect: function(friendId) {
            ...
            var focusNode = this.lastNode,
                focusOffset = this.lastOffset,
                focusValue = focusNode.data ? focusNode.data : focusNode.innerText;
            if (!focusValue.substr) focusValue = '';
            var friendName = (friendId > 0) ? ('id' + friendId) : ('club' + (-friendId)),
                href = friendName;
        ...
        fixContents: function() {
            ...
            var fixEl = function() {
                ...
                if (this.tagName == 'A' || this.tagName == 'P')
...
try {
    jsDispatcher.triggerOnload('mentions', initMentionClass);
} catch (e) {}
try {
    stManager.done('mentions.js');
} catch (e) {}