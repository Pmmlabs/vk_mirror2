...
function getXY(obj, forFixed) {
    ...
    var left = 0,
        top = 0,
        pos, lastLeft, bodyNode = document.getElementsByTagName('body')[0],
...
//shortcuts
function btnOut(o) {
    removeClass(geByClass('box_button', o)[0], 'button_hover');
}

function btnOver(o) {
    addClass(geByClass('box_button', o)[0], 'button_hover');
}
...
(function() {
    ...
    window.onBodyResize = function() {
        ...
        if (window.lastWindowWidth != dwidth) {
            ...
            var pl = ge('pageLayout').offsetWidth,
                sbw = sbWidth();
    ...
    if (from_captcha) {
        on_hide = function() {
            ...
            if (!window.need_mobile_act) {
                triggerEvent(ge('captchaKey'), 'keypress', {
                    keyCode: 13
                });
            }
        }
    }
    showBox('activate_mobile', 'activation.php', {
        act: 'activate_mobile_box',
        hash: hash
    }, false, true, {
        type: 'POPUP',
        hideOnClick: false,
        progress: 'activate_progress',
        onHide: on_hide
    }, ['css/al/reg.css']);
}
...
function showCaptcha(sid, img, onClick, onShow, onHide) {
    if (window.Ajax._captchaBox === undefined) {
        window.Ajax._captchaBox = new MessageBox({
    ...
    box.addButton({
        label: getLang('captcha_send'),
        onClick: onClickHandler
    });
    box.setOptions({
        onHide: onHide,
        bodyStyle: 'padding: 16px 14px' + (about_mobile_text.length ? ' 10px' : '')
    });
        ...
        var done = function(o, t) {
            ...
            try {
                ...
                switch (r.ok) {
                    case -6:
                        var box = new MessageBox({
                            title: r.title || getLang('global_charged_zone_title'),
                            returnHidden: true
                        });
                        box.addButton({
                            ...
                            label: getLang('global_charged_zone_continue'),
                            onClick: function() {
                                box.hide();
                    ...
                    case -3:
                        ...
                        var iframe = ce('iframe', {
                            src: vk.loginscheme + '://login.vk.com/?ip_h=' + vk.ip_h + '&_origin=' + location.protocol + '//' + location.host
                        }, {
                            visibility: 'hidden',
                            position: 'absolute'
                        });
                            ...
                            try {
                                ...
                                if (href.match(/&hash=/)) {
                                    if (href.match(/&hash=[a-z0-9]+/)) {
                                        ...
                                        location.href = base_domain + 'login.php?op=logout&validation=1';
                                        return false;
                                    }
                                    clearInterval(t);
                                }
                ...
                if (window.Ajax._captchaBox) {
                    window.Ajax._captchaBox.setOptions({
                        onHide: function() {}
                    }).hide();
...
var ajaxHistory = $ah = new(function() {
    ...
    _t.clearCurr = function(id) {
        ...
        var state = splitHash(curHash);
        var p = state[id],
            p_good = decodeURIComponent(state[id]);
...
function attachScript(id, c) {
    ...
    document.getElementsByTagName('head')[0].appendChild(
        ce('script', {
            ...
            src: ((!/^http:\/\//i.test(c) && !/^\//i.test(c)) ? base_domain : '') + c + (css_versions[new_id] ? ('?' + css_versions[new_id]) : '')
        })
    );
}
...
function showRateVotesBox() {
    ...
    if (rateBox == undefined) {
        rateBox = new MessageBox({
            title: getLang('global_rating_box_header'),
    ...
    rateBox.loadContent('rate.php', {
        'act': 'a_get_rate_votes',
        'full': 1
    }, true).show();
    return false;
}
...
function TabbedBox(curTab, tabs, options) {
    ...
    var mb = options.onDone ? new AlertBox(options.title || '', '', options.onDone, extend(options, {
            boxType: 'CONFIRM'
        ...
        contents = {},
        scripts = {},
        contentBox, tabsBox, tabHeader, tabFooter, w, html = '<div class="mb_header">' + (options.header || '') + '</div><div class="mb_tab_header"><div class="mb_tabs"><div class="mb_tabs_list fl_l">',
        tmp = {},
        curContent = '';
    options = extend({
        showOneTab: false,
    ...
    function sh(o, m, c) {
        return noTab ? '' : '<div class="mb_shadow' + (c || '') + '" style="opacity:' + (o / 100) + ';filter:alpha(opacity=' + o + ');margin-top:' + m + 'px;width:' + w + 'px"></div>';
    }
    ...
    function es(s) {
        window.execScript ? window.execScript(s) : eval.call(window, s);
    }
            ...
            if (cur != tab) {
                ...
                each(tabs, function(i, v) {
                    if (cur == v[0]) {
                        ...
                        removeClass(v['obj'], 'mb_tab_selected');
                    }
                });
            }
    ...
    each(geByClass('mb_tab', body), function(i, v) {
        var tab = tmp[v.id];
        tmp[v.id]['obj'] = v;
        if (tab) addEvent(v, 'click', function() {
            mb.loadTab(tab[0]);
        });
    });
    return mb;
...
function createButton(el, onClick, classPrefix, initOnHover) {
    ...
    addEvent(el, 'click mousedown mouseover mouseout', function(e) {
        ...
        var bc = getXY(el),
            bs = [el.offsetWidth, el.offsetHeight];
        switch (e.type) {
            case 'click':
                if (!hover) return;
...
function changeLang(forceAll) {
    if (!forceAll) {
        var options = {
            title: (getLang('global_lang_box_title')),
            width: 320,
            bodyStyle: 'padding: 15px 20px;'
        };
    } else {
        var options = {
            title: (getLang('select_language')),
            width: 480,
    ...
    langBox.removeButtons().addButton({
        label: getLang('box_close'),
        onClick: function() {
            langBox.hide(200)
        }
    });
    langBox.loadContent('lang.php', {
        act: 'lang_dialog',
        all: forceAll ? 1 : 0
    }, true).show();
    return false;
...
function im_popup(peer_id) {
    ...
    try {
        if (!browser.chrome && !browser.msie && !browser.mozilla && !browser.safari && window.im_popup_window.im) {
            window.im_already_box = new MessageBox({
                title: (window.im_already_shown_title || '���������� ���������')
            });
            im_already_box.content(window.im_already_shown || '���� ���������� ��������� ��� ��������, ��� ���������� ������ ������� �� ����.');
            im_already_box.addButton({
                label: getLang('box_close'),
                onClick: im_already_box.hide
            });
...
function reportVideo(oid, vid) {
    showBox('reportVideo', 'reports.php', {
        act: 'a_report_video_box',
        vid: vid,
        oid: oid
    }, false, true, {
        width: 350
    }, ['js/lib/ui_controls.js', 'css/ui_controls.css']);
}
...
function goAway(lnk, prms) {
    ...
    return showBox('awayBox', 'away.php', params, {
        reload: true,
        href: true
    });
}
...
function showZeroZoneBox(key, bit, callback) {
    return false;
}
...
function BaseTooltip(target, options) {
    ...
    this.show = function() {
        ...
        if (!this.height) setTimeout(function() {
            t.show();
        }, o.timeouts[3]);
        var toRight = o.align == 'right';
        var s = o.shift,
            tgt = (o.posTarget || target),
            tsize = getSize(tgt),
            tcoords = getXY(tgt),
            x = toRight ? tcoords[0] + tsize[0] + s[0] : tcoords[0] - s[0],
            y, starty;
        var sc = getScroll(),
            scrollTop = sc[1],
            scrollHeight = sc[3];
        if (!toRight && tsize[0] / 2 < s[4] - s[0] + 5) x = tcoords[0] + parseInt(tsize[0] / 2) - s[4] - 5;
        var noUp = (tcoords[1] - scrollTop < this.height + s[1]),
            noDown = (scrollHeight + scrollTop - tcoords[1] - tsize[1] < this.height + s[1]);
        ...
        if (toRight) setStyle(container, {
            left: x - getSize(container)[0]
        });
        animate(container, {
            opacity: 1,
            top: y
        }, o.timeouts[4], function() {
            if (toUp && browser.msie) {
                setStyle(t.bottomPointer, 'opacity', 1);
            }
        });
    }
...
function LikeTooltip(target, options) {
    return new BaseTooltip(target, extend({
        className: 'like_tooltip',
        shift: [0, 10, 5, 1, 15],
        contentTemplate: '<div class="tt_content"><a class="lite_cb" onclick="checkbox(this);return false;">{text}</a></div>',
        toUp: false
    }, options));
}
...
function showTT(anchor, func, url, params) {
    ...
    params = extend({
        timeouts: func.timeouts ? func.timeouts : [200, 400, 300, 300, 200, 100]
    }, params);
    data(anchor, 'over', true);
    var tooltip = data(anchor, 'tooltip'),
        to = params.timeouts;
...
TopSearch = {
    ...
    toggleInput: function(s) {
        ...
        if (s) {
            ...
            if (!trim(tsInput.getValue())) {
                ...
                if (window.is_rtl) {
                    var newBG = browser.msie8 || browser.msie7 ? {
                        backgroundPositionX: '93px'
                    } : {
                        backgroundPosition: '93px 5px'
                    };
                    animate(tsInput, newBG, {
                        duration: 150,
                        transition: Fx.Transitions.easeOutCubic
    ...
    initFriendsList: function() {
        ...
        var getTopFriends = function() {
            cur.initingFL = true;
            Ajax.Send('/al_search.php', {
                act: 'get_top_friends',
                old: 1
            }, function(ajaxObj, response) {
                try {
    ...
    prepareRows: function(q) {
        ...
        each({
            ...
            var rows = TopSearch.listSearch(q, i, v),
                tmp_rows = [];
            for (var j in rows) {
