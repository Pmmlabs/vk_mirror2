...
onSortEnd = function(elem) {
	...
	if (startPos != newPos && newPos >= 0 && mid[1] ) {
		 Ajax.Send('groups_ajax.php', {act: 'aorderofficers', id: mid[1], gid: groupId, oldpos: startPos, newpos: newPos});
	}
};
...
function sendRequest(mid, gid, act, hash, callback) {
  ...
  Ajax.Send('groups_ajax.php', {act: act, gid: gid, id:mid, hash: hash}, function(ajaxObj, responseText){
    var result = eval('('+responseText+')');
    if (result.result) {
      actions.innerHTML = result.result;
      animate(el, {backgroundColor: '#E7F1F9', borderColor: '#4C96D4'}, 800);
      setTimeout(function(){animate(el, {backgroundColor: '#FFFFFF', borderColor: '#D8DFEA'}, 3000);}, 3000);
...
function editOfficer(mid, gid, hash, name) {
 if (!officerBox) officerBox = new MessageBox({progress: 'officer_progress'});
 officerBox.setOptions({title: name});
 ...
 officerBox.addButton({label: groups_cancel, style: 'button_no', onClick: officerBox.hide}).addButton({label: groups_edit, onClick: function(){
  var position = ge('position').value, admin = uiIsAdmin.val();
  position = position.replace(/'/g, "&#39;").replace(/>/g, "&rt;").replace(/</g, "&lt;");
  //officerBox.content('<div class="box_loader"></div>');
  show('officer_progress');
  Ajax.Send('groups_ajax.php', {act: 'a_edit_officer', id: mid, gid: gid, hash: hash, position: position, admin: admin}, function() {
    officerBox.hide();
...
function appointMember(mid, gid, hash, name) {
  if(!appointMB)appointMB = new MessageBox({progress: 'officer_progress'});
  appointMB.setOptions({title:name});
  appointMB.removeButtons().addButton({label: groups_cancel, style: 'button_no', onClick: appointMB.hide}).addButton({label: groups_assign, onClick:function(){
    var position = ge('position'+mid).value;
    var admin = uiIsAdmin.val();
    show('officer_progress');
    Ajax.Send('groups_ajax.php', {act: 'a_appoint_officer', id: mid, gid: gid, hash: hash, position: position, admin: admin}, {onSuccess: function(res, text) {
      appointMB.hide();
      hide('officer_progress');
